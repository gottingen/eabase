...
#include "eabase/fiber/errno.h"                 // EWOULDBLOCK
#include "eabase/fiber/sys_futex.h"             // futex_*
...
#ifdef SHOW_FIBER_BUTEX_WAITER_COUNT_IN_VARS
struct ButexWaiterCount : public eabase::Adder<int64_t> {
    ButexWaiterCount() : eabase::Adder<int64_t>("fiber_butex_waiter_count") {}
};
inline eabase::Adder<int64_t>& butex_waiter_count() {
    return *eabase::get_leaky_singleton<ButexWaiterCount>();
}
#endif
...
struct ButexPthreadWaiter : public ButexWaiter {
    eabase::atomic<int> sig;
};

typedef eabase::LinkedList<ButexWaiter> ButexWaiterList;

enum ButexPthreadSignal { PTHREAD_NOT_SIGNALLED, PTHREAD_SIGNALLED };

struct EA_CACHELINE_ALIGNMENT Butex {
    Butex() {}
    ~Butex() {}

    eabase::atomic<int> value;
    ButexWaiterList waiters;
    internal::FastPthreadMutex waiter_lock;
};

    static_assert(offsetof(Butex, value) == 0, "offsetof_value_must_0");
    static_assert(sizeof(Butex) == BAIDU_CACHELINE_SIZE, "butex_fits_in_one_cacheline");

static void wakeup_pthread(ButexPthreadWaiter* pw) {
    // release fence makes wait_pthread see changes before wakeup.
    pw->sig.store(PTHREAD_SIGNALLED, eabase::memory_order_release);
...
int wait_pthread(ButexPthreadWaiter& pw, const timespec* abstime) {
    ...
    while (true) {
        if (abstime != NULL) {
            timeout_us = eabase::timespec_to_microseconds(*abstime) - eabase::gettimeofday_us();
            timeout = eabase::microseconds_to_timespec(timeout_us);
            ptimeout = &timeout;
        }
        if (timeout_us > MIN_SLEEP_US || abstime == NULL) {
            rc = futex_wait_private(&pw.sig, PTHREAD_NOT_SIGNALLED, ptimeout);
            if (PTHREAD_NOT_SIGNALLED != pw.sig.load(eabase::memory_order_acquire)) {
        ...
        if (rc != 0 && errno == ETIMEDOUT) {
            ...
            if (!erase_from_butex(&pw, false, WAITER_STATE_TIMEDOUT)) {
                ...
                // Acquire fence makes this thread sees changes before wakeup.
                if (pw.sig.load(eabase::memory_order_acquire) == PTHREAD_NOT_SIGNALLED) {
...
void* butex_create() {
    Butex* b = eabase::get_object<Butex>();
    if (b) {
        return &b->value;
    }
    return NULL;
...
void butex_destroy(void* butex) {
    ...
    Butex* b = static_cast<Butex*>(
        container_of(static_cast<eabase::atomic<int>*>(butex), Butex, value));
    eabase::return_object(b);
}
...
int butex_wake(void* arg, bool nosignal) {
    Butex* b = container_of(static_cast<eabase::atomic<int>*>(arg), Butex, value);
    ButexWaiter* front = NULL;
    {
        ...
        front->container.store(NULL, eabase::memory_order_relaxed);
    }
    if (front->tid == 0) {
...
int butex_wake_all(void* arg, bool nosignal) {
    ...
    ButexWaiterList pthread_waiters;
    {
        ...
        while (!b->waiters.empty()) {
            ...
            bw->RemoveFromList();
            bw->container.store(NULL, eabase::memory_order_relaxed);
            if (bw->tid) {
...
int butex_requeue(void* arg, void* arg2) {
    Butex* b = container_of(static_cast<eabase::atomic<int>*>(arg), Butex, value);
    Butex* m = container_of(static_cast<eabase::atomic<int>*>(arg2), Butex, value);
    ...
    ButexWaiter* front = NULL;
    {
        ...
        while (!b->waiters.empty()) {
            ...
            bw->RemoveFromList();
            m->waiters.Append(bw);
            bw->container.store(m, eabase::memory_order_relaxed);
        }
    }
...
inline bool erase_from_butex(ButexWaiter* bw, bool wakeup, WaiterState state) {
    ...
    while ((b = bw->container.load(eabase::memory_order_acquire))) {
        // b can be NULL when the waiter is scheduled but queued.
        BAIDU_SCOPED_LOCK(b->waiter_lock);
        if (b == bw->container.load(eabase::memory_order_relaxed)) {
            bw->RemoveFromList();
            bw->container.store(NULL, eabase::memory_order_relaxed);
            if (bw->tid) {
...
static int butex_wait_from_pthread(TaskGroup* g, Butex* b, int expected_value,
    ...
    pw.tid = 0;
    pw.sig.store(PTHREAD_NOT_SIGNALLED, eabase::memory_order_relaxed);
    int rc = 0;
    
    if (g) {
        task = g->current_task();
        task->current_waiter.store(&pw, eabase::memory_order_release);
    }
    b->waiter_lock.lock();
    if (b->value.load(eabase::memory_order_relaxed) != expected_value) {
        ...
        b->waiters.Append(&pw);
        pw.container.store(b, eabase::memory_order_relaxed);
        b->waiter_lock.unlock();

#ifdef SHOW_FIBER_BUTEX_WAITER_COUNT_IN_VARS
        eabase::Adder<int64_t>& num_waiters = butex_waiter_count();
        num_waiters << 1;
#endif
    ...
    if (task) {
        ...
        BT_LOOP_WHEN(task->current_waiter.exchange(
                         NULL, eabase::memory_order_acquire) == NULL,
...
int butex_wait(void* arg, int expected_value, const timespec* abstime) {
    ...
    if (b->value.load(eabase::memory_order_relaxed) != expected_value) {
        ...
        eabase::atomic_thread_fence(eabase::memory_order_acquire);
        return -1;
    }
    TaskGroup* g = tls_task_group;
    ...
    bbw.container.store(NULL, eabase::memory_order_relaxed);
    ...
    if (abstime != NULL) {
        ...
        if (eabase::timespec_to_microseconds(*abstime) <
            (eabase::gettimeofday_us() + MIN_SLEEP_US)) {
            // Already timed out.
            errno = ETIMEDOUT;
            return -1;
        }
    ...
    bbw.task_meta->current_waiter.store(&bbw, eabase::memory_order_release);
...
namespace eabase {
template <> struct ObjectPoolBlockMaxItem<eabase::Butex> {
