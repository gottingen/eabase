...
#ifndef  FIBER_CONDITION_VARIABLE_H_
#define  FIBER_CONDITION_VARIABLE_H_
...
namespace eabase {
    ...
    class ConditionVariable {
        EA_DISALLOW_COPY_AND_ASSIGN(ConditionVariable);
    ...
    public:
        ...
        void wait(std::unique_lock<eabase::FiberMutex> &lock) {
        ...
        int wait_for(std::unique_lock<eabase::FiberMutex> &lock,
                     long timeout_us) {
            return wait_until(lock, eabase::microseconds_from_now(timeout_us));
        }
        ...
        int wait_until(std::unique_lock<eabase::FiberMutex> &lock,
                       timespec duetime) {
...
#endif  // FIBER_CONDITION_VARIABLE_H_
