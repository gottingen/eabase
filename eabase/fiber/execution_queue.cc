...
#include "eabase/utility/object_pool.h"           // eabase::get_object
#include "eabase/utility/resource_pool.h"         // eabase::get_resource
...
namespace /*anonymous*/ {
typedef eabase::ResourceId<ExecutionQueueBase> slot_id_t;
...
struct ExecutionQueueVars {
    eabase::Adder<int64_t> running_task_count;
    eabase::Adder<int64_t> execq_count;
    eabase::Adder<int64_t> execq_active_count;
    
    ExecutionQueueVars();
};

ExecutionQueueVars::ExecutionQueueVars()
    : running_task_count("fiber_execq_running_task_count")
    , execq_count("fiber_execq_count")
    , execq_active_count("fiber_execq_active_count") {
}

inline ExecutionQueueVars* get_execq_vars() {
    return eabase::get_leaky_singleton<ExecutionQueueVars>();
}
...
void ExecutionQueueBase::start_execute(TaskNode* node) {
    ...
    if (node->high_priority) {
        ...
        _high_priority_tasks.fetch_add(1, eabase::memory_order_relaxed);
    }
    TaskNode* const prev_head = _head.exchange(node, eabase::memory_order_release);
    if (prev_head != NULL) {
        node->next = prev_head;
        return;
    }
    ...
    if (node->in_place) {
        ...
        // return if no more
        if (node->high_priority) {
            _high_priority_tasks.fetch_sub(niterated, eabase::memory_order_relaxed);
        }
    ...
    if (nullptr == _options.executor) {
        if (_options.use_pthread) {
            ...
            if (fiber_start_lazy(&tid, &_options.fiber_attr,
...
void* ExecutionQueueBase::_execute_tasks(void* arg) {
    ...
    for (;;) {
        ...
        int rc = 0;
        if (m->_high_priority_tasks.load(eabase::memory_order_relaxed) > 0) {
            int nexecuted = 0;
            // Don't care the return value
            rc = m->_execute(head, true, &nexecuted);
            m->_high_priority_tasks.fetch_sub(
                    nexecuted, eabase::memory_order_relaxed);
            if (nexecuted == 0) {
    ...
    if (destroy_queue) {
        CHECK(m->_head.load(eabase::memory_order_relaxed) == NULL);
        CHECK(m->_stopped);
        ...
        m->_join_butex->fetch_add(2, eabase::memory_order_release/*1*/);
        butex_wake_all(m->_join_butex);
        vars->execq_count << -1;
        eabase::return_resource(slot_of_id(m->_this_id));
    }
    vars->execq_active_count << -1;
    return NULL;
}
...
void* ExecutionQueueBase::_execute_tasks_pthread(void* arg) {
    eabase::PlatformThread::SetName("ExecutionQueue");
    ...
    while (true) {
        ...
        if (expected != m->_join_butex->load(eabase::memory_order_relaxed)) {
...
void ExecutionQueueBase::return_task_node(TaskNode* node) {
    ...
    eabase::return_object<TaskNode>(node);
...
void ExecutionQueueBase::_on_recycle() {
    // Push a closed tasks
    while (true) {
        TaskNode* node = eabase::get_object<TaskNode>();
...
int ExecutionQueueBase::join(uint64_t id) {
    ...
    ExecutionQueueBase* const m = eabase::address_resource(slot);
    if (m == NULL) {
    ...
    while (expected == m->_join_butex->load(eabase::memory_order_acquire)) {
...
int ExecutionQueueBase::stop() {
    ...
    uint64_t vref = _versioned_ref.load(eabase::memory_order_relaxed);
    for (;;) {
        ...
        if (_versioned_ref.compare_exchange_strong(
            ...
            _stopped.store(true, eabase::memory_order_release);
...
TaskNode* ExecutionQueueBase::allocate_node() {
    get_execq_vars()->running_task_count << 1;
    return eabase::get_object<TaskNode>();
}
...
int ExecutionQueueBase::create(uint64_t* id, const ExecutionQueueOptions* options,
    ...
    slot_id_t slot;
    ExecutionQueueBase* const m = eabase::get_resource(&slot, Forbidden());
    if (BAIDU_LIKELY(m != NULL)) {
        ...
        CHECK(m->_head.load(eabase::memory_order_relaxed) == NULL);
        CHECK_EQ(0, m->_high_priority_tasks.load(eabase::memory_order_relaxed));
        ExecutionQueueOptions opt;
        if (options != NULL) {
            opt = *options;   
        }
        m->_options = opt;
        m->_stopped.store(false, eabase::memory_order_relaxed);
        m->_this_id = make_id(
                _version_of_vref(m->_versioned_ref.fetch_add(
                                    1, eabase::memory_order_release)), slot);
        *id = m->_this_id;
...
inline bool TaskIteratorBase::should_break_for_high_priority_tasks() {
    if (!_high_priority && 
            _q->_high_priority_tasks.load(eabase::memory_order_relaxed) > 0) {
        _should_break = true;
        return true;
    }
    return false;
