...
#ifndef  FIBER_EXECUTION_QUEUE_INL_H_
#define  FIBER_EXECUTION_QUEUE_INL_H_

#include "eabase/utility/atomicops.h"             // eabase::atomic
#include "eabase/utility/macros.h"                // EA_CACHELINE_ALIGNMENT
#include "eabase/utility/memory/scoped_ptr.h"     // eabase::scoped_ptr
#include "eabase/utility/logging.h"               // LOG
#include "eabase/utility/time.h"                  // eabase::cpuwide_time_ns
#include "eabase/var/var.h"                  // eabase::Adder
#include "eabase/fiber/butex.h"              // butex_construct
...
struct EA_CACHELINE_ALIGNMENT TaskNode {
    TaskNode()
        : version(0)
        , status(UNEXECUTED)
        , stop_task(false)
    ...
    eabase::Mutex mutex;  // to guard version and status
    int64_t version;
    uint8_t status;
    bool stop_task;
    bool iterated;
    ...
    void clear_before_return(clear_task_mem clear_func) {
        ...
        q = NULL;
        std::unique_lock<eabase::Mutex> lck(mutex);
        ++version;
...
class EA_CACHELINE_ALIGNMENT ExecutionQueueBase {
EA_DISALLOW_COPY_AND_ASSIGN(ExecutionQueueBase);
struct Forbidden {};
...
public:
    ...
    ExecutionQueueBase(Forbidden)
        ...
        , _cond(&_mutex)
        , _current_head(NULL) {
        _join_butex = butex_create_checked<eabase::atomic<int> >();
        _join_butex->store(0, eabase::memory_order_relaxed);
    }
    ...
    bool stopped() const { return _stopped.load(eabase::memory_order_acquire); }
    int stop();
...
private:
    ...
    EA_CACHELINE_ALIGNMENT eabase::atomic<TaskNode*> _head;
    EA_CACHELINE_ALIGNMENT eabase::atomic<uint64_t> _versioned_ref;
    EA_CACHELINE_ALIGNMENT eabase::atomic<bool> _stopped;
    eabase::atomic<int64_t> _high_priority_tasks;
    uint64_t _this_id;
    void* _meta;
...
public:
    ...
    typedef eabase::ExecutionQueueId<T>                        id_t;
    ...
    int execute(typename eabase::add_const_reference<T>::type task) {
...
#endif  // FIBER_EXECUTION_QUEUE_INL_H_
