...
#  include <iostream>
#  include "eabase/fiber/mutex.h"        // use fiber_mutex_t in the RAII way

#endif

#include "eabase/fiber/session.h"
...
extern int fiber_start(fiber_t *__restrict tid,
...
extern int fiber_start_lazy(fiber_t *__restrict tid,
                                    const fiber_attr_t *__restrict attr,
                                    void *(*fn)(void *),
                                    void *__restrict args);
...
// Terminate calling eabase/fiber/pthread and make `retval' available to any
...
extern int fiber_key_create(fiber_key_t *key,
...
// Returns 0 on success, error code otherwise.
extern int fiber_key_delete(fiber_key_t key);
...
// NOTE: If the thread is not created by eabase server and lifetime is
...
// but noticeable in shortly-lived threads. Threads in eabase server
// are special since they reuse keytables from a fiber_keytable_pool_t
// in the server.
// Returns 0 on success, error code otherwise.
// If the key is invalid or deleted, return EINVAL.
extern int fiber_setspecific(fiber_key_t key, void *data);
...
extern void *fiber_getspecific(fiber_key_t key);

// Return current fiber tag
extern fiber_tag_t fiber_self_tag(void);

extern void fiber_assign_data(void* data);

extern void* fiber_get_assigned_data();

extern int is_running_on_fiber();

extern int is_running_on_pthread();
