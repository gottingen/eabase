...
#include "eabase/fiber/errno.h"                       // EAGAIN
...
// Stats.
    static eabase::static_atomic<size_t> nkeytable = BUTIL_STATIC_ATOMIC_INIT(0);
    static eabase::static_atomic<size_t> nsubkeytable = BUTIL_STATIC_ATOMIC_INIT(0);

// The second-level array.
// Align with cacheline to avoid false sharing.
    class EA_CACHELINE_ALIGNMENT SubKeyTable {
    public:
        SubKeyTable() {
            memset(_data, 0, sizeof(_data));
            nsubkeytable.fetch_add(1, eabase::memory_order_relaxed);
        }

        // NOTE: Call clear first.
        ~SubKeyTable() {
            nsubkeytable.fetch_sub(1, eabase::memory_order_relaxed);
        }
        ...
        void clear(uint32_t offset) {
            for (uint32_t i = 0; i < KEY_2NDLEVEL_SIZE; ++i) {
                ...
                if (p) {
                    ...
                    KeyInfo info = eabase::s_key_info[offset + i];
...
// Align with cacheline to avoid false sharing.
    class EA_CACHELINE_ALIGNMENT KeyTable {
    public:
        KeyTable() : next(NULL) {
            memset(_subs, 0, sizeof(_subs));
            nkeytable.fetch_add(1, eabase::memory_order_relaxed);
        }

        ~KeyTable() {
            nkeytable.fetch_sub(1, eabase::memory_order_relaxed);
...
// Caller of this function must hold the KeyTable
    ...
    static int get_key_count(void *) {
        BAIDU_SCOPED_LOCK(eabase::s_key_mutex);
        return (int) nkey - (int) nfreekey;
    }

    static size_t get_keytable_count(void *) {
        return nkeytable.load(eabase::memory_order_relaxed);
    }

    static size_t get_keytable_memory(void *) {
        const size_t n = nkeytable.load(eabase::memory_order_relaxed);
        const size_t nsub = nsubkeytable.load(eabase::memory_order_relaxed);
        return n * sizeof(KeyTable) + nsub * sizeof(SubKeyTable);
    }

    static eabase::PassiveStatus<int> s_fiber_key_count(
            "fiber_key_count", get_key_count, NULL);
    static eabase::PassiveStatus<size_t> s_fiber_keytable_count(
            "fiber_keytable_count", get_keytable_count, NULL);
    static eabase::PassiveStatus<size_t> s_fiber_keytable_memory(
...
int fiber_keytable_pool_destroy(fiber_keytable_pool_t *pool) {
    ...
    eabase::KeyTable *saved_free_keytables = NULL;
    {
        BAIDU_SCOPED_LOCK(pool->mutex);
        if (pool->free_keytables) {
            saved_free_keytables = (eabase::KeyTable *) pool->free_keytables;
            pool->free_keytables = NULL;
        }
        pool->destroyed = 1;
    }
    // Cheat get/setspecific and destroy the keytables.
    eabase::TaskGroup *const g = eabase::tls_task_group;
    eabase::KeyTable *old_kt = eabase::tls_bls.keytable;
    while (saved_free_keytables) {
        eabase::KeyTable *kt = saved_free_keytables;
        saved_free_keytables = kt->next;
        eabase::tls_bls.keytable = kt;
        if (g) {
    ...
    eabase::tls_bls.keytable = old_kt;
    if (g) {
...
int fiber_keytable_pool_getstat(fiber_keytable_pool_t *pool,
    ...
    std::unique_lock<pthread_mutex_t> mu(pool->mutex);
    size_t count = 0;
    eabase::KeyTable *p = (eabase::KeyTable *) pool->free_keytables;
...
void fiber_keytable_pool_reserve(fiber_keytable_pool_t *pool,
    ...
    for (size_t i = stat.nfree; i < nfree; ++i) {
        eabase::KeyTable *kt = new(std::nothrow) eabase::KeyTable;
        if (kt == NULL) {
            break;
        }
        ...
        kt->next = (eabase::KeyTable *) pool->free_keytables;
        pool->free_keytables = kt;
        if (data == NULL) {
            break;
        }
...
int fiber_key_create2(fiber_key_t *key,
    ...
    {
        BAIDU_SCOPED_LOCK(eabase::s_key_mutex);
        if (eabase::nfreekey > 0) {
            index = eabase::s_free_keys[--eabase::nfreekey];
        } else if (eabase::nkey < eabase::KEYS_MAX) {
            index = eabase::nkey++;
        } else {
            return EAGAIN;  // what pthread_key_create returns in this case.
        }
    }
    eabase::s_key_info[index].dtor = dtor;
    eabase::s_key_info[index].dtor_args = dtor_args;
    key->index = index;
    key->version = eabase::s_key_info[index].version;
    if (key->version == 0) {
        ++eabase::s_key_info[index].version;
        ++key->version;
    }
    return 0;
}
...
int fiber_key_create(fiber_key_t *key, void (*dtor)(void *)) {
    if (dtor == NULL) {
        ...
        return fiber_key_create2(key, eabase::arg_as_dtor, (const void *) dtor);
    }
}

int fiber_key_delete(fiber_key_t key) {
    if (key.index < eabase::KEYS_MAX &&
        key.version == eabase::s_key_info[key.index].version) {
        BAIDU_SCOPED_LOCK(eabase::s_key_mutex);
        if (key.version == eabase::s_key_info[key.index].version) {
            if (++eabase::s_key_info[key.index].version == 0) {
                ++eabase::s_key_info[key.index].version;
            }
            eabase::s_key_info[key.index].dtor = NULL;
            eabase::s_key_info[key.index].dtor_args = NULL;
            eabase::s_free_keys[eabase::nfreekey++] = key.index;
            return 0;
        }
    }
...
int fiber_setspecific(fiber_key_t key, void *data) {
    eabase::KeyTable *kt = eabase::tls_bls.keytable;
    if (NULL == kt) {
        kt = new(std::nothrow) eabase::KeyTable;
        if (NULL == kt) {
            return ENOMEM;
        }
        eabase::tls_bls.keytable = kt;
        ...
        if (g) {
            ...
            if (!eabase::tls_ever_created_keytable) {
                eabase::tls_ever_created_keytable = true;
                CHECK_EQ(0, eabase::thread_atexit(eabase::cleanup_pthread, kt));
            }
        }
    }
...
void *fiber_getspecific(fiber_key_t key) {
    ...
    eabase::TaskGroup *const g = eabase::tls_task_group;
    if (g) {
        eabase::TaskMeta *const task = g->current_task();
        kt = eabase::borrow_keytable(task->attr.keytable_pool);
        if (kt) {
            g->current_task()->local_storage.keytable = kt;
            eabase::tls_bls.keytable = kt;
            return kt->get_data(key);
...
void fiber_assign_data(void *data) {
    eabase::tls_bls.assigned_data = data;
}

void *fiber_get_assigned_data() {
    return eabase::tls_bls.assigned_data;
}
