...
#include "eabase/var/collector.h"
...
#include "eabase/utility/files/file.h"
#include "eabase/utility/files/file_path.h"
#include "eabase/utility/file_util.h"
#include "eabase/utility/unique_ptr.h"
#include "eabase/utility/third_party/murmurhash3/murmurhash3.h"
#include "eabase/utility/logging.h"
#include "eabase/utility/object_pool.h"
#include "eabase/fiber/butex.h"                       // butex_*
#include "eabase/fiber/processor.h"                   // cpu_relax, barrier
#include "eabase/fiber/mutex.h"                       // fiber_mutex_t
#include "eabase/fiber/sys_futex.h"
#include "eabase/fiber/log.h"
...
static eabase::CollectorSpeedLimit g_cp_sl = VAR_COLLECTOR_SPEED_LIMIT_INITIALIZER;
...
struct SampledContention : public eabase::Collected {
    ...
    // Implement eabase::Collected
    void dump_and_destroy(size_t round) override;
    void destroy() override;
    eabase::CollectorSpeedLimit* speed_limit() override { return &g_cp_sl; }
    ...
    size_t hash_code() const {
        ...
        if (_hash_code == 0) {
            ...
            eabase::MurmurHash3_x86_32(stack, sizeof(void*) * nframes, seed, &_hash_code);
        }
        return _hash_code;
    }
private:
friend eabase::ObjectPool<SampledContention>;
    SampledContention()
    ...
    static_assert(sizeof(SampledContention) == 256, "be friendly toallocator");
...
public:
    typedef eabase::FlatMap<SampledContention*, SampledContention*,
...
private:
    ...
    eabase::IOBuf _disk_buf;  // temp buf before saving the file.
...
void ContentionProfiler::flush_to_disk(bool ending) {
    ...
    if (ending) {
        ...
        eabase::IOPortal mem_maps;
        const eabase::fd_guard fd(open("/proc/self/maps", O_RDONLY));
        if (fd >= 0) {
            while (true) {
    ...
    eabase::File::Error error;
    eabase::FilePath path(_filename);
    eabase::FilePath dir = path.DirName();
    if (!eabase::CreateDirectoryAndGetError(dir, &error)) {
    ...
    eabase::fd_guard fd(open(_filename.c_str(), O_WRONLY|O_CREAT|flag, 0666));
    if (fd < 0) {
...
EA_CACHELINE_ALIGNMENT static ContentionProfiler* g_cp = NULL;
...
struct EA_CACHELINE_ALIGNMENT MutexMapEntry {
    eabase::static_atomic<uint64_t> versioned_mutex;
...
void SampledContention::destroy() {
    _hash_code = 0;
    eabase::return_object(this);
}

// Remember the conflict hashes for troubleshooting, should be 0 at most of time.
static eabase::static_atomic<int64_t> g_nconflicthash = BUTIL_STATIC_ATOMIC_INIT(0);
static int64_t get_nconflicthash(void*) {
    return g_nconflicthash.load(eabase::memory_order_relaxed);
}
...
bool ContentionProfilerStart(const char* filename) {
    ...
    static eabase::PassiveStatus<int64_t> g_nconflicthash_var
        ("contention_profiler_conflict_hash", get_nconflicthash, NULL);
    static eabase::DisplaySamplingRatio g_sampling_ratio_var(
...
//   #1  0x00000000004b4df7 in eabase::internal::SpinLockDelay(int volatile*, int, int) ()
...
//   #9  0x0000000000666517 in eabase::init_sys_mutex_lock () at eabase/fiber/mutex.cpp:358
//   #10 0x00007effddc97a90 in pthread_once () at ../nptl/sysdeps/unix/sysv/linux/x86_64/pthread_once.S:103
//   #11 0x000000000066649f in eabase::first_sys_pthread_mutex_lock (mutex=0xbaf880 <_ULx86_64_lock>) at eabase/fiber/mutex.cpp:366
//   #12 0x00000000006678bc in pthread_mutex_lock_impl (mutex=0xbaf880 <_ULx86_64_lock>) at eabase/fiber/mutex.cpp:489
//   #13 pthread_mutex_lock (__mutex=__mutex@entry=0xbaf880 <_ULx86_64_lock>) at eabase/fiber/mutex.cpp:751
...
inline uint64_t hash_mutex_ptr(const pthread_mutex_t* m) {
    return eabase::fmix64((uint64_t)m);
}
...
add_pthread_contention_site(pthread_mutex_t* mutex) {
    ...
    eabase::static_atomic<uint64_t>& m = entry.versioned_mutex;
    uint64_t expected = m.load(eabase::memory_order_relaxed);
    ...
    if (expected == 0 ||
        ...
        if (m.compare_exchange_strong(
                expected, desired, eabase::memory_order_acquire)) {
            return &entry.csite;
        }
    }
    g_nconflicthash.fetch_add(1, eabase::memory_order_relaxed);
    return NULL;
}
...
inline bool remove_pthread_contention_site(
    ...
    MutexMapEntry& entry = g_mutex_map[hash_mutex_ptr(mutex) & (MUTEX_MAP_SIZE - 1)];
    eabase::static_atomic<uint64_t>& m = entry.versioned_mutex;
    if ((m.load(eabase::memory_order_relaxed) & ((((uint64_t)1) << PTR_BITS) - 1))
        != (uint64_t)mutex) {
    ...
    *saved_csite = entry.csite;
    ...
    m.store(0, eabase::memory_order_release);
    return true;
}
...
void submit_contention(const fiber_contention_site_t& csite, int64_t now_ns) {
    ...
    auto sc = eabase::get_object<SampledContention>();
    ...
    // collecting thread.
    sc->duration_ns = csite.duration_ns * eabase::COLLECTOR_SAMPLING_BASE
        / csite.sampling_range;
    sc->count = eabase::COLLECTOR_SAMPLING_BASE / (double)csite.sampling_range;
...
BUTIL_FORCE_INLINE int pthread_mutex_lock_impl(pthread_mutex_t* mutex) {
    ...
    // Ask eabase::Collector if this (contended) locking should be sampled
    const size_t sampling_range = eabase::is_collectable(&g_cp_sl);
    ...
    const int64_t start_ns = eabase::cpuwide_time_ns();
    ...
    if (!rc) { // Inside lock
        ...
        csite->duration_ns = eabase::cpuwide_time_ns() - start_ns;
...
BUTIL_FORCE_INLINE int pthread_mutex_unlock_impl(pthread_mutex_t* mutex) {
    ...
    for (int i = fast_alt.count - 1; i >= 0; --i) {
        if (fast_alt.list[i].mutex == mutex) {
            if (is_contention_site_valid(fast_alt.list[i].csite)) {
                ...
                unlock_start_ns = eabase::cpuwide_time_ns();
            }
    ...
    // [Outside lock]
    if (unlock_start_ns) {
        const int64_t unlock_end_ns = eabase::cpuwide_time_ns();
...
struct MutexInternal {
    eabase::static_atomic<unsigned char> locked;
    eabase::static_atomic<unsigned char> contended;
...
#define FIBER_MUTEX_CONTENDED (*(const unsigned*)&eabase::MUTEX_CONTENDED_RAW)
#define FIBER_MUTEX_LOCKED (*(const unsigned*)&eabase::MUTEX_LOCKED_RAW)
...
inline int mutex_lock_contended(fiber_mutex_t* m) {
    eabase::atomic<unsigned>* whole = (eabase::atomic<unsigned>*)m->butex;
    while (whole->exchange(FIBER_MUTEX_CONTENDED) & FIBER_MUTEX_LOCKED) {
        if (eabase::butex_wait(whole, FIBER_MUTEX_CONTENDED, NULL) < 0 &&
...
inline int mutex_timedlock_contended(
    ...
    eabase::atomic<unsigned>* whole = (eabase::atomic<unsigned>*)m->butex;
    while (whole->exchange(FIBER_MUTEX_CONTENDED) & FIBER_MUTEX_LOCKED) {
        if (eabase::butex_wait(whole, FIBER_MUTEX_CONTENDED, abstime) < 0 &&
            errno != EWOULDBLOCK && errno != EINTR/*note*/) {
...
int FastPthreadMutex::lock_contended() {
    eabase::atomic<unsigned>* whole = (eabase::atomic<unsigned>*)&_futex;
    while (whole->exchange(FIBER_MUTEX_CONTENDED) & FIBER_MUTEX_LOCKED) {
...
void FastPthreadMutex::lock() {
    eabase::MutexInternal* split = (eabase::MutexInternal*)&_futex;
    if (split->locked.exchange(1, eabase::memory_order_acquire)) {
        (void)lock_contended();
    }
}
...
bool FastPthreadMutex::try_lock() {
    eabase::MutexInternal* split = (eabase::MutexInternal*)&_futex;
    return !split->locked.exchange(1, eabase::memory_order_acquire);
}

void FastPthreadMutex::unlock() {
    eabase::atomic<unsigned>* whole = (eabase::atomic<unsigned>*)&_futex;
    const unsigned prev = whole->exchange(0, eabase::memory_order_release);
...
int fiber_mutex_init(fiber_mutex_t* __restrict m,
    ...
    eabase::make_contention_site_invalid(&m->csite);
    m->butex = eabase::butex_create_checked<unsigned>();
    if (!m->butex) {
        return ENOMEM;
    }
    *m->butex = 0;
...
int fiber_mutex_destroy(fiber_mutex_t* m) {
    eabase::butex_destroy(m->butex);
    return 0;
}

int fiber_mutex_trylock(fiber_mutex_t* m) {
    eabase::MutexInternal* split = (eabase::MutexInternal*)m->butex;
    if (!split->locked.exchange(1, eabase::memory_order_acquire)) {
        return 0;
    }
    return EBUSY;
...
int fiber_mutex_lock_contended(fiber_mutex_t* m) {
    return eabase::mutex_lock_contended(m);
}
...
int fiber_mutex_lock(fiber_mutex_t* m) {
    ...
    const size_t sampling_range = eabase::is_collectable(&eabase::g_cp_sl);
    if (!sampling_range) { // Don't sample
        return eabase::mutex_lock_contended(m);
    }
    // Start sampling.
    ...
    const int rc = eabase::mutex_lock_contended(m);
    if (!rc) { // Inside lock
        m->csite.duration_ns = eabase::cpuwide_time_ns() - start_ns;
...
int fiber_mutex_timedlock(fiber_mutex_t* __restrict m,
    ...
    // Don't sample when contention profiler is off.
    if (!eabase::g_cp) {
        return eabase::mutex_timedlock_contended(m, abstime);
    }
    // Ask Collector if this (contended) locking should be sampled.
    const size_t sampling_range = eabase::is_collectable(&eabase::g_cp_sl);
    if (!sampling_range) { // Don't sample
    ...
    // NOTE: Don't modify m->csite outside lock since multiple threads are
    // still contending with each other.
    const int rc = eabase::mutex_timedlock_contended(m, abstime);
    if (!rc) { // Inside lock
        m->csite.duration_ns = eabase::cpuwide_time_ns() - start_ns;
        m->csite.sampling_range = sampling_range;
    } else if (rc == ETIMEDOUT) {
        // Failed to lock due to ETIMEDOUT, submit the elapse directly.
        const int64_t end_ns = eabase::cpuwide_time_ns();
        const fiber_contention_site_t csite = {end_ns - start_ns, sampling_range};
        eabase::submit_contention(csite, end_ns);
    }
    return rc;
}
...
int fiber_mutex_unlock(fiber_mutex_t* m) {
    eabase::atomic<unsigned>* whole = (eabase::atomic<unsigned>*)m->butex;
    fiber_contention_site_t saved_csite = {0, 0};
    if (eabase::is_contention_site_valid(m->csite)) {
        saved_csite = m->csite;
        eabase::make_contention_site_invalid(&m->csite);
    }
    const unsigned prev = whole->exchange(0, eabase::memory_order_release);
    ...
    // Wakeup one waiter
    if (!eabase::is_contention_site_valid(saved_csite)) {
        eabase::butex_wake(whole);
        return 0;
    }
    const int64_t unlock_start_ns = eabase::cpuwide_time_ns();
    eabase::butex_wake(whole);
    const int64_t unlock_end_ns = eabase::cpuwide_time_ns();
    saved_csite.duration_ns += unlock_end_ns - unlock_start_ns;
    eabase::submit_contention(saved_csite, unlock_end_ns);
    return 0;
}

int pthread_mutex_lock (pthread_mutex_t *__mutex) {
    return eabase::pthread_mutex_lock_impl(__mutex);
}
int pthread_mutex_unlock (pthread_mutex_t *__mutex) {
    return eabase::pthread_mutex_unlock_impl(__mutex);
}
