...
#ifndef FIBER_PARKING_LOT_H_
#define FIBER_PARKING_LOT_H_
...
// Park idle workers.
class EA_CACHELINE_ALIGNMENT ParkingLot {
public:
    class State {
    public:
        State(): val(0) {}
    ...
    int signal(int num_task) {
        _pending_signal.fetch_add((num_task << 1), eabase::memory_order_release);
        return futex_wake_private(&_pending_signal, num_task);
    }

    // Get a state for later wait().
    State get_state() {
        return _pending_signal.load(eabase::memory_order_acquire);
    }
...
private:
    ...
    eabase::atomic<int> _pending_signal;
};

}  // namespace eabase

#endif  // FIBER_PARKING_LOT_H_
