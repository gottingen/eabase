...
private:
    EA_DISALLOW_COPY_AND_ASSIGN(SmallQueue);
...
struct PendingError {
    ...
    PendingError() : id(INVALID_FIBER_SESSION), error_code(0), location(NULL) {}
};

struct EA_CACHELINE_ALIGNMENT Id {
    ...
    Id() {
        ...
        butex = eabase::butex_create_checked<uint32_t>();
        join_butex = eabase::butex_create_checked<uint32_t>();
        *butex = 0;
        *join_butex = 0;
    }
    ...
    ~Id() {
        ...
        eabase::butex_destroy(join_butex);
    }
...
typedef eabase::ResourceId<Id> IdResourceId;
...
inline bool id_exists_with_true_negatives(fiber_session_t id) {
    ...
    const uint32_t id_ver = eabase::get_version(id);
...
void id_status(fiber_session_t id, std::ostream &os) {
    eabase::Id* const meta = address_resource(eabase::get_slot(id));
    if (!meta) {
    ...
    if (valid) {
        os << "First id: "
           << eabase::make_id(first_ver, eabase::get_slot(id)).value << '\n'
...
void id_pool_status(std::ostream &os) {
    os << eabase::describe_resources<Id>() << '\n';
}
...
struct IdTraits {
    ...
    { return eabase::id_exists_with_true_negatives(id); }
};
const fiber_session_t IdTraits::ID_INIT = INVALID_FIBER_SESSION;
...
size_t get_sizes(const fiber_session_list_t* list, size_t* cnt, size_t n) {
    ...
    return static_cast<eabase::IdList*>(list->impl)->get_sizes(cnt, n);
}
...
int fiber_session_create(
    fiber_session_t* id, void* data,
    int (*on_error)(fiber_session_t, void*, int)) {
    return eabase::id_create_impl(
        id, data,
        (on_error ? on_error : eabase::default_fiber_session_on_error), NULL);
}

int fiber_session_create_ranged(fiber_session_t* id, void* data,
                             int (*on_error)(fiber_session_t, void*, int),
                             int range) {
    return eabase::id_create_ranged_impl(
        id, data, 
        (on_error ? on_error : eabase::default_fiber_session_on_error),
        NULL, range);
}
        ...
        if (*butex == meta->first_ver) {
            ...
            if (range == 0) {
                // fast path
                       ...
                       range > eabase::ID_MAX_RANGE ||
                       range + meta->first_ver <= meta->locked_ver) {
                LOG_IF(FATAL, range < 0) << "range must be positive, actually "
                                         << range;
                LOG_IF(FATAL, range > eabase::ID_MAX_RANGE)
                    << "max range is " << eabase::ID_MAX_RANGE
                    << ", actually " << range;
            } else {
            ...
            *butex = meta->contended_ver();
            ...
            meta->mutex.unlock();
            ever_contended = true;
            if (eabase::butex_wait(butex, expected_ver, NULL) < 0 &&
...
int fiber_session_about_to_destroy(fiber_session_t id) {
    ...
    *butex = meta->unlockable_ver();
    ...
    if (contended) {
        ...
        eabase::butex_wake_except(butex, 0);
    }
    return 0;
}
...
int fiber_session_cancel(fiber_session_t id) {
    ...
    *butex = meta->end_ver();
    ...
    meta->mutex.unlock();
    return_resource(eabase::get_slot(id));
    return 0;
}

int fiber_session_join(fiber_session_t id) {
    const eabase::IdResourceId slot = eabase::get_slot(id);
    eabase::Id* const meta = address_resource(slot);
    if (!meta) {
    ...
    while (1) {
        ...
        if (eabase::butex_wait(join_butex, expected_ver, NULL) < 0 &&
            errno != EWOULDBLOCK && errno != EINTR) {
            return errno;
        }
    }
...
int fiber_session_unlock_and_destroy(fiber_session_t id) {
    ...
    *join_butex = next_ver;
    ...
    eabase::butex_wake_except(butex, 0);
    eabase::butex_wake_all(join_butex);
    return_resource(eabase::get_slot(id));
    return 0;
}
...
void fiber_session_list_destroy(fiber_session_list_t* list) {
    delete static_cast<eabase::IdList*>(list->impl);
    list->impl = NULL;
}

int fiber_session_list_add(fiber_session_list_t* list, fiber_session_t id) {
    if (list->impl == NULL) {
        list->impl = new (std::nothrow) eabase::IdList;
        if (NULL == list->impl) {
            return ENOMEM;
        }
    }
    return static_cast<eabase::IdList*>(list->impl)->add(id);
}
...
int fiber_session_create2(
    ...
    return eabase::id_create_impl(
        id, data, NULL,
        (on_error ? on_error : eabase::default_fiber_session_on_error2));
}
...
int fiber_session_create2_ranged(
    ...
    return eabase::id_create_ranged_impl(
        id, data, NULL,
        (on_error ? on_error : eabase::default_fiber_session_on_error2), range);
}
...
int fiber_session_list_reset2(fiber_session_list_t* list,
                           ...
                           const std::string& error_text) {
    if (list->impl != NULL) {
        static_cast<eabase::IdList*>(list->impl)->apply(
            eabase::IdResetter(error_code, error_text));
    }
    return 0;
}
