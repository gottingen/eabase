...
#include "eabase/utility/compat.h"                   // OS_MACOSX
#include "eabase/utility/macros.h"                   // ARRAY_SIZE
...
#include "eabase/fiber/errno.h"                  // ESTOP
...
// defined in eabase/fiber/key.cpp
...
#include "eabase/fiber/offset_inl.list"
};
...
void TaskGroup::run_main_task() {
    eabase::PassiveStatus<double> cumulated_cputime(
        get_cumulated_cputime_from_this, this);
    std::unique_ptr<eabase::PerSecond<eabase::PassiveStatus<double> > > usage_var;
...
#endif
            usage_var.reset(new eabase::PerSecond<eabase::PassiveStatus<double> >
                             (name, &cumulated_cputime, 1));
        }
    }
    // Don't forget to add elapse of last wait_task.
    current_task()->stat.cputime_ns += eabase::cpuwide_time_ns() - _last_run_ns;
}
...
TaskGroup::TaskGroup(TaskControl* c)
    ...
    , _num_nosignal(0)
    , _nsignaled(0)
    , _last_run_ns(eabase::cpuwide_time_ns())
...
int TaskGroup::init(size_t runqueue_capacity) {
    ...
    eabase::ResourceId<TaskMeta> slot;
    TaskMeta* m = eabase::get_resource<TaskMeta>(&slot);
    if (NULL == m) {
    ...
    m->cpuwide_start_ns = eabase::cpuwide_time_ns();
    m->stat = EMPTY_STAT;
...
void TaskGroup::task_runner(intptr_t skip_remained) {
    ...
    do {
        ...
        if (FLAGS_show_fiber_creation_in_vars) {
            ...
            // considerable time because a single eabase::LatencyRecorder
            // contains many var.
            g->_control->exposed_pending_time() <<
                (eabase::cpuwide_time_ns() - m->cpuwide_start_ns) / 1000L;
        }
...
int TaskGroup::start_foreground(TaskGroup** pg,
    ...
    eabase::ResourceId<TaskMeta> slot;
    TaskMeta* m = eabase::get_resource(&slot);
    if (__builtin_expect(!m, 0)) {
        return ENOMEM;
    }
    CHECK(m->current_waiter.load(eabase::memory_order_relaxed) == NULL);
    m->stop = false;
    m->interrupted = false;
    m->about_to_quit = false;
    m->fn = fn;
...
void TaskGroup::sched_to(TaskGroup** pg, TaskMeta* next_meta) {
    ...
    TaskMeta* const cur_meta = g->_cur_meta;
    const int64_t now = eabase::cpuwide_time_ns();
...
void TaskGroup::flush_nosignal_tasks_remote_locked(eabase::Mutex& locked_mutex) {
...
void TaskGroup::_add_sleep_event(void* void_args) {
    ...
    sleep_id = get_global_timer_thread()->schedule(
        ...
        eabase::microseconds_from_now(e.timeout_us));
...
static int interrupt_and_consume_waiters(
    ...
    BAIDU_SCOPED_LOCK(m->version_lock);
    if (given_ver == *m->version_butex) {
        *pw = m->current_waiter.exchange(NULL, eabase::memory_order_acquire);
...
static int set_butex_waiter(fiber_t tid, ButexWaiter* w) {
    ...
    if (m != NULL) {
        ...
        if (given_ver == *m->version_butex) {
            ...
            m->current_waiter.store(w, eabase::memory_order_release);
            return 0;
        }
    }
    return EINVAL;
...
void print_task(std::ostream& os, fiber_t tid) {
    ...
    if (!matched) {
        ...
        os << "fiber=" << tid << " :\nstop=" << stop
           ...
           << "}\nhas_tls=" << has_tls
           << "\nuptime_ns=" << eabase::cpuwide_time_ns() - cpuwide_start_ns
