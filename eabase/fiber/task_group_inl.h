...
#ifndef FIBER_TASK_GROUP_INL_H_
#define FIBER_TASK_GROUP_INL_H_

namespace eabase {

// Utilities to manipulate fiber_t
inline fiber_t make_tid(uint32_t version, eabase::ResourceId<TaskMeta> slot) {
    return (((fiber_t)version) << 32) | (fiber_t)slot.value;
}

inline eabase::ResourceId<TaskMeta> get_slot(fiber_t tid) {
    eabase::ResourceId<TaskMeta> id = { (tid & 0xFFFFFFFFul) };
    return id;
}
