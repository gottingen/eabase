...
#include "eabase/utility/third_party/murmurhash3/murmurhash3.h"   // fmix64
#include "eabase/utility/resource_pool.h"
...
struct EA_CACHELINE_ALIGNMENT TimerThread::Task {
    ...
    //                      this struct)
    eabase::atomic<uint32_t> version;
...
class EA_CACHELINE_ALIGNMENT TimerThread::Bucket {
public:
    Bucket()
...
inline TimerThread::TaskId make_task_id(
    eabase::ResourceId<TimerThread::Task> slot, uint32_t version) {
    return TimerThread::TaskId((((uint64_t)version) << 32) | slot.value);
}

inline
eabase::ResourceId<TimerThread::Task> slot_of_task_id(TimerThread::TaskId id) {
    eabase::ResourceId<TimerThread::Task> slot = { (id & 0xFFFFFFFFul) };
    return slot;
}
...
void* TimerThread::run_this(void* arg) {
    eabase::PlatformThread::SetName("ea_timer");
...
TimerThread::Bucket::schedule(void (*fn)(void*), void* arg,
                              const timespec& abstime) {
    eabase::ResourceId<Task> slot_id;
    Task* task = eabase::get_resource<Task>(&slot_id);
    if (task == NULL) {
    ...
    task->fn = fn;
    task->arg = arg;
    task->run_time = eabase::timespec_to_microseconds(abstime);
    uint32_t version = task->version.load(eabase::memory_order_relaxed);
    if (version == 0) {  // skip 0.
        task->version.fetch_add(2, eabase::memory_order_relaxed);
        version = 2;
    }
...
TimerThread::TaskId TimerThread::schedule(
    ...
    if (_stop.load(eabase::memory_order_relaxed) || !_started) {
    ...
    const Bucket::ScheduleResult result = 
        _buckets[eabase::fmix64(pthread_numeric_id()) % _options.num_buckets]
        .schedule(fn, arg, abstime);
    if (result.earlier) {
        bool earlier = false;
        const int64_t run_time = eabase::timespec_to_microseconds(abstime);
        {
            BAIDU_SCOPED_LOCK(_mutex);
...
int TimerThread::unschedule(TaskId task_id) {
    const eabase::ResourceId<Task> slot_id = slot_of_task_id(task_id);
    Task* const task = eabase::address_resource(slot_id);
    if (task == NULL) {
...
bool TimerThread::Task::run_and_delete() {
    ...
    if (version.compare_exchange_strong(
            expected_version, id_version + 1, eabase::memory_order_relaxed)) {
        fn(arg);
        // The release fence is paired with acquire fence in
        // TimerThread::unschedule to make changes of fn(arg) visible.
        version.store(id_version + 2, eabase::memory_order_release);
        eabase::return_resource(slot_of_task_id(task_id));
        return true;
...
bool TimerThread::Task::try_delete() {
    const uint32_t id_version = version_of_task_id(task_id);
    if (version.load(eabase::memory_order_relaxed) != id_version) {
        CHECK_EQ(version.load(eabase::memory_order_relaxed), id_version + 2);
        eabase::return_resource(slot_of_task_id(task_id));
        return true;
    }
    return false;
...
void TimerThread::run() {
    ...
    int64_t last_sleep_time = eabase::gettimeofday_us();
    ...
    // vars
    size_t nscheduled = 0;
    eabase::PassiveStatus<size_t> nscheduled_var(deref_value<size_t>, &nscheduled);
    eabase::PerSecond<eabase::PassiveStatus<size_t> > nscheduled_second(&nscheduled_var);
    size_t ntriggered = 0;
    eabase::PassiveStatus<size_t> ntriggered_var(deref_value<size_t>, &ntriggered);
    eabase::PerSecond<eabase::PassiveStatus<size_t> > ntriggered_second(&ntriggered_var);
    double busy_seconds = 0;
    eabase::PassiveStatus<double> busy_seconds_var(deref_value<double>, &busy_seconds);
    eabase::PerSecond<eabase::PassiveStatus<double> > busy_seconds_second(&busy_seconds_var);
    if (!_options.var_prefix.empty()) {
        nscheduled_second.expose_as(_options.var_prefix, "scheduled_second");
        ntriggered_second.expose_as(_options.var_prefix, "triggered_second");
    ...
    while (!_stop.load(eabase::memory_order_relaxed)) {
        ...
        while (!tasks.empty()) {
            ...
            if (eabase::gettimeofday_us() < task1->run_time) {  // not ready yet.
                break;
            }
