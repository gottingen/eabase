...
#include "eabase/rapidjson/robust_json.h"
#include <fstream>
#include "eabase/strings/numbers.h"
#include "eabase/rapidjson/ostreamwrapper.h"
#include "eabase/rapidjson/writer.h"
#include "eabase/files/filesystem.h"
#include "eabase/files/sequential_read_file.h"
...
RAPIDJSON_NAMESPACE_BEGIN
    ...
    flare::result_status robust_parse(const std::string &file_path, rapidjson::Document *doc) {
        std::error_code ec;
        auto r = flare::exists(file_path, ec);
        if (ec) {
            return flare::result_status::from_error_code(ec);
        }
        if(!r) {
            return flare::result_status(-1, "not exists file");
        }
        ...
        if (!static_cast<rapidjson::ParseResult>(doc->Parse(content.c_str()))) {
            return flare::result_status(-2, "json parse error");
        }
    ...
    flare::result_status robust_write(const rapidjson::Document &d, const std::string &file_path) {
        ...
        if (!ofs) {
            return flare::result_status(-2, "open file error");
        }
        rapidjson::OStreamWrapper osw(ofs);
        rapidjson::Writer<rapidjson::OStreamWrapper> writer(osw);
        if (!d.Accept(writer)) {
            return  flare::result_status(-2, "write file error");
        }
        return flare::result_status::success();
    }
    ...
    std::optional<uint64_t> as_uint64(OptionalRefRapidJsonValue v_) {
        if (v_.has_value()) {
            ...
            if (v.IsString()) {
                uint64_t n;
                if (!flare::simple_atoi(std::string_view{v.GetString(), v.GetStringLength()}, &n)) {
                    return std::nullopt;
                }
                return n;
            }
    ...
    std::optional<double> as_double(OptionalRefRapidJsonValue v_) {
        if (v_.has_value()) {
            ...
            if (v.IsString()) {
                double n;
                if (!flare::simple_atod(std::string_view{v.GetString(), v.GetStringLength()}, &n)) {
                    return std::nullopt;
                }
                return n;
            }
