...
int ComlogSink::SetupFromConfig(const std::string& conf_path_str) {
    Unload();
    eabase::FilePath path(conf_path_str);
...
int ComlogSink::Setup(const ComlogSinkOptions* options) {
    ...
    if (!_options.log_dir.empty()) {
        eabase::FilePath log_dir(_options.log_dir);
        if (log_dir.IsAbsolute()) {
            cwd = log_dir;
        } else {
            if (!eabase::GetCurrentDirectory(&cwd)) {
    ...
    eabase::File::Error err;
    if (!eabase::CreateDirectoryAndGetError(cwd, &err)) {
