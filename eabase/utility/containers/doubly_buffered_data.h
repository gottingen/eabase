...
public:
    ...
    private:
        EA_DISALLOW_COPY_AND_ASSIGN(ScopedPtr);
        const T* _data;
...
private:
    ...
    const T* UnsafeRead() const {
        return _data + _index.load(eabase::memory_order_acquire);
    }

    const T* UnsafeRead(int& index) const {
        index = _index.load(eabase::memory_order_acquire);
        return _data + index;
    }
...
public:
    ...
    struct EA_CACHELINE_ALIGNMENT ThreadBlock {
    ...
    inline static DoublyBufferedData::Wrapper* get_or_create_tls_data(WrapperTLSId id) {
        ...
        if (_s_tls_blocks == NULL) {
            ...
            eabase::thread_atexit(_destroy_tls_blocks);
        }
...
size_t DoublyBufferedData<T, TLS, AllowFiberSuspended>::Modify(Fn& fn) {
    ...
    int bg_index = !_index.load(eabase::memory_order_relaxed);
    ...
    _index.store(bg_index, eabase::memory_order_release);
    bg_index = !bg_index;
    ...
    CHECK_EQ(ret2, ret) << "index=" << _index.load(eabase::memory_order_relaxed);
    return ret2;
}
