...
//
// Q. Should I use std::list or eabase::LinkedList?
//
// A. The main reason to use eabase::LinkedList over std::list is
//    performance. If you don't care about the performance differences
//    then use an STL container, as it makes for better code readability.
//
//    Comparing the performance of eabase::LinkedList<T> to std::list<T*>:
//
//    * Erasing an element of type T* from eabase::LinkedList<T> is
//      an O(1) operation. Whereas for std::list<T*> it is O(n).
//      That is because with std::list<T*> you must obtain an
//      iterator to the T* element before you can call erase(iterator).
//
//    * Insertion operations with eabase::LinkedList<T> never require
//      heap allocations.
//
// Q. How does eabase::LinkedList implementation differ from std::list?
//
// A. Doubly-linked lists are made up of nodes that contain "next" and
//    "previous" pointers that reference other nodes in the list.
//
//    With eabase::LinkedList<T>, the type being inserted already reserves
//    space for the "next" and "previous" pointers (eabase::LinkNode<T>*).
...
class LinkNode {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(LinkNode);
};
...
class LinkedList {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(LinkedList);
};
