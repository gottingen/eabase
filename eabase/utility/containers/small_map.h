...
#include "eabase/utility/memory/manual_constructor.h"
...
//
//  - eabase::hash_map should be used if you need O(1) lookups. It may waste
...
//
//   eabase::SmallMap<std::map<MyKey, MyValue>, 4, std::equal_to<KyKey> >
//
//
// USAGE
// -----
...
//
// example:
//   eabase::SmallMap< std::map<string, int> > days;
//   days["sunday"   ] = 0;
//   days["monday"   ] = 1;
//   days["tuesday"  ] = 2;
//   days["wednesday"] = 3;
...
//
// If we switch to using std::unordered_map for eabase::hash_map, then the
...
template <typename KeyType, typename ValueType>
struct select_equal_key< eabase::hash_map<KeyType, ValueType>, false> {
  struct equal_key {
    bool operator()(const KeyType& left, const KeyType& right) {
      return left == right;
    }
