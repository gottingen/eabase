...
#include "eabase/utility/format_macros.h"
...
typedef std::map<eabase::StringPiece, CrashKey> CrashKeyMap;
...
void SetCrashKeyValue(const eabase::StringPiece& key,
  ...
  for (size_t i = chunks.size();
       ...
       ++i) {
    g_clear_key_func_(eabase::StringPrintf(kChunkFormatString, key.data(), i+1));
  }

  // Set the chunked keys.
  for (size_t i = 0; i < chunks.size(); ++i) {
    g_set_key_func_(eabase::StringPrintf(kChunkFormatString, key.data(), i+1),
                    chunks[i]);
  }
}

void ClearCrashKey(const eabase::StringPiece& key) {
...
void SetCrashKeyToStackTrace(const eabase::StringPiece& key,
...
void SetCrashKeyFromAddresses(const eabase::StringPiece& key,
  ...
  if (addresses && count) {
    ...
    for (size_t i = 0; i < count; ++i) {
      std::string s = eabase::StringPrintf("%p", addresses[i]);
      length += s.length() + 1;
...
ScopedCrashKey::ScopedCrashKey(const eabase::StringPiece& key,
                               const eabase::StringPiece& value)
    : key_(key.as_string()) {
  SetCrashKeyValue(key, value);
}
...
const CrashKey* LookupCrashKey(const eabase::StringPiece& key) {
  if (!g_crash_keys_)
    return NULL;
