...
BUTIL_EXPORT void SetCrashKeyValue(const eabase::StringPiece& key,
                                  const eabase::StringPiece& value);
BUTIL_EXPORT void ClearCrashKey(const eabase::StringPiece& key);

// Records the given StackTrace into a crash key.
BUTIL_EXPORT void SetCrashKeyToStackTrace(const eabase::StringPiece& key,
...
BUTIL_EXPORT void SetCrashKeyFromAddresses(const eabase::StringPiece& key,
...
class BUTIL_EXPORT ScopedCrashKey {
 public:
  ScopedCrashKey(const eabase::StringPiece& key, const eabase::StringPiece& value);
  ~ScopedCrashKey();

 private:
  std::string key_;

  EA_DISALLOW_COPY_AND_ASSIGN(ScopedCrashKey);
};
...
BUTIL_EXPORT const CrashKey* LookupCrashKey(const eabase::StringPiece& key);

// In the platform crash reporting implementation, these functions set and
// clear the NUL-termianted key-value pairs.
typedef void (*SetCrashKeyValueFuncT)(const eabase::StringPiece&,
                                      const eabase::StringPiece&);
typedef void (*ClearCrashKeyValueFuncT)(const eabase::StringPiece&);
