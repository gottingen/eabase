...
//
// We have to keep eabase::EndPoint ABI compatible because it is used so widely, and the size of eabase::EndPoint is
...
// eabase::details. Those two headers will not be published.
...
public:
    ...
    static GlobalEndPointSet* instance() {
        return ::eabase::get_leaky_singleton<GlobalEndPointSet>();
    }
...
public:
    ...
    ExtendedEndPoint(void) {
        _ref_count.store(0, eabase::memory_order_relaxed);
...
public:
    ...
    static ExtendedEndPoint* address(const EndPoint& ep) {
        ...
        ::eabase::ResourceId<ExtendedEndPoint> id;
        id.value = ep.ip.s_addr;
        ExtendedEndPoint* eep = ::eabase::address_resource<ExtendedEndPoint>(id);
        CHECK(eep) << "fail to address ExtendedEndPoint from EndPoint";
        return eep;
    }

    // Check if an EndPoint has embedded ExtendedEndPoint
    static bool is_extended(const eabase::EndPoint& ep) {
...
private:
    ...
    static ExtendedEndPoint* new_extended_endpoint(sa_family_t family) {
        ::eabase::ResourceId<ExtendedEndPoint> id;
        ExtendedEndPoint* eep = ::eabase::get_resource(&id);
        if (eep) {
            int64_t old_ref = eep->_ref_count.load(eabase::memory_order_relaxed);
            ...
            eep->_ref_count.store(1, eabase::memory_order_relaxed);
            eep->_id = id;
    ...
    static ExtendedEndPoint* dedup(ExtendedEndPoint* eep) {
        ...
        if (first_eep != eep) {
            eep->_ref_count.store(0, eabase::memory_order_relaxed);
            eep->_u.sa.sa_family = AF_UNSPEC;
            ::eabase::return_resource(eep->_id);
        }
        return first_eep;
    }
...
public:
    ...
    void dec_ref(void) {
        int64_t old_ref = _ref_count.fetch_sub(1, eabase::memory_order_relaxed);
        CHECK(old_ref >= 1) << "ExtendedEndPoint has unexpected reference " << old_ref;
        if (old_ref == 1) {
            global_set()->erase(this);
            _u.sa.sa_family = AF_UNSPEC;
            ::eabase::return_resource(_id);
        }
    }

    void inc_ref(void) {
        int64_t old_ref = _ref_count.fetch_add(1, eabase::memory_order_relaxed);
        CHECK(old_ref >= 1) << "ExtendedEndPoint has unexpected reference " << old_ref;
    }
...
private:
    ...
    eabase::atomic<int64_t> _ref_count;
    eabase::ResourceId<ExtendedEndPoint> _id;
...
inline ExtendedEndPoint* GlobalEndPointSet::insert(ExtendedEndPoint* p) {
    ...
    auto it = _set.find(p);
    if (it != _set.end()) {
        if ((*it)->_ref_count.fetch_add(1, eabase::memory_order_relaxed) == 0) {
            // another thread is calling dec_ref(), do not reuse it
            (*it)->_ref_count.fetch_sub(1, eabase::memory_order_relaxed);
            _set.erase(it);
            _set.insert(p);
            return p;
        } else {
