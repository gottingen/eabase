...
#include "eabase/utility/containers/hash_tables.h"         // hashing functions
...
inline bool operator<(eabase::ip_t lhs, eabase::ip_t rhs) {
    return eabase::ip2int(lhs) < eabase::ip2int(rhs);
}
inline bool operator>(eabase::ip_t lhs, eabase::ip_t rhs) {
    return rhs < lhs;
}
...
inline std::ostream& operator<<(std::ostream& os, const eabase::IPStr& ip_str) {
    return os << ip_str.c_str();
}
inline std::ostream& operator<<(std::ostream& os, eabase::ip_t ip) {
    return os << eabase::ip2str(ip);
}
...
inline std::size_t hash_value(const eabase::EndPoint& ep) {
    return eabase::HashPair(eabase::ip2int(ep.ip), ep.port);
}

#elif defined(COMPILER_GCC)
template <>
struct hash<eabase::EndPoint> {
    std::size_t operator()(const eabase::EndPoint& ep) const {
        return eabase::HashPair(eabase::ip2int(ep.ip), ep.port);
    }
};
