...
const char* berror(int error_code) {
    ...
    if (error_code >= eabase::ERRNO_BEGIN && error_code < eabase::ERRNO_END) {
        const char* s = eabase::errno_desc[error_code - eabase::ERRNO_BEGIN];
        if (s) {
            return s;
        }
#if defined(OS_MACOSX)
        const int rc = strerror_r(error_code, eabase::tls_error_buf, eabase::ERROR_BUFSIZE);
        if (rc == 0 || rc == ERANGE/*bufsize is not long enough*/) {
            return eabase::tls_error_buf;
        }
#else
        s = strerror_r(error_code, eabase::tls_error_buf, eabase::ERROR_BUFSIZE);
        if (s) {
            return s;
        }
#endif
    }
    snprintf(eabase::tls_error_buf, eabase::ERROR_BUFSIZE,
