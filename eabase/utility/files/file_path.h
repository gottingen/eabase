...
// through eabase::i18n::WrapPathWithLTRFormatting() before displaying it in the
// RTL UI.
//
...
class BUTIL_EXPORT FilePath {
 public:
  ...
  // long "extensions" to logic like eabase::GetUniquePathNumber().
  ...
  FilePath InsertBeforeExtensionASCII(
      const eabase::StringPiece& suffix) const WARN_UNUSED_RESULT;
  ...
  FilePath AppendASCII(const eabase::StringPiece& component)
...
BUTIL_EXPORT extern void PrintTo(const eabase::FilePath& path, std::ostream* out);
...
template<>
struct hash<eabase::FilePath> {
  size_t operator()(const eabase::FilePath& f) const {
    return hash<eabase::FilePath::StringType>()(f.value());
  }
};

#elif defined(COMPILER_MSVC)

inline size_t hash_value(const eabase::FilePath& f) {
