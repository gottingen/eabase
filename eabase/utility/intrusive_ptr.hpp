...
// NOTE: sp_convertible is different from eabase::is_convertible
// (in eabase/utility/type_traits.h) that it converts pointers only. Using
// eabase::is_convertible results in ctor/dtor issues.
...
template<typename T>
struct hash<eabase::intrusive_ptr<T> > {
    std::size_t operator()(const eabase::intrusive_ptr<T>& p) const {
        return hash<T*>()(p.get());
    }
};
#elif defined(COMPILER_MSVC)
template<typename T>
inline size_t hash_value(const eabase::intrusive_ptr<T>& sp) {
