...
#include "eabase/utility/ssl_compat.h"               // BIO_fd_non_fatal_error
...
#include "eabase/utility/build_config.h"             // ARCH_CPU_X86_64
...
#include "eabase/utility/logging.h"                  // CHECK, LOG
#include "eabase/utility/fd_guard.h"                 // eabase::fd_guard
...
inline iov_function get_preadv_func() {
    ...
    return user_preadv;
#endif
    eabase::fd_guard fd(open("/dev/zero", O_RDONLY));
    if (fd < 0) {
...
inline iov_function get_pwritev_func() {
    eabase::fd_guard fd(open("/dev/null", O_WRONLY));
    if (fd < 0) {
...
eabase::static_atomic<size_t> g_nblock = BUTIL_STATIC_ATOMIC_INIT(0);
eabase::static_atomic<size_t> g_blockmem = BUTIL_STATIC_ATOMIC_INIT(0);
eabase::static_atomic<size_t> g_newbigview = BUTIL_STATIC_ATOMIC_INIT(0);

}  // namespace iobuf

size_t IOBuf::block_count() {
    return iobuf::g_nblock.load(eabase::memory_order_relaxed);
}

size_t IOBuf::block_memory() {
    return iobuf::g_blockmem.load(eabase::memory_order_relaxed);
}

size_t IOBuf::new_bigview_count() {
    return iobuf::g_newbigview.load(eabase::memory_order_relaxed);
}
...
struct IOBuf::Block {
    ...
    Block(char* data_in, uint32_t data_size)
        ...
        , u({NULL})
        , data(data_in) {
        iobuf::g_nblock.fetch_add(1, eabase::memory_order_relaxed);
    ...
    void inc_ref() {
        check_abi();
        nshared.fetch_add(1, eabase::memory_order_relaxed);
    }
        
    void dec_ref() {
        check_abi();
        if (nshared.fetch_sub(1, eabase::memory_order_release) == 1) {
            eabase::atomic_thread_fence(eabase::memory_order_acquire);
            if (!flags) {
                iobuf::g_nblock.fetch_sub(1, eabase::memory_order_relaxed);
    ...
    int ref_count() const {
        return nshared.load(eabase::memory_order_relaxed);
    }
...
static eabase::static_atomic<size_t> g_num_hit_tls_threshold = BUTIL_STATIC_ATOMIC_INIT(0);
...
IOBuf::Block* share_tls_block() {
    ...
    if (b) {
        ...
        eabase::thread_atexit(remove_tls_block_chain);
    }
    if (!new_block) {
...
inline void release_tls_block(IOBuf::Block* b) {
    ...
    if (b->full()) {
        ...
        b->dec_ref();
        g_num_hit_tls_threshold.fetch_add(1, eabase::memory_order_relaxed);
    } else {
...
void release_tls_block_chain(IOBuf::Block* b) {
    ...
    if (tls_data.num_blocks >= MAX_BLOCKS_PER_THREAD) {
        ...
        g_num_hit_tls_threshold.fetch_add(n, eabase::memory_order_relaxed);
        return;
    }
...
inline IOBuf::BlockRef* acquire_blockref_array(size_t cap) {
    iobuf::g_newbigview.fetch_add(1, eabase::memory_order_relaxed);
...
size_t IOBuf::block_count_hit_tls_threshold() {
    return iobuf::g_num_hit_tls_threshold.load(eabase::memory_order_relaxed);
}

    static_assert(sizeof(IOBuf::SmallView) == sizeof(IOBuf::BigView),
                  "sizeof small and big view should equal");

    static_assert(IOBuf::DEFAULT_BLOCK_SIZE / 4096 * 4096 == IOBuf::DEFAULT_BLOCK_SIZE,
                  "sizeof block should be multiply of 4096");
...
void IOBuf::append(const Movable& movable_other) {
    if (empty()) {
        ...
        eabase::IOBuf& other = movable_other.value();
        const size_t nref = other._ref_num();
        for (size_t i = 0; i < nref; ++i) {
...
bool IOBuf::equals(const eabase::StringPiece& s) const {
    if (size() != s.size()) {
        return false;
    }
    const size_t nref = _ref_num();
...
bool IOBuf::equals(const eabase::IOBuf& other) const {
...
IOBufCutter::IOBufCutter(eabase::IOBuf* buf)
    : _data(NULL)
    , _data_end(NULL)
    , _block(NULL)
    , _buf(buf) {
...
size_t IOBufCutter::cutn(eabase::IOBuf* out, size_t n) {
    if (n == 0) {
        return 0;
    }
...
IOBufAsSnappySink::IOBufAsSnappySink(eabase::IOBuf& buf)
...
char* IOBufAsSnappySink::GetAppendBuffer(size_t length, char* scratch) {
    // TODO: eabase::IOBuf supports dynamic sized blocks.
...
size_t IOBufBytesIterator::append_and_forward(eabase::IOBuf* buf, size_t n) {
    size_t nc = 0;
...
void* fast_memcpy(void *__restrict dest, const void *__restrict src, size_t n) {
    return eabase::iobuf::cp(dest, src, n);
}
