...
#include "eabase/utility/third_party/snappy/snappy-sinksource.h"
#include "eabase/utility/zero_copy_stream_as_streambuf.h"
#include "eabase/utility/macros.h"
#include "eabase/utility/reader_writer.h"
#include "eabase/utility/binary_printer.h"
...
public:
    ...
    // Explicitly declare this overload as error to avoid copy_to(eabase::IOBuf*)
    ...
    bool equals(const eabase::StringPiece&) const;
...
inline bool operator==(const eabase::IOBuf& b, const eabase::StringPiece& s)
{ return b.equals(s); }
inline bool operator==(const eabase::StringPiece& s, const eabase::IOBuf& b)
{ return b.equals(s); }
inline bool operator!=(const eabase::IOBuf& b, const eabase::StringPiece& s)
{ return !b.equals(s); }
inline bool operator!=(const eabase::StringPiece& s, const eabase::IOBuf& b)
{ return !b.equals(s); }
inline bool operator==(const eabase::IOBuf& b1, const eabase::IOBuf& b2)
{ return b1.equals(b2); }
inline bool operator!=(const eabase::IOBuf& b1, const eabase::IOBuf& b2)
{ return !b1.equals(b2); }
...
class IOBufCutter {
public:
    explicit IOBufCutter(eabase::IOBuf* buf);
    ~IOBufCutter();

    // Cut off n bytes and APPEND to `out'
    // Returns bytes cut.
    size_t cutn(eabase::IOBuf* out, size_t n);
    size_t cutn(std::string* out, size_t n);
    size_t cutn(void* out, size_t n);
...
class IOBufAsSnappySource : public eabase::snappy::Source {
public:
    explicit IOBufAsSnappySource(const eabase::IOBuf& buf)
...
private:
    const eabase::IOBuf* _buf;
    eabase::IOBufAsZeroCopyInputStream _stream;
};

// Wrap IOBuf into output of snappy compression.
class IOBufAsSnappySink : public eabase::snappy::Sink {
public:
    explicit IOBufAsSnappySink(eabase::IOBuf& buf);
...
private:
    ...
    int _cur_len;
    eabase::IOBuf* _buf;
    eabase::IOBufAsZeroCopyOutputStream _buf_stream;
};
...
public:
    ...
    int append(const eabase::StringPiece& str);
...
public:
    explicit IOBufBytesIterator(const eabase::IOBuf& buf);
    ...
    size_t append_and_forward(eabase::IOBuf* buf, size_t n);
...
namespace std {
template <>
inline void swap(eabase::IOBuf& a, eabase::IOBuf& b) {
    return a.swap(b);
}
} // namespace std

#include "eabase/utility/iobuf_inl.h"
