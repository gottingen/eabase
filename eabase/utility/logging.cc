...
#include "eabase/utility/debug/debugger.h"
#include "eabase/utility/debug/stack_trace.h"
#include "eabase/utility/posix/eintr_wrapper.h"
#include "eabase/utility/strings/string_util.h"
#include "eabase/utility/strings/stringprintf.h"
#include "eabase/utility/strings/utf_string_conversions.h"
#include "eabase/utility/synchronization/condition_variable.h"
...
#include "eabase/utility/containers/doubly_buffered_data.h"
#include "eabase/utility/memory/singleton.h"
#include "eabase/utility/endpoint.h"
#ifdef BAIDU_INTERNAL
#include "eabase/utility/comlog_sink.h"
#endif

extern "C" {
uint64_t EA_WEAK fiber_self();
typedef struct {
    uint32_t index;    // index in KeyTable
    uint32_t version;  // ABA avoidance
} fiber_key_t;
int EA_WEAK fiber_key_create(fiber_key_t* key,
                                  void (*destructor)(void* data));
int EA_WEAK fiber_setspecific(fiber_key_t key, void* data);
void* EA_WEAK fiber_getspecific(fiber_key_t key);
}
...
static PathString GetProcessName() {
    eabase::fd_guard fd(open("/proc/self/cmdline", O_RDONLY));
    if (fd < 0) {
        return "unknown";
    }
    char buf[512];
    ...
    return eabase::FilePath(std::string(buf)).BaseName().value();
}
#endif
...
// static
eabase::Mutex* LoggingLock::log_lock = NULL;
// static
...
struct EA_CACHELINE_ALIGNMENT LogRequest {
...
class AsyncLogger : public eabase::SimpleThread {
public:
...
friend struct DefaultSingletonTraits<AsyncLogger>;
    ...
    eabase::atomic<LogRequest*> _log_head;
    eabase::Mutex _mutex;
    eabase::ConditionVariable _cond;
    LogRequest* _current_log_request;
    eabase::atomic<int32_t> _log_request_count;
    eabase::atomic<bool> _stop;
};
...
AsyncLogger::AsyncLogger()
    : eabase::SimpleThread("async_log_thread")
    , _log_head(NULL)
    , _cond(&_mutex)
...
void AsyncLogger::Log(const std::string& log) {
    ...
    bool is_full = FLAGS_max_async_log_queue_size > 0 &&
        _log_request_count.fetch_add(1, eabase::memory_order_relaxed) >
        FLAGS_max_async_log_queue_size;
    if (is_full || _stop.load(eabase::memory_order_relaxed)) {
        // Async logger is full or stopped, fallback to sync log.
        DoLog(log);
        return;
    }

    auto log_req = eabase::get_object<LogRequest>();
    if (!log_req) {
...
void AsyncLogger::LogImpl(LogRequest* log_req) {
    ...
    LogRequest* const prev_head =
        _log_head.exchange(log_req, eabase::memory_order_release);
    if (prev_head != NULL) {
    ...
    if (!FLAGS_async_log_in_background_always) {
        ...
        if (IsLogComplete(log_req)) {
            eabase::return_object(log_req);
            return;
        }
    }

    BAIDU_SCOPED_LOCK(_mutex);
    if (_stop.load(eabase::memory_order_relaxed)) {
...
void AsyncLogger::StopAndJoin() {
    if (!_stop.exchange(true, eabase::memory_order_relaxed)) {
        BAIDU_SCOPED_LOCK(_mutex);
        _cond.Signal();
    }
    if (!HasBeenJoined()) {
...
void AsyncLogger::Run() {
    while (true) {
        BAIDU_SCOPED_LOCK(_mutex);
        while (!_stop.load(eabase::memory_order_relaxed) &&
...
void AsyncLogger::LogTask(LogRequest* req) {
    do {
        ...
        if (req->next != NULL && req->data.empty()) {
            ...
            req = req->next;
            eabase::return_object(saved_req);
        }
...
bool AsyncLogger::IsLogComplete(LogRequest* old_head) {
    ...
    if (_log_head.compare_exchange_strong(
        new_head, desired, eabase::memory_order_acquire)) {
...
void PrintLogPrefix(std::ostream& os, int severity,
    ...
    os << ' ' << std::setfill(' ') << std::setw(5)
       << eabase::PlatformThread::CurrentId() << std::setfill('0');
    if (FLAGS_log_hostname) {
        eabase::StringPiece hostname(eabase::my_hostname());
...
static void PrintLogPrefixAsJSON(std::ostream& os, int severity,
    ...
    os << "\"tid\":\"" << eabase::PlatformThread::CurrentId() << "\",";
    if (FLAGS_log_hostname) {
        eabase::StringPiece hostname(eabase::my_hostname());
        if (hostname.ends_with(".baidu.com")) { // make it shorter
            hostname.remove_suffix(10);
...
void EscapeJson(std::ostream& os, const eabase::StringPiece& s) {
...
inline void OutputLog(std::ostream& os, const eabase::StringPiece& s) {
    if (FLAGS_escape_log) {
        EscapeJson(os, s);
    } else {
...
void PrintLog(std::ostream& os, int severity, const char* file, int line,
              const char* func, const eabase::StringPiece& content) {
...
void DisplayDebugMessageInDialog(const std::string& str) {
    ...
    std::wstring cmdline = eabase::UTF8ToWide(str);
    if (cmdline.empty())
        return;
...
bool StringSink::OnLogMessage(int severity, const char* file,
    ...
    {
        eabase::AutoLock lock_guard(_lock);
        append(msg);
    }
    return true;
}
...
static void create_stream_key_or_die() {
    if (is_fiber_linked()) {
        int rc = fiber_key_create(&stream_bkey, destroy_tls_streams);
        if (rc) {
            fprintf(stderr, "Fail to fiber_key_create");
            exit(1);
        }
    } else {
...
static LogStream** get_tls_stream_array() {
    ...
    if (is_fiber_linked()) {
        return (LogStream**)fiber_getspecific(stream_bkey);
    } else {
...
static LogStream** get_or_new_tls_stream_array() {
    ...
    if (a == NULL) {
        ...
        if (is_fiber_linked()) {
            fiber_setspecific(stream_bkey, a);
        } else {
...
public:
    ...
    bool OnLogMessage(int severity, const char* file, int line,
                      const eabase::StringPiece& content) override {
        return OnLogMessage(severity, file, line, "", content);
    }
    ...
    if (FLAGS_print_stack_on_check && _is_check && _severity == BLOG_FATAL) {
        ...
        *this << std::endl;  // Newline to separate from log message.
        if (count > 3) {
            ...
            // #2 may be ~LogStream
            //   #0 0x00000059ccae eabase::debug::StackTrace::StackTrace()
            //   #1 0x0000005947c7 logging::LogStream::FlushWithoutReset()
            //   #2 0x000000594b88 logging::LogMessage::~LogMessage()
            eabase::debug::StackTrace trace_stripped(addrs + 3, count - 3);
...
FINISH_LOGGING:
    if (FLAGS_crash_on_fatal_log && _severity == BLOG_FATAL) {
        ...
        eabase::StringPiece str = content();
        char str_stack[1024];
        str.copy(str_stack, arraysize(str_stack));
        eabase::debug::Alias(str_stack);
...
BUTIL_EXPORT std::string SystemErrorCodeToString(SystemErrorCode error_code) {
    ...
    if (len) {
        ...
        return eabase::CollapseWhitespaceASCII(msgbuf, true) +
            eabase::StringPrintf(" (0x%X)", error_code);
    }
    return eabase::StringPrintf("Error (0x%X) while retrieving error. (0x%X)",
...
struct VLogSite {
    VLogSite(const char* filename, int required_v, int line_no)
        ...
        eabase::StringPiece s(filename);
        size_t pos = s.find_last_of("./");
        if (pos != eabase::StringPiece::npos) {
            if (s[pos] == '.') {
                s.remove_suffix(s.size() - pos);
                _full_module.assign(s.data(), s.size());
                size_t pos2 = s.find_last_of('/');
                if (pos2 != eabase::StringPiece::npos) {
    ...
    VLogSite* next() { return (VLogSite*)eabase::subtle::Acquire_Load(&_next); }
    const VLogSite* next() const
    { return (VLogSite*)eabase::subtle::Acquire_Load(&_next); }
    void set_next(VLogSite* next)
    { eabase::subtle::Release_Store(&_next, (eabase::subtle::AtomicWord)next); }
...
private:
    ...
    eabase::subtle::AtomicWord _next;
...
struct VModuleList {
    ...
    int init(const char* vmodules) {
        ...
        for (eabase::StringSplitter sp(vmodules, ','); sp; ++sp) {
    ...
    if (old_module_list) {
        ...
        deleting_vmodule_list->push_back(
            std::make_pair(old_module_list,
                           eabase::gettimeofday_us() + DELAY_DELETION_SEC * 1000000L));
        while (!deleting_vmodule_list->empty() &&
               deleting_vmodule_list->front().second <= eabase::gettimeofday_us()) {
...
std::ostream& operator<<(std::ostream& out, const wchar_t* wstr) {
    return out << eabase::WideToUTF8(std::wstring(wstr));
}

#endif  // EABASE_WITH_GLOG
