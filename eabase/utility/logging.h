...
#define EABASE_WITH_GLOG 0
#include <inttypes.h>
#include <string>
#include <cstring>
#include <sstream>
#include "eabase/utility/macros.h"    // BAIDU_CONCAT
#include "eabase/utility/atomicops.h" // Used by LOG_EVERY_N, LOG_FIRST_N etc
#include "eabase/utility/time.h"      // gettimeofday_us()
...
#include "eabase/utility/strings/string_piece.h"
...
public:
    ...
    virtual bool OnLogMessage(int severity, const char* file, int line,
                              const eabase::StringPiece& log_content) = 0;
    virtual bool OnLogMessage(int severity, const char* file,
                              int line, const char* func,
                              const eabase::StringPiece& log_content) {
        return true;
    }
private:
    EA_DISALLOW_COPY_AND_ASSIGN(LogSink);
};
...
public:
    bool OnLogMessage(int severity, const char* file, int line,
                      const eabase::StringPiece& log_content) override;
...
#define CHECK(condition)                                                \
    !(condition) ? ::eabase::debug::BreakDebugger() : BAIDU_EAT_STREAM_PARAMS
...
public:
    ...
    eabase::StringPiece content() const
...
private:
    EA_DISALLOW_COPY_AND_ASSIGN(LogMessage);
...
inline void LogAtLevel(int const log_level, const eabase::StringPiece &msg) {
...
private:
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(Win32ErrorLogMessage);
};
#elif defined(OS_POSIX)
...
private:
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(ErrnoLogMessage);
};
#endif  // OS_WIN
...
#define BAIDU_LOG_IF_EVERY_N_IMPL(logifmacro, severity, condition, N)   \
    static ::eabase::subtle::Atomic32 BAIDU_CONCAT(logeveryn_, __LINE__) = -1; \
    const static int BAIDU_CONCAT(logeveryn_sc_, __LINE__) = (N);       \
    const int BAIDU_CONCAT(logeveryn_c_, __LINE__) =                    \
        ::eabase::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logeveryn_, __LINE__), 1); \
...
#define BAIDU_LOG_IF_FIRST_N_IMPL(logifmacro, severity, condition, N)   \
    static ::eabase::subtle::Atomic32 BAIDU_CONCAT(logfstn_, __LINE__) = 0; \
    logifmacro(severity, (condition) && BAIDU_CONCAT(logfstn_, __LINE__) < N && \
               ::eabase::subtle::NoBarrier_AtomicIncrement(&BAIDU_CONCAT(logfstn_, __LINE__), 1) <= N)

// Helper macro included by all *_EVERY_SECOND macros.
#define BAIDU_LOG_IF_EVERY_SECOND_IMPL(logifmacro, severity, condition) \
    static ::eabase::subtle::Atomic64 BAIDU_CONCAT(logeverys_, __LINE__) = 0; \
    const int64_t BAIDU_CONCAT(logeverys_ts_, __LINE__) = ::eabase::gettimeofday_us(); \
    ...
    logifmacro(severity, (condition) && BAIDU_CONCAT(logeverys_ts_, __LINE__) >= \
               ...
               ::eabase::subtle::NoBarrier_CompareAndSwap(                \
