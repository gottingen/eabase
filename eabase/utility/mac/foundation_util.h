...
#include "eabase/utility/mac/scoped_cftyperef.h"
...
BUTIL_EXPORT bool AmIBundled();
BUTIL_EXPORT void SetOverrideAmIBundled(bool value);

// Returns true if this process is marked as a "Background only process".
BUTIL_EXPORT bool IsBackgroundOnlyProcess();

// Returns the path to a resource within the framework bundle.
BUTIL_EXPORT FilePath PathForFrameworkBundleResource(CFStringRef resourceName);
...
BUTIL_EXPORT void NSObjectRetain(void* obj);
BUTIL_EXPORT void NSObjectRelease(void* obj);
...
BUTIL_EXPORT void SetBaseBundleID(const char* new_base_bundle_id);
...
//
// Example usage:
// CFNumberRef some_number = eabase::mac::CFCast<CFNumberRef>(
//     CFArrayGetValueAtIndex(array, index));
//
// CFTypeRef hello = CFSTR("hello world");
// CFStringRef some_string = eabase::mac::CFCastStrict<CFStringRef>(hello);
...
//
// Example usage:
// NSString* version = eabase::mac::ObjCCast<NSString>(
//     [bundle objectForInfoDictionaryKey:@"CFBundleShortVersionString"]);
//
// NSString* str = eabase::mac::ObjCCastStrict<NSString>(
...
BUTIL_EXPORT NSString* FilePathToNSString(const FilePath& path);

// Converts |str| to a FilePath. Returns an empty path if |str| is nil.
BUTIL_EXPORT FilePath NSStringToFilePath(NSString* str);
...
// e.g. LOG(INFO) << eabase::mac::NSToCFCast(@"foo");
