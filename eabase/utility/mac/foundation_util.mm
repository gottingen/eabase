...
#include "eabase/utility/mac/bundle_locations.h"
//#include "eabase/utility/mac/mac_logging.h"
...
bool UncachedAmIBundled() {
  ...
  // Yes, this is cheap.
  return [[eabase::mac::OuterBundle() bundlePath] hasSuffix:@".app"];
#endif
}
...
bool IsBackgroundOnlyProcess() {
  ...
  NSDictionary* info_dictionary = [eabase::mac::MainBundle() infoDictionary];
...
FilePath PathForFrameworkBundleResource(CFStringRef resourceName) {
  NSBundle* bundle = eabase::mac::FrameworkBundle();
...
std::string GetValueFromDictionaryErrorMessage(
  ...
  return "Expected value for key " +
      eabase::SysCFStringRefToUTF8(key) +
      " to be " +
      expected_type +
      " but it was " +
      eabase::SysCFStringRefToUTF8(actual_type_ref) +
      " instead";
}
...
std::ostream& operator<<(std::ostream& o, const CFStringRef string) {
  return o << eabase::SysCFStringRefToUTF8(string);
}

std::ostream& operator<<(std::ostream& o, const CFErrorRef err) {
  eabase::ScopedCFTypeRef<CFStringRef> desc(CFErrorCopyDescription(err));
  eabase::ScopedCFTypeRef<CFDictionaryRef> user_info(CFErrorCopyUserInfo(err));
