...
#include "eabase/utility/compiler_specific.h"  // For ALLOW_UNUSED.
#include "eabase/utility/string_printf.h"      // For eabase::string_printf().

// There must be many copy-paste versions of these macros which are same
// things, undefine them to avoid conflict.
#undef DISALLOW_COPY
#undef DISALLOW_ASSIGN
#undef EA_DISALLOW_COPY_AND_ASSIGN
...
#define EA_DISALLOW_COPY_AND_ASSIGN(TypeName)                      \
    BUTIL_DELETE_FUNCTION(TypeName(const TypeName&));            \
    BUTIL_DELETE_FUNCTION(void operator=(const TypeName&))
...
#define DISALLOW_EVIL_CONSTRUCTORS(TypeName) EA_DISALLOW_COPY_AND_ASSIGN(TypeName)
...
#define DISALLOW_IMPLICIT_CONSTRUCTORS(TypeName) \
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(TypeName)
...
#define arraysize(array) (sizeof(::eabase::ArraySizeHelper(array)))
...
// the eabase::LINKER_INITIALIZED argument.  Normally, it is unsafe to declare a
// static variable that has a constructor or a destructor because invocation
// order is undefined.  However, IF the type can be initialized by filling with
// zeroes (which the loader does for static variables), AND the destructor also
// does nothing to the storage, AND there are no virtual methods, then a
// constructor declared as
//       explicit MyClass(eabase::LinkerInitialized x) {}
// and invoked as
//       static MyClass my_variable_name(eabase::LINKER_INITIALIZED);
namespace eabase {
enum LinkerInitialized { LINKER_INITIALIZED };

// Use these to declare and define a static local variable (static T;) so that
// it is leaked so that its destructors are not called at exit. If you need
// thread-safe initialization, use eabase/utility/lazy_instance.h instead.
...
#ifndef EA_TYPEOF
# if defined(BUTIL_CXX11_ENABLED)
#  define EA_TYPEOF decltype
# else
#  ifdef _MSC_VER
#   include <boost/typeof/typeof.hpp>
#   define EA_TYPEOF BOOST_TYPEOF
#  else
#   define EA_TYPEOF typeof
#  endif
# endif // BUTIL_CXX11_ENABLED
#endif  // EA_TYPEOF
...
# define container_of(ptr, type, member) ({                             \
            const EA_TYPEOF( ((type *)0)->member ) *__mptr = (ptr);  \
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::eabase::internal::ArrayDeleter<Tp> name##_array_deleter;            \
...
# define DEFINE_SMALL_ARRAY(Tp, name, size, maxsize)                    \
    ...
    ::eabase::internal::ArrayDeleter<char> name##_array_deleter;          \
    if (name##_stack_array_size) {                                      \
    ...
    const ::eabase::internal::ArrayCtorDtor<Tp> name##_array_ctor_dtor(name, name##_size);
...
#define ASSERT_LOG(fmt, ...)                                            \
    do {                                                                \
        std::string log = eabase::string_printf(fmt, ## __VA_ARGS__);    \
        LOG(FATAL) << log;                                              \
    } while (false)
