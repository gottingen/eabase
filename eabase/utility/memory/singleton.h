...
#include "eabase/utility/third_party/dynamic_annotations/dynamic_annotations.h"
...
// TODO(joth): Move more of this file into namespace eabase
...
struct StaticMemorySingletonTraits {
  ...
  static Type* New() {
    ...
    if (eabase::subtle::NoBarrier_AtomicExchange(&dead_, 1))
      return NULL;
  ...
  static void Resurrect() {
    eabase::subtle::NoBarrier_Store(&dead_, 0);
  }

 private:
  static eabase::AlignedMemory<sizeof(Type), ALIGNOF(Type)> buffer_;
  // Signal the object was already deleted, so it is not revived.
  static eabase::subtle::Atomic32 dead_;
};

template <typename Type> eabase::AlignedMemory<sizeof(Type), ALIGNOF(Type)>
    StaticMemorySingletonTraits<Type>::buffer_;
template <typename Type> eabase::subtle::Atomic32
...
//
//     EA_DISALLOW_COPY_AND_ASSIGN(FooClass);
//   };
//
// In your source file:
...
// process exit. More precisely it uses eabase::AtExitManager which requires an
...
class Singleton {
 private:
  ...
  static Type* get() {
    ...
    eabase::subtle::AtomicWord value = eabase::subtle::Acquire_Load(&instance_);
    if (value != 0 && value != eabase::internal::kBeingCreatedMarker) {
    ...
    if (eabase::subtle::Acquire_CompareAndSwap(
          &instance_, 0, eabase::internal::kBeingCreatedMarker) == 0) {
      ...
      eabase::subtle::Release_Store(
          &instance_, reinterpret_cast<eabase::subtle::AtomicWord>(newval));

      if (newval != NULL && Traits::kRegisterAtExit)
        eabase::AtExitManager::RegisterCallback(OnExit, NULL);

      return newval;
    }

    // We hit a race. Wait for the other thread to complete it.
    value = eabase::internal::WaitForInstance(&instance_);
  ...
  static void OnExit(void* /*unused*/) {
    ...
    Traits::Delete(
        reinterpret_cast<Type*>(eabase::subtle::NoBarrier_Load(&instance_)));
    instance_ = 0;
  }
  static eabase::subtle::AtomicWord instance_;
};

template <typename Type, typename Traits, typename DifferentiatingType>
eabase::subtle::AtomicWord Singleton<Type, Traits, DifferentiatingType>::
    instance_ = 0;
