...
#include "eabase/utility/sys_byteorder.h"
...
const eabase::IOBuf* Record::Meta(const char* name) const {
...
eabase::IOBuf* Record::MutableMeta(const char* name_cstr, bool null_on_found) {
    const eabase::StringPiece name = name_cstr;
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
    ...
    p.data = std::make_shared<eabase::IOBuf>();
    _metas.push_back(p);
    return p.data.get();
}

eabase::IOBuf* Record::MutableMeta(const std::string& name, bool null_on_found) {
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
            return null_on_found ? NULL : _metas[i].data.get();
        }
...
bool Record::RemoveMeta(const eabase::StringPiece& name) {
    for (size_t i = 0; i < _metas.size(); ++i) {
        if (_metas[i].name == name) {
...
bool RecordReader::CutUntilNextRecordCandidate() {
    ...
    if (*(const uint32_t*)dummy == *(const uint32_t*)EA_RECORDIO_MAGIC) {
...
int RecordReader::CutRecord(Record* rec) {
    ...
    if (*(const uint32_t*)dummy != *(const uint32_t*)EA_RECORDIO_MAGIC) {
        LOG(ERROR) << "Invalid magic_num="
                   << eabase::PrintedAsBinary(std::string((char*)headbuf, 4))
                   << ", offset=" << offset();
        return -1;
    }
    ...
    while (has_meta) {
        ...
        eabase::IOBuf* meta = rec->MutableMeta(name, true/*null_on_found*/);
        if (meta == NULL) {
...
int RecordWriter::WriteWithoutFlush(const Record& rec) {
    ...
    void* dummy = headbuf;  // suppressing strict-aliasing warning
    *(uint32_t*)dummy = *(const uint32_t*)EA_RECORDIO_MAGIC;
