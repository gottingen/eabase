...
public:
    struct NamedMeta {
        std::string name;
        std::shared_ptr<eabase::IOBuf> data;
    };
    ...
    const eabase::IOBuf* Meta(const char* name) const;
    ...
    // are a lot.
    eabase::IOBuf* MutableMeta(const char* name, bool null_on_found = false);
    eabase::IOBuf* MutableMeta(const std::string& name, bool null_on_found = false);

    // Remove meta with the name. The impl. may scan all fields.
    // Returns true on erased, false on absent.
    bool RemoveMeta(const eabase::StringPiece& name);

    // Get the payload. Empty by default.
    const eabase::IOBuf& Payload() const { return _payload; }

    // Get a mutable pointer to the payload.
    eabase::IOBuf* MutablePayload() { return &_payload; }
