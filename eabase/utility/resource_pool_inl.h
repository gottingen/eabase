...
#include "eabase/utility/macros.h"                 // EA_CACHELINE_ALIGNMENT
#include "eabase/utility/scoped_lock.h"            // BAIDU_SCOPED_LOCK
#include "eabase/utility/thread_local.h"           // thread_atexit
#include <vector>

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_ADD1                \
    (_global_nfree.fetch_add(1, eabase::memory_order_relaxed))
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_SUB1                \
    (_global_nfree.fetch_sub(1, eabase::memory_order_relaxed))
#else
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_ADD1
#define BAIDU_RESOURCE_POOL_FREE_ITEM_NUM_SUB1
#endif
...
template <typename T>
class EA_CACHELINE_ALIGNMENT ResourcePool {
public:
    ...
    struct EA_CACHELINE_ALIGNMENT Block {
    ...
    struct BlockGroup {
        eabase::atomic<size_t> nblock;
        eabase::atomic<Block*> blocks[RP_GROUP_NBLOCK];
        ...
        BlockGroup() : nblock(0) {
            ...
            memset(static_cast<void*>(blocks), 0, sizeof(eabase::atomic<Block*>) * RP_GROUP_NBLOCK);
        }
    };


    // Each thread has an instance of this class.
    class EA_CACHELINE_ALIGNMENT LocalPool {
    public:
...
#undef BAIDU_RESOURCE_POOL_GET
    ...
    static inline T* unsafe_address_resource(ResourceId<T> id) {
        ...
        return (T*)(_block_groups[(block_index >> RP_GROUP_NBLOCK_NBIT)]
                    .load(eabase::memory_order_consume)
                    ->blocks[(block_index & (RP_GROUP_NBLOCK - 1))]
                    .load(eabase::memory_order_consume)->items) +
    ...
    static inline T* address_resource(ResourceId<T> id) {
        ...
        if (__builtin_expect(group_index < RP_MAX_BLOCK_NGROUP, 1)) {
            BlockGroup* bg =
                _block_groups[group_index].load(eabase::memory_order_consume);
    ...
    void clear_resources() {
        ...
        if (lp) {
            _local_pool = NULL;
            eabase::thread_atexit_cancel(LocalPool::delete_local_pool, lp);
            delete lp;
        }
    }
    ...
    ResourcePoolInfo describe_resources() const {
        ...
        info.local_pool_num = _nlocal.load(eabase::memory_order_relaxed);
        info.block_group_num = _ngroup.load(eabase::memory_order_acquire);
        info.block_num = 0;
        info.item_num = 0;
        info.free_chunk_item_num = free_chunk_nitem();
        info.block_item_num = BLOCK_NITEM;
#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
        info.free_item_num = _global_nfree.load(eabase::memory_order_relaxed);
#endif

        for (size_t i = 0; i < info.block_group_num; ++i) {
            BlockGroup* bg = _block_groups[i].load(eabase::memory_order_consume);
            if (NULL == bg) {
                break;
            }
            size_t nblock = std::min(bg->nblock.load(eabase::memory_order_relaxed),
                                     RP_GROUP_NBLOCK);
            info.block_num += nblock;
            for (size_t j = 0; j < nblock; ++j) {
                Block* b = bg->blocks[j].load(eabase::memory_order_consume);
                if (NULL != b) {
    ...
    static inline ResourcePool* singleton() {
        ResourcePool* p = _singleton.load(eabase::memory_order_consume);
        if (p) {
            return p;
        }
        pthread_mutex_lock(&_singleton_mutex);
        p = _singleton.load(eabase::memory_order_consume);
        if (!p) {
            p = new ResourcePool();
            _singleton.store(p, eabase::memory_order_release);
        } 
...
private:
    ...
    static Block* add_block(size_t* index) {
        ...
        size_t ngroup;
        do {
            ngroup = _ngroup.load(eabase::memory_order_acquire);
            if (ngroup >= 1) {
                BlockGroup* const g =
                    _block_groups[ngroup - 1].load(eabase::memory_order_consume);
                const size_t block_index =
                    g->nblock.fetch_add(1, eabase::memory_order_relaxed);
                if (block_index < RP_GROUP_NBLOCK) {
                    g->blocks[block_index].store(
                        new_block, eabase::memory_order_release);
                    *index = (ngroup - 1) * RP_GROUP_NBLOCK + block_index;
                    return new_block;
                }
                g->nblock.fetch_sub(1, eabase::memory_order_relaxed);
            }
    ...
    static bool add_block_group(size_t old_ngroup) {
        ...
        const size_t ngroup = _ngroup.load(eabase::memory_order_acquire);
        ...
        if (ngroup < RP_MAX_BLOCK_NGROUP) {
            ...
            if (NULL != bg) {
                ...
                // threads.
                _block_groups[ngroup].store(bg, eabase::memory_order_release);
                _ngroup.store(ngroup + 1, eabase::memory_order_release);
            }
        }
        return bg != NULL;
    }
    ...
    inline LocalPool* get_or_new_local_pool() {
        ...
        _local_pool = lp;
        eabase::thread_atexit(LocalPool::delete_local_pool, lp);
        _nlocal.fetch_add(1, eabase::memory_order_relaxed);
        return lp;
    }
    ...
    void clear_from_destructor_of_local_pool() {
        ...
        if (_nlocal.fetch_sub(1, eabase::memory_order_relaxed) != 1) {
            return;
        }
        ...
        if (_nlocal.load(eabase::memory_order_relaxed) != 0) {
            return;
        }
        ...
        // Delete all memory
        const size_t ngroup = _ngroup.exchange(0, eabase::memory_order_relaxed);
        for (size_t i = 0; i < ngroup; ++i) {
            BlockGroup* bg = _block_groups[i].load(eabase::memory_order_relaxed);
            if (NULL == bg) {
                break;
            }
            size_t nblock = std::min(bg->nblock.load(eabase::memory_order_relaxed),
                                     RP_GROUP_NBLOCK);
            for (size_t j = 0; j < nblock; ++j) {
                Block* b = bg->blocks[j].load(eabase::memory_order_relaxed);
                if (NULL == b) {
                    continue;
                }
...
private:
    ...
    static eabase::static_atomic<ResourcePool*> _singleton;
    static pthread_mutex_t _singleton_mutex;
    static BAIDU_THREAD_LOCAL LocalPool* _local_pool;
    static eabase::static_atomic<long> _nlocal;
    static eabase::static_atomic<size_t> _ngroup;
    static pthread_mutex_t _block_group_mutex;
    static pthread_mutex_t _change_thread_mutex;
    static eabase::static_atomic<BlockGroup*> _block_groups[RP_MAX_BLOCK_NGROUP];

    std::vector<DynamicFreeChunk*> _free_chunks;
    pthread_mutex_t _free_chunks_mutex;

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
    static eabase::static_atomic<size_t> _global_nfree;
#endif
};
...
template <typename T>
eabase::static_atomic<ResourcePool<T>*> ResourcePool<T>::_singleton =
...
template <typename T>
eabase::static_atomic<long> ResourcePool<T>::_nlocal = BUTIL_STATIC_ATOMIC_INIT(0);

template <typename T>
eabase::static_atomic<size_t> ResourcePool<T>::_ngroup = BUTIL_STATIC_ATOMIC_INIT(0);
...
template <typename T>
eabase::static_atomic<typename ResourcePool<T>::BlockGroup*>
ResourcePool<T>::_block_groups[RP_MAX_BLOCK_NGROUP] = {};

#ifdef BUTIL_RESOURCE_POOL_NEED_FREE_ITEM_NUM
template <typename T>
eabase::static_atomic<size_t> ResourcePool<T>::_global_nfree = BUTIL_STATIC_ATOMIC_INIT(0);
#endif
