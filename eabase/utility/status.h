...
public:
    ...
    Status(int code, const eabase::StringPiece& error_msg) : _state(NULL) {
    ...
    int set_error(int code, const char* error_format, ...)
        __attribute__ ((__format__ (__printf__, 3, 4)));
    int set_error(int code, const eabase::StringPiece& error_msg);
    ...
    eabase::StringPiece error_data() const {
        return (_state == NULL ? eabase::StringPiece("OK", 2)
                : eabase::StringPiece(_state->message, _state->size));
    }
    std::string error_str() const;

    void swap(eabase::Status& other) { std::swap(_state, other._state); }
...
inline void Status::operator=(const Status& s) {
    ...
    if (s._state == NULL) {
        ...
        set_error(s._state->code,
                  eabase::StringPiece(s._state->message, s._state->size));
    }
}
