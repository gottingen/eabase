...
//
// StringPiece16 is similar to StringPiece but for eabase::string16 instead of
...
// BasicStringPiece<std::string> or BasicStringPiece<eabase::string16>.
template <typename STRING_TYPE> class BasicStringPiece {
 public:
...
// This hash function is copied from eabase/utility/containers/hash_tables.h. We don't
...
template<>
struct hash<eabase::StringPiece> {
  std::size_t operator()(const eabase::StringPiece& sp) const {
    HASH_STRING_PIECE(eabase::StringPiece, sp);
  }
};
template<>
struct hash<eabase::StringPiece16> {
  std::size_t operator()(const eabase::StringPiece16& sp16) const {
    HASH_STRING_PIECE(eabase::StringPiece16, sp16);
  }
};

#elif defined(COMPILER_MSVC)

inline size_t hash_value(const eabase::StringPiece& sp) {
  HASH_STRING_PIECE(eabase::StringPiece, sp);
}
inline size_t hash_value(const eabase::StringPiece16& sp16) {
  HASH_STRING_PIECE(eabase::StringPiece16, sp16);
}
