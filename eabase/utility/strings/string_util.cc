...
#include "eabase/utility/strings/utf_string_conversion_utils.h"
#include "eabase/utility/strings/utf_string_conversions.h"
#include "eabase/utility/third_party/icu/icu_utf.h"
#include "eabase/utility/build_config.h"

// Remove when this entire file is in namespace eabase.
using eabase::char16;
using eabase::string16;
...
TrimPositions TrimWhitespace(const string16& input,
                             TrimPositions positions,
                             string16* output) {
  return TrimStringT(input, eabase::string16(kWhitespaceUTF16), positions,
                     output);
}

TrimPositions TrimWhitespace(const eabase::StringPiece16& input,
                             TrimPositions positions,
                             eabase::StringPiece16* output) {
  return TrimStringT(input, eabase::StringPiece16(kWhitespaceUTF16), positions,
                     output);
}
...
TrimPositions TrimWhitespaceASCII(const eabase::StringPiece& input,
                                  TrimPositions positions,
                                  eabase::StringPiece* output) {
  return TrimStringT(input, eabase::StringPiece(kWhitespaceASCII), positions, output);
}
...
TrimPositions TrimWhitespace(const eabase::StringPiece& input,
                             TrimPositions positions,
                             eabase::StringPiece* output) {
  return TrimWhitespaceASCII(input, positions, output);
}
...
bool EqualsASCII(const string16& a, const eabase::StringPiece& b) {
...
bool StartsWithASCII(const std::string& str,
  ...
  if (case_sensitive)
    ...
    return eabase::strncasecmp(str.c_str(), search.c_str(), search.length()) == 0;
}
...
bool StartsWithT(const STR& str, const STR& search, bool case_sensitive) {
  if (case_sensitive) {
    ...
    return std::equal(search.begin(), search.end(), str.begin(),
                      eabase::CaseInsensitiveCompare<typename STR::value_type>());
  }
}
...
string16 FormatBytesUnlocalized(int64_t bytes) {
  ...
  if (bytes != 0 && dimension > 0 && unit_amount < 100) {
    eabase::snprintf(buf, arraysize(buf), "%.1lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  } else {
    eabase::snprintf(buf, arraysize(buf), "%.0lf%s", unit_amount,
                   kByteStringsUnlocalized[dimension]);
  }

  return eabase::ASCIIToUTF16(buf);
}
...
size_t Tokenize(const eabase::StringPiece& str,
...
std::string ReplaceStringPlaceholders(const eabase::StringPiece& format_string,
...
bool MatchPattern(const eabase::StringPiece& eval,
...
size_t eabase::strlcpy(char* dst, const char* src, size_t dst_size) {
  return lcpyT<char>(dst, src, dst_size);
}
size_t eabase::wcslcpy(wchar_t* dst, const wchar_t* src, size_t dst_size) {
