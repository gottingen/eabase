...
#include "eabase/utility/strings/utf_offset_string_conversions.h"
...
string16 UTF8ToUTF16AndAdjustOffsets(
  ...
  std::for_each(offsets_for_adjustment->begin(),
                ...
                LimitOffset<eabase::StringPiece>(utf8.length()));
  OffsetAdjuster::Adjustments adjustments;
...
std::string UTF16ToUTF8AndAdjustOffsets(
    const eabase::StringPiece16& utf16,
    std::vector<size_t>* offsets_for_adjustment) {
  std::for_each(offsets_for_adjustment->begin(),
                offsets_for_adjustment->end(),
                LimitOffset<eabase::StringPiece16>(utf16.length()));
  std::string result;
