...
#define SPIN_FOR_1_SECOND_OR_UNTIL_TRUE(expression) \
    SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(eabase::TimeDelta::FromSeconds(1), \
                                     (expression))

#define SPIN_FOR_TIMEDELTA_OR_UNTIL_TRUE(delta, expression) do { \
  eabase::TimeTicks start = eabase::TimeTicks::Now(); \
  const eabase::TimeDelta kTimeout = delta; \
    while (!(expression)) { \
      if (kTimeout < eabase::TimeTicks::Now() - start) { \
      EXPECT_LE((eabase::TimeTicks::Now() - start).InMilliseconds(), \
                kTimeout.InMilliseconds()) << "Timed out"; \
        break; \
      } \
      eabase::PlatformThread::Sleep(eabase::TimeDelta::FromMilliseconds(50)); \
    } \
  } while (0)
