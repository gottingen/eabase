...
static eabase::LazyInstance<eabase::Lock>::Leaky
...
inline static void ACQUIRE_DTOA_LOCK(size_t n) {
  DCHECK(n < 2);
  eabase::Lock* lock = n == 0 ? dtoa_lock_0.Pointer() : dtoa_lock_1.Pointer();
  lock->Acquire();
}
...
#include "eabase/utility/third_party/dmg_fp/dtoa.cc"
