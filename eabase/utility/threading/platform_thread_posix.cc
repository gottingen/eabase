...
#include "eabase/utility/safe_strerror_posix.h"
...
void* ThreadFunc(void* params) {
  ...
  if (!thread_params->joinable)
    eabase::ThreadRestrictions::SetSingletonAllowed(false);
...
bool CreateThread(size_t stack_size, bool joinable,
  ...
  if (stack_size == 0)
    stack_size = eabase::GetDefaultThreadStackSize(attributes);
...
bool PlatformThread::Create(size_t stack_size, Delegate* delegate,
                            PlatformThreadHandle* thread_handle) {
  eabase::ThreadRestrictions::ScopedAllowWait allow_wait;
...
void PlatformThread::Join(PlatformThreadHandle thread_handle) {
  ...
  // blocking tasks.
  eabase::ThreadRestrictions::AssertIOAllowed();
