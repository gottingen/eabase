...
using eabase::internal::PlatformThreadLocalStorage;
...
eabase::subtle::AtomicWord g_native_tls_key =
...
eabase::subtle::Atomic32 g_last_used_tls_key = 0;
...
volatile eabase::ThreadLocalStorage::TLSDestructorFunc
...
void** ConstructTlsVector() {
  PlatformThreadLocalStorage::TLSKey key =
      eabase::subtle::NoBarrier_Load(&g_native_tls_key);
  if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
    ...
    if (PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES !=
        eabase::subtle::NoBarrier_CompareAndSwap(&g_native_tls_key,
            PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES, key)) {
      // We've been shortcut. Another thread replaced g_native_tls_key first so
      // we need to destroy our index and use the one the other thread got
      // first.
      PlatformThreadLocalStorage::FreeTLS(key);
      key = eabase::subtle::NoBarrier_Load(&g_native_tls_key);
    }
  }
...
void OnThreadExitInternal(void* value) {
  ...
  while (need_to_scan_destructors) {
    ...
    eabase::subtle::Atomic32 last_used_tls_key =
        eabase::subtle::NoBarrier_Load(&g_last_used_tls_key);
    for (int slot = last_used_tls_key; slot > 0; --slot) {
      void* value = stack_allocated_tls_data[slot];
      if (value == NULL)
        continue;

      eabase::ThreadLocalStorage::TLSDestructorFunc destructor =
...
bool ThreadLocalStorage::StaticSlot::Initialize(TLSDestructorFunc destructor) {
  ...
  // Grab a new slot.
  slot_ = eabase::subtle::NoBarrier_AtomicIncrement(&g_last_used_tls_key, 1);
  DCHECK_GT(slot_, 0);
