...
#include "eabase/utility/mac/scoped_mach_port.h"
...
uint64_t ComputeCurrentTicks() {
  ...
  eabase::TimeDelta time_difference = eabase::Time::Now() -
      (eabase::Time::FromTimeT(boottime.tv_sec) +
       eabase::TimeDelta::FromMicroseconds(boottime.tv_usec));
  ...
  absolute_micro =
      mach_absolute_time() / eabase::Time::kNanosecondsPerMicrosecond *
...
uint64_t ComputeThreadTicks() {
  ...
  return 0;
#else
  eabase::mac::ScopedMachSendRight thread(mach_thread_self());
  ...
  return (thread_info_data.user_time.seconds *
              eabase::Time::kMicrosecondsPerSecond) +
...
Time Time::FromExploded(bool is_local, const Exploded& exploded) {
  ...
  eabase::ScopedCFTypeRef<CFTimeZoneRef> time_zone(
