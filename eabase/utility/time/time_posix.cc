...
eabase::TimeTicks ClockNow(clockid_t clk_id) {
  uint64_t absolute_micro;

  struct timespec ts;
  if (clock_gettime(clk_id, &ts) != 0) {
    NOTREACHED() << "clock_gettime(" << clk_id << ") failed.";
    return eabase::TimeTicks();
  }

  absolute_micro =
      (static_cast<int64_t>(ts.tv_sec) * eabase::Time::kMicrosecondsPerSecond) +
      (static_cast<int64_t>(ts.tv_nsec) / eabase::Time::kNanosecondsPerMicrosecond);

  return eabase::TimeTicks::FromInternalValue(absolute_micro);
}
