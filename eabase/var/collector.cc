...
class Collector : public eabase::Reducer<Collected*, CombineCollected> {
public:
    Collector();
    ~Collector();
...
private:
    ...
    static void* run_grab_thread(void* arg) {
        eabase::PlatformThread::SetName("var_collector_grabber");
        static_cast<Collector*>(arg)->grab_thread();
        return NULL;
    }

    static void* run_dump_thread(void* arg) {
        eabase::PlatformThread::SetName("var_collector_dumper");
...
static CollectorSpeedLimit g_null_speed_limit = VAR_COLLECTOR_SPEED_LIMIT_INITIALIZER;
...
void Collector::grab_thread() {
    ...
    // vars
    eabase::PassiveStatus<int64_t> pending_sampled_data(
    ...
    eabase::PerSecond<eabase::PassiveStatus<double> > busy_seconds_second(
        "var_collector_grab_thread_usage", &busy_seconds_var);

    eabase::PassiveStatus<int64_t> ngrab_var(deref_value<int64_t>, &_ngrab);
    eabase::PerSecond<eabase::PassiveStatus<int64_t> > ngrab_second(
    ...
    while (!_stop) {
        ...
        eabase::LinkNode<Collected>* head = this->reset();
        if (head) {
            eabase::LinkNode<Collected> tmp_root;
            head->InsertBeforeAsList(&tmp_root);
            head = NULL;
            
            // Group samples by preprocessors.
            for (eabase::LinkNode<Collected>* p = tmp_root.next(); p != &tmp_root;) {
                eabase::LinkNode<Collected>* saved_next = p->next();
                p->RemoveFromList();
            ...
            // Iterate prep_map
            eabase::LinkNode<Collected> root;
            ...
            if (root.next() != &root) {  // non empty
                eabase::LinkNode<Collected>* head2 = root.next();
                root.RemoveFromList();
        ...
        int64_t now = eabase::cpuwide_time_us();
        ...
        if (!_stop && abstime > now) {
            timespec abstimespec = eabase::microseconds_from_now(abstime - now);
    ...
    *last_ngrab = cur_ngrab;
    ...
    if (!sl->ever_grabbed) {
        if (sl->first_sample_real_us) {
            interval_us = eabase::gettimeofday_us() - sl->first_sample_real_us;
            if (interval_us < 0) {
                interval_us = 0;
            }
        } else {
...
size_t is_collectable_before_first_time_grabbed(CollectorSpeedLimit* sl) {
    if (!sl->ever_grabbed) {
        int before_add = sl->count_before_grabbed.fetch_add(
            1, eabase::memory_order_relaxed);
        if (before_add == 0) {
            sl->first_sample_real_us = eabase::gettimeofday_us();
        } else if (before_add >= FLAGS_var_collector_expected_per_second) {
            eabase::get_leaky_singleton<Collector>()->wakeup_grab_thread();
        }
    }
...
void Collector::dump_thread() {
    int64_t last_ns = eabase::cpuwide_time_ns();
    ...
    eabase::PassiveStatus<int64_t> ndumped_var(deref_value<int64_t>, &_ndump);
    eabase::PerSecond<eabase::PassiveStatus<int64_t> > ndumped_second(
        "eabase::collector_dump_second", &ndumped_var);
    ...
    while (!_stop) {
        ...
        // Call callbacks.
        for (eabase::LinkNode<Collected>* p = root.next(); !_stop && p != &root;) {
...
void Collected::submit(int64_t cpuwide_us) {
    Collector* d = eabase::get_leaky_singleton<Collector>();
