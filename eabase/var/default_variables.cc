...
#include "eabase/utility/files/dir_reader_posix.h"
#include "eabase/utility/file_util.h"
#include "eabase/utility/process_util.h"            // ReadCommandLine
#include "eabase/utility/popen.h"                   // read_command_output
...
#define VAR_MEMBER_TYPE(member) EA_TYPEOF(eabase::get_member_type(member))
...
static bool read_proc_status(ProcStat &stat) {
    ...
    eabase::ScopedFILE fp("/proc/self/stat", "r");
    if (NULL == fp) {
    ...
    if (eabase::read_command_output(oss, cmdbuf) != 0) {
...
public:
    ...
    static const T& get_value(const ReadFn& fn) {
        CachedReader* p = eabase::get_leaky_singleton<CachedReader>();
...
static bool read_proc_memory(ProcMemory &m) {
    ...
    errno = 0;
#if defined(OS_LINUX)
    eabase::ScopedFILE fp("/proc/self/statm", "r");
    if (NULL == fp) {
...
#if defined(OS_LINUX)
    eabase::ScopedFILE fp("/proc/loadavg", "r");
    if (NULL == fp) {
...
#elif defined(OS_MACOSX)
    std::ostringstream oss;
    if (eabase::read_command_output(oss, "sysctl -n vm.loadavg") != 0) {
...
#if defined(OS_LINUX)
    eabase::DirReaderPosix dr("/proc/self/fd");
    int count = 0;
    if (!dr.IsValid()) {
...
static eabase::static_atomic<bool> s_ever_reached_fd_scan_limit = BUTIL_STATIC_ATOMIC_INIT(false);
class FdReader {
public:
    bool operator()(int* stat) const {
        if (s_ever_reached_fd_scan_limit.load(eabase::memory_order_relaxed)) {
        ...
        if (count == MAX_FD_SCAN_COUNT - 2 
                && s_ever_reached_fd_scan_limit.exchange(
                        true, eabase::memory_order_relaxed) == false) {
...
#if defined(OS_LINUX)
    eabase::ScopedFILE fp("/proc/self/io", "r");
    if (NULL == fp) {
...
#if defined(OS_LINUX)
    eabase::ScopedFILE fp("/proc/diskstats", "r");
    if (NULL == fp) {
...
struct ReadSelfCmdline {
    ...
    ReadSelfCmdline() {
        char buf[1024];
        const ssize_t nr = eabase::ReadCommandLine(buf, sizeof(buf), true);
        content.append(buf, nr);
    }
};
static void get_cmdline(std::ostream& os, void*) {
    os << eabase::get_leaky_singleton<ReadSelfCmdline>()->content;
}
...
struct ReadVersion {
    ...
    ReadVersion() {
        std::ostringstream oss;
        if (eabase::read_command_output(oss, "uname -ap") != 0) {
...
static void get_kernel_version(std::ostream& os, void*) {
    os << eabase::get_leaky_singleton<ReadVersion>()->content;
}

// ======================================

static int64_t g_starting_time = eabase::gettimeofday_us();

static timeval get_uptime(void*) {
    int64_t uptime_us = eabase::gettimeofday_us() - g_starting_time;
    timeval tm;
...
static TimePercent get_cputime_percent(void*) {
    TimePercent tp = { eabase::timeval_to_microseconds(g_ru_stime.get_value()) +
                       eabase::timeval_to_microseconds(g_ru_utime.get_value()),
                       eabase::timeval_to_microseconds(g_uptime.get_value()) };
    return tp;
}
...
static TimePercent get_utime_percent(void*) {
    TimePercent tp = { eabase::timeval_to_microseconds(g_ru_utime.get_value()),
                       eabase::timeval_to_microseconds(g_uptime.get_value()) };
    return tp;
}
...
void get_work_dir(std::ostream& os, void*) {
    eabase::FilePath path;
    const bool rc = eabase::GetCurrentDirectory(&path);
