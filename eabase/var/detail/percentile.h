...
#ifndef  EABASE_VAR_DETAIL_PERCENTILE_H_
#define  EABASE_VAR_DETAIL_PERCENTILE_H_
...
#include "eabase/var/reducer.h"               // Reducer
#include "eabase/var/window.h"                // Window
#include "eabase/var/detail/combiner.h"       // AgentCombiner
...
inline unsigned long round_of_expectation(unsigned long a, unsigned long b) {
    ...
    return a / b + (eabase::fast_rand_less_than(b) < a % b);
}
...
public:
    ...
    void merge(const PercentileInterval<size2> &rhs) {
        ...
        if (_num_added + rhs._num_added <= SAMPLE_SIZE) {
            ...
            for (size_t i = _num_samples; i > num_remain; --i) {
                _samples[eabase::fast_rand_less_than(i)] = _samples[i - 1];
            }
            ...
            for (size_t i = 0; i < num_remain_from_rhs; ++i) {
                const int index = eabase::fast_rand_less_than(rhs._num_samples - i);
    ...
    void merge_with_expectation(PercentileInterval<size2>& mutable_rhs, size_t n) {
        ...
        for (size_t i = 0; i < n; ++i) {
            size_t index = eabase::fast_rand_less_than(mutable_rhs._num_samples - i);
            if (_num_samples < SAMPLE_SIZE) {
                _samples[_num_samples++] = mutable_rhs._samples[index];
            } else {
                _samples[eabase::fast_rand_less_than(_num_samples)]
    ...
    void combine_of(const Iterator& begin, const Iterator& end) {
        ...
        for (size_t i = 0; i < NUM_INTERVALS; ++i) {
            ...
            for (Iterator iter = begin; iter != end; ++iter) {
                ...
                typename eabase::add_reference<EA_TYPEOF(*(iter->_intervals[i]))>::type
...
public:
    ...
    void set_debug_name(const eabase::StringPiece& name) {
        _debug_name.assign(name.data(), name.size());
    }

private:
    EA_DISALLOW_COPY_AND_ASSIGN(Percentile);
...
#endif  // EABASE_VAR_DETAIL_PERCENTILE_H_
