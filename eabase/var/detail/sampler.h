...
#ifndef  EABASE_VAR_DETAIL_SAMPLER_H_
#define  EABASE_VAR_DETAIL_SAMPLER_H_
...
#include "eabase/utility/containers/bounded_queue.h"// BoundedQueue
#include "eabase/utility/type_traits.h"           // is_same
...
class Sampler : public eabase::LinkNode<Sampler> {
public:
    Sampler();
...
public:
    ...
    void take_sample() override {
        ...
        if ((size_t)_window_size + 1 > _q.capacity()) {
            ...
            eabase::BoundedQueue<Sample<T> > new_q(
                mem, memsize, eabase::OWNS_STORAGE);
            Sample<T> tmp;
            while (_q.pop(&tmp)) {
                new_q.push(tmp);
            }
        ...
        Sample<T> latest;
        if (eabase::is_same<InvOp, VoidOp>::value) {
        ...
        latest.time_us = eabase::gettimeofday_us();
        _q.elim_push(latest);
    }
...
private:
    R* _reducer;
    time_t _window_size;
    eabase::BoundedQueue<Sample<T> > _q;
};

}  // namespace detail
}  // namespace eabase

#endif  // EABASE_VAR_DETAIL_SAMPLER_H_
