...
#ifndef  EABASE_VAR_DETAIL_SERIES_H_
#define  EABASE_VAR_DETAIL_SERIES_H_

#include <math.h>                       // round
#include <ostream>
#include "eabase/utility/scoped_lock.h"           // BAIDU_SCOPED_LOCK
#include "eabase/utility/type_traits.h"
#include "eabase/var/vector.h"
...
template <typename T, typename Op>
struct DivideOnAddition<T, Op, typename eabase::enable_if<
                                   eabase::is_integral<T>::value>::type> {
...
template <typename T, typename Op>
struct DivideOnAddition<T, Op, typename eabase::enable_if<
                                   eabase::is_floating_point<T>::value>::type> {
    static void inplace_divide(T& obj, const Op& op, int number) {
        static ProbablyAddtition<T, Op> probably_add(op);
        if (probably_add) {
            obj /= number;
...
struct DivideOnAddition<Vector<T,N>, Op, typename eabase::enable_if<
                                             eabase::is_integral<T>::value>::type> {
...
void Series<Vector<T,N>, Op>::describe(std::ostream& os,
    ...
    eabase::StringSplitter sp(vector_names ? vector_names->c_str() : "", ',');
    os << '[';
    for (size_t j = 0; j < N; ++j) {
        ...
        os << "{\"label\":\"";
        if (sp) {
            os << eabase::StringPiece(sp.field(), sp.length());
            ++sp;
        } else {
            os << "Vector[" << j << ']';
        }
...
#endif  // EABASE_VAR_DETAIL_SERIES_H_
