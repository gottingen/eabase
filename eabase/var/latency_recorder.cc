...
const bool ALLOW_UNUSED dummy_var_latency_p1 = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_latency_p1, valid_percentile);
const bool ALLOW_UNUSED dummy_var_latency_p2 = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_latency_p2, valid_percentile);
const bool ALLOW_UNUSED dummy_var_latency_p3 = ::GFLAGS_NS::RegisterFlagValidator(
...
static int64_t double_to_random_int(double dval) {
    ...
    if (dval > ival + eabase::fast_rand_double()) {
        ival += 1;
    }
    return ival;
}
...
int LatencyRecorder::expose(const eabase::StringPiece& prefix1,
                            const eabase::StringPiece& prefix2) {
    if (prefix2.empty()) {
        LOG(ERROR) << "Parameter[prefix2] is empty";
        return -1;
    }
    eabase::StringPiece prefix = prefix2;
