...
#ifndef EABASE_VAR_MULTI_DIMENSION_INL_H_
#define EABASE_VAR_MULTI_DIMENSION_INL_H_
...
template <typename T>
inline
MultiDimension<T>::MultiDimension(const eabase::StringPiece& name,
...
template <typename T>
inline
MultiDimension<T>::MultiDimension(const eabase::StringPiece& prefix,
...
template <>
inline
size_t MultiDimension<eabase::LatencyRecorder>::dump(Dumper* dumper, const DumpOptions*) {
    std::vector<key_type> label_names;
    list_stats(&label_names);
    ...
    size_t n = 0;
    for (auto &label_name : label_names) {
        eabase::LatencyRecorder* var = get_stats_impl(label_name);
        if (!var) {
            continue;
        }
...
#endif // EABASE_VAR_MULTI_DIMENSION_INL_H_
