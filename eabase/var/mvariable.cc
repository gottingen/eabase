...
const bool ALLOW_UNUSED dummp_var_max_multi_dimension_metric_number = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_max_multi_dimension_metric_number, validator_var_max_multi_dimension_metric_number);

const bool ALLOW_UNUSED dummp_var_max_dump_multi_dimension_metric_number = ::GFLAGS_NS::RegisterFlagValidator(
...
typedef eabase::FlatMap<std::string, MVarEntry> MVarMap;
...
int MVariable::expose_impl(const eabase::StringPiece& prefix,
    ...
    if (count_exposed() > (size_t)FLAGS_var_max_multi_dimension_metric_number) {
        LOG(ERROR) << "Too many metric seen, overflow detected, max metric count:" << FLAGS_var_max_multi_dimension_metric_number;
        return -1;
    }
