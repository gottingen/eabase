...
#ifndef  EABASE_VAR_REDUCER_H_
#define  EABASE_VAR_REDUCER_H_

#include <limits>                                 // std::numeric_limits
#include "eabase/utility/logging.h"                         // LOG()
#include "eabase/utility/type_traits.h"                     // eabase::add_cr_non_integral
#include "eabase/utility/class_name.h"                      // class_name_str
#include "eabase/var/variable.h"                        // Variable
#include "eabase/var/detail/combiner.h"                 // detail::AgentCombiner
#include "eabase/var/detail/sampler.h"                  // ReducerSampler
#include "eabase/var/detail/series.h"
...
// }
// eabase::Adder<MyType> my_type_sum;
...
public:
    ...
    Reducer(typename eabase::add_cr_non_integral<T>::type identity = T(),
            const Op& op = Op(),
    ...
    Reducer& operator<<(typename eabase::add_cr_non_integral<T>::type value);
    ...
    T get_value() const {
        CHECK(!(eabase::is_same<InvOp, detail::VoidOp>::value) || _sampler == NULL)
            << "You should not call Reducer<" << eabase::class_name_str<T>()
            << ", " << eabase::class_name_str<Op>() << ">::get_value() when a"
    ...
    void describe(std::ostream& os, bool quote_string) const override {
        if (eabase::is_same<T, std::string>::value && quote_string) {
            os << '"' << get_value() << '"';
        } else {
            os << get_value();
        }
...
protected:
    int expose_impl(const eabase::StringPiece& prefix,
        ...
        if (rc == 0 &&
            _series_sampler == NULL &&
            !eabase::is_same<InvOp, detail::VoidOp>::value &&
            !eabase::is_same<T, std::string>::value &&
            FLAGS_save_series) {
...
inline Reducer<T, Op, InvOp>& Reducer<T, Op, InvOp>::operator<<(
    typename eabase::add_cr_non_integral<T>::type value) {
...
struct AddTo {
    void operator()(Tp & lhs, 
                    typename eabase::add_cr_non_integral<Tp>::type rhs) const
    { lhs += rhs; }
};
template <typename Tp>
struct MinusFrom {
...
public:
    Adder() : Base() {}
    explicit Adder(const eabase::StringPiece& name) : Base() {
        this->expose(name);
    }
    Adder(const eabase::StringPiece& prefix,
          const eabase::StringPiece& name) : Base() {
        this->expose_as(prefix, name);
    }
    ~Adder() { Variable::hide(); }
};

// eabase::Maxer<int> max_value;
// max_value << 1 << 2 << 3 << 4;
...
public:
    ...
    explicit Maxer(const eabase::StringPiece& name)
        : Base(std::numeric_limits<T>::min()) {
        this->expose(name);
    }
    Maxer(const eabase::StringPiece& prefix, const eabase::StringPiece& name)
        : Base(std::numeric_limits<T>::min()) {
        this->expose_as(prefix, name);
    }
    ~Maxer() { Variable::hide(); }
private:
    ...
    Maxer(T default_value, const eabase::StringPiece& prefix,
          const eabase::StringPiece& name)
        : Base(default_value) {
        this->expose_as(prefix, name);
    }
    Maxer(T default_value, const eabase::StringPiece& name) : Base(default_value) {
        this->expose(name);
    }
};

// eabase::Miner<int> min_value;
// min_value << 1 << 2 << 3 << 4;
...
public:
    ...
    explicit Miner(const eabase::StringPiece& name)
        : Base(std::numeric_limits<T>::max()) {
        this->expose(name);
    }
    Miner(const eabase::StringPiece& prefix, const eabase::StringPiece& name)
        : Base(std::numeric_limits<T>::max()) {
        this->expose_as(prefix, name);
    }
    ~Miner() { Variable::hide(); }
