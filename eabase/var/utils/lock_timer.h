...
#ifndef  EABASE_VAR_LOCK_TIMER_H_
#define  EABASE_VAR_LOCK_TIMER_H_

#include "eabase/utility/time.h"             // eabase::Timer
#include "eabase/utility/scoped_lock.h"      // std::lock_guard std::unique_lock
#include "eabase/utility/macros.h"           // EA_DISALLOW_COPY_AND_ASSIGN

#include "eabase/var/recorder.h"         // IntRecorder
#include "eabase/var/latency_recorder.h" // LatencyRecorder
...
//
// eabase::LatencyRecorder
//     g_mutex_contention("my_mutex_contention");
//                       // ^^^
//                       // you can replace this with a meaningful name
//
// typedef ::eabase::MutexWithLatencyRecorder<pthread_mutex_t> my_mutex_t;
//                                       // ^^^
...
// Specialize for pthread_mutex_t
    ...
    namespace detail {
        ...
        class MutexWithRecorderBase {
            EA_DISALLOW_COPY_AND_ASSIGN(MutexWithRecorderBase);
        ...
        template<typename Mutex>
        class LockGuardBase {
            EA_DISALLOW_COPY_AND_ASSIGN(LockGuardBase);
        ...
        private:
            ...
            struct TimerAndMutex {
                TimerAndMutex(Mutex &m)
                        : timer(eabase::Timer::STARTED), mutex(&m) {}
        ...
        class UniqueLockBase {
            EA_DISALLOW_COPY_AND_ASSIGN(UniqueLockBase);

        public:
            typedef Mutex mutex_type;

            explicit UniqueLockBase(mutex_type &mutex)
                    : _timer(eabase::Timer::STARTED), _lock(mutex.mutex()),
                      _mutex(&mutex) {
                _timer.stop();
            }
            ...
            UniqueLockBase(mutex_type &mutex, std::try_to_lock_t try_to_lock)
                    : _timer(eabase::Timer::STARTED), _lock(mutex.mutex(), try_to_lock), _mutex(&mutex) {
...
// Specialize lock_guard and unique_lock
    template<typename Mutex>
    class lock_guard<eabase::MutexWithRecorder<Mutex> >
            : public ::eabase::detail::
            LockGuardBase<::eabase::MutexWithRecorder<Mutex> > {
    public:
        typedef ::eabase::detail::
        LockGuardBase<eabase::MutexWithRecorder<Mutex> > Base;

        explicit lock_guard(::eabase::MutexWithRecorder<Mutex> &mutex)
                : Base(mutex) {}
    };

    template<typename Mutex>
    class lock_guard<eabase::MutexWithLatencyRecorder<Mutex> >
            : public ::eabase::detail::
            LockGuardBase<::eabase::MutexWithLatencyRecorder<Mutex> > {
    public:
        typedef ::eabase::detail::
        LockGuardBase<eabase::MutexWithLatencyRecorder<Mutex> > Base;

        explicit lock_guard(::eabase::MutexWithLatencyRecorder<Mutex> &mutex)
                : Base(mutex) {}
    };

    template<typename Mutex>
    class unique_lock<eabase::MutexWithRecorder<Mutex> >
            : public ::eabase::detail::
            UniqueLockBase<::eabase::MutexWithRecorder<Mutex> > {
    public:
        typedef ::eabase::detail::
        UniqueLockBase<::eabase::MutexWithRecorder<Mutex> > Base;
    ...
    template<typename Mutex>
    class unique_lock<eabase::MutexWithLatencyRecorder<Mutex> >
            : public ::eabase::detail::
            UniqueLockBase<::eabase::MutexWithLatencyRecorder<Mutex> > {
    public:
        typedef ::eabase::detail::
        UniqueLockBase<::eabase::MutexWithLatencyRecorder<Mutex> > Base;
        typedef typename Base::mutex_type mutex_type;
...
#endif  // EABASE_VAR_LOCK_TIMER_H_
