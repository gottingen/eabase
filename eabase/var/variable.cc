...
#include "eabase/utility/string_splitter.h"               // eabase::StringSplitter
#include "eabase/utility/errno.h"                         // berror
#include "eabase/utility/time.h"                          // milliseconds_from_now
#include "eabase/utility/file_util.h"                     // eabase::FilePath
...
const bool ALLOW_UNUSED dummy_var_abort_on_same_name = ::GFLAGS_NS::RegisterFlagValidator(
...
typedef eabase::FlatMap<std::string, VarEntry> VarMap;
...
int Variable::expose_impl(const eabase::StringPiece& prefix,
    ...
    if (!prefix.empty()) {
        ...
        if (!_name.empty() && eabase::back_char(_name) != '_') {
            _name.push_back('_');
        }
    }
...
public:
    ...
    int sync() override;
    void reset();
    eabase::StringPiece data() {
        return eabase::StringPiece(pbase(), pptr() - pbase());
    }
...
public:
    WildcardMatcher(const std::string& wildcards,
        ...
        for (eabase::StringMultiSplitter sp(wildcards.c_str(), ",;");
             sp != NULL; ++sp) {
    ...
    if (white_matcher.wildcards().empty() &&
        ...
        for (std::set<std::string>::const_iterator
            ...
            if (!black_matcher.match(name)) {
                if (eabase::Variable::describe_exposed(
        ...
        eabase::Variable::list_exposed(&varnames, opt.display_filter);
...
std::string read_command_name() {
    ...
    if (command_name.size() >= 2UL && command_name[0] == '(' &&
        eabase::back_char(command_name) == ')') {
        // remove parenthesis.
        to_underscored_name(&s,
                            eabase::StringPiece(command_name.data() + 1,
...
public:
    FileDumper(const std::string& filename, eabase::StringPiece s/*prefix*/)
...
protected:
    bool dump_impl(const std::string& name, const eabase::StringPiece& desc, const std::string& separator) {
        if (_fp == NULL) {
            eabase::File::Error error;
            eabase::FilePath dir = eabase::FilePath(_filename).DirName();
...
public:
    CommonFileDumper(const std::string& filename, eabase::StringPiece prefix)
        : FileDumper(filename, prefix)
        , _separator(":") {}
    bool dump(const std::string& name, const eabase::StringPiece& desc) {
...
public:
    PrometheusFileDumper(const std::string& filename, eabase::StringPiece prefix)
        : FileDumper(filename, prefix)
        , _separator(" ") {}
    bool dump(const std::string& name, const eabase::StringPiece& desc) {
        return dump_impl(name, desc, _separator);
...
public:
    FileDumperGroup(std::string tabs, std::string filename, 
                     eabase::StringPiece s/*prefix*/) {
        eabase::FilePath path(filename);
        if (path.FinalExtension() == ".data") {
            // .data will be appended later
            path = path.RemoveFinalExtension();
        }

        for (eabase::KeyValuePairsSplitter sp(tabs, ';', '='); sp; ++sp) {
    ...
    bool dump(const std::string& name, const eabase::StringPiece& desc) override {
...
#if !defined(VAR_NOT_LINK_DEFAULT_VARIABLES)
...
static void* dumping_thread(void*) {
    ...
    eabase::PlatformThread::SetName("var_dumper");
    ...
    while (1) {
        ...
        if (!GFLAGS_NS::GetCommandLineOption("var_dump_file", &filename)) {
            LOG(ERROR) << "Fail to get gflag var_dump_file";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("var_dump_include",
                                          &options.white_wildcards)) {
            LOG(ERROR) << "Fail to get gflag var_dump_include";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("var_dump_exclude",
                                          &options.black_wildcards)) {
            LOG(ERROR) << "Fail to get gflag var_dump_exclude";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("var_dump_prefix", &prefix)) {
            LOG(ERROR) << "Fail to get gflag var_dump_prefix";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("var_dump_tabs", &tabs)) {
            LOG(ERROR) << "Fail to get gflags var_dump_tabs";
            return NULL;
        }

        // We can't access string flags directly because it's thread-unsafe.
        if (!GFLAGS_NS::GetCommandLineOption("mvar_dump_file", &mvar_filename)) {
            LOG(ERROR) << "Fail to get gflag mvar_dump_file";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("mvar_dump_prefix", &mvar_prefix)) {
            LOG(ERROR) << "Fail to get gflag mvar_dump_prefix";
            return NULL;
        }
        if (!GFLAGS_NS::GetCommandLineOption("mvar_dump_format", &mvar_format)) {
        ...
        timespec deadline = eabase::milliseconds_from_now(cond_sleep_ms);
...
const bool ALLOW_UNUSED dummy_var_dump = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump, validate_var_dump);

// validators (to make these gflags reloadable in eabase)
static bool validate_var_dump_interval(const char*, int32_t v) {
    ...
    if (v < 1) {
        LOG(ERROR) << "Invalid var_dump_interval=" << v;
        return false;
    }
    return true;
}
const bool ALLOW_UNUSED dummy_var_dump_interval = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_interval, validate_var_dump_interval);

static bool validate_var_log_dumpped(const char *, bool) { return true; }
const bool ALLOW_UNUSED dummy_var_log_dumpped = ::GFLAGS_NS::RegisterFlagValidator(
...
const bool ALLOW_UNUSED dummy_var_dump_file = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_file, wakeup_dumping_thread);
const bool ALLOW_UNUSED dummy_var_dump_filter = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_include, wakeup_dumping_thread);
const bool ALLOW_UNUSED dummy_var_dump_exclude = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_exclude, wakeup_dumping_thread);
const bool ALLOW_UNUSED dummy_var_dump_prefix = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_prefix, wakeup_dumping_thread);
const bool ALLOW_UNUSED dummy_var_dump_tabs = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_var_dump_tabs, wakeup_dumping_thread);

const bool ALLOW_UNUSED dummy_mvar_dump = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_mvar_dump, validate_var_dump);
const bool ALLOW_UNUSED dummy_mvar_dump_prefix = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_mvar_dump_prefix, wakeup_dumping_thread);
const bool ALLOW_UNUSED dump_mvar_dump_file = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_mvar_dump_file, wakeup_dumping_thread);

static bool validate_mvar_dump_format(const char*, const std::string& format) {
    if (format != "common"
        && format != "prometheus") {
        LOG(ERROR) << "Invalid mvar_dump_format=" << format;
        return false;
    }
...
const bool ALLOW_UNUSED dummy_mvar_dump_format = ::GFLAGS_NS::RegisterFlagValidator(
    &FLAGS_mvar_dump_format, validate_mvar_dump_format);

void to_underscored_name(std::string* name, const eabase::StringPiece& src) {
    name->reserve(name->size() + src.size() + 8/*just guess*/);
    for (const char* p = src.data(); p != src.data() + src.size(); ++p) {
        if (isalpha(*p)) {
            if (*p < 'a') { // upper cases
                if (p != src.data() && !isupper(p[-1]) &&
                    eabase::back_char(*name) != '_') {
                    name->push_back('_');
                }
            ...
            name->push_back(*p);
        } else if (name->empty() || eabase::back_char(*name) != '_') {
            name->push_back('_');
        }
    }
}
