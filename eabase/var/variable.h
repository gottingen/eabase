...
#ifndef  EABASE_VAR_VARIABLE_H_
#define  EABASE_VAR_VARIABLE_H_
...
public:
    ...
    virtual bool dump(const std::string& name,
                      const eabase::StringPiece& description) = 0;
...
public:
    ...
    int expose(const eabase::StringPiece& name,
        ...
        return expose_impl(eabase::StringPiece(), name, display_filter);
    }
...
protected:
    virtual int expose_impl(const eabase::StringPiece& prefix,
                            const eabase::StringPiece& name,
                            DisplayFilter display_filter);
...
private:
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(Variable);
};
...
//   RPCTest         -> rpctest
//   HELLO           -> hello
void to_underscored_name(std::string* out, const eabase::StringPiece& name);
...
inline ostream& operator<<(ostream &os, const ::eabase::Variable &var) {
