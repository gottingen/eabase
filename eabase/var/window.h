...
#ifndef  EABASE_VAR_WINDOW_H_
#define  EABASE_VAR_WINDOW_H_
...
public:
    ...
    void describe(std::ostream& os, bool quote_string) const override {
        if (eabase::is_same<value_type, std::string>::value && quote_string) {
            os << '"' << get_value() << '"';
        } else {
            os << get_value();
        }
...
public:
    ...
    Window(const eabase::StringPiece& name, R* var, time_t window_size)
        : Base(var, window_size) {
        this->expose(name);
    }
    Window(const eabase::StringPiece& prefix,
           const eabase::StringPiece& name, R* var, time_t window_size)
        : Base(var, window_size) {
...
public:
    ...
    PerSecond(const eabase::StringPiece& name, R* var) : Base(var, -1) {
        this->expose(name);
    }
    PerSecond(const eabase::StringPiece& name, R* var, time_t window_size)
        : Base(var, window_size) {
        this->expose(name);
    }
    PerSecond(const eabase::StringPiece& prefix,
              const eabase::StringPiece& name, R* var)
        : Base(var, -1) {
        this->expose_as(prefix, name);
    }
    PerSecond(const eabase::StringPiece& prefix,
    ...
    value_type get_value(time_t window_size) const override {
        ...
        if (eabase::is_floating_point<value_type>::value) {
...
public:
    typedef R var_type;
    typedef eabase::Window<var_type > window_type;
    typedef typename R::value_type value_type;
    struct WindowExVar {
...
public:
    typedef R var_type;
    typedef eabase::PerSecond<var_type > window_type;
    typedef typename R::value_type value_type;
    struct WindowExVar {
        WindowExVar(time_t window_size) : window(&var, window_size) {}
        var_type var;
...
public:
    ...
    WindowEx(const eabase::StringPiece& name) : Base(window_size) {
        this->expose(name);
    }

    WindowEx(const eabase::StringPiece& prefix,
             const eabase::StringPiece& name)
        : Base(window_size) {
        this->expose_as(prefix, name);
    }
...
public:
    ...
    PerSecondEx(const eabase::StringPiece& name) : Base(window_size) {
        this->expose(name);
    }

    PerSecondEx(const eabase::StringPiece& prefix,
                const eabase::StringPiece& name)
        : Base(window_size) {
        this->expose_as(prefix, name);
    }
