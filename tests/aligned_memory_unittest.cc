...
TEST(AlignedMemoryTest, DynamicAllocation) {
  void* p = eabase::AlignedAlloc(8, 8);
  EXPECT_TRUE(p);
  EXPECT_ALIGNED(p, 8);
  eabase::AlignedFree(p);

  p = eabase::AlignedAlloc(8, 16);
  EXPECT_TRUE(p);
  EXPECT_ALIGNED(p, 16);
  eabase::AlignedFree(p);

  p = eabase::AlignedAlloc(8, 256);
  EXPECT_TRUE(p);
  EXPECT_ALIGNED(p, 256);
  eabase::AlignedFree(p);

  p = eabase::AlignedAlloc(8, 4096);
  EXPECT_TRUE(p);
...
TEST(AlignedMemoryTest, ScopedDynamicAllocation) {
  scoped_ptr<float, eabase::AlignedFreeDeleter> p(
      static_cast<float*>(eabase::AlignedAlloc(8, 8)));
  EXPECT_TRUE(p.get());
