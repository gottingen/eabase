...
protected:
    BaiduThreadLocalTest(){
        if (!register_check) {
            register_check = true;
            eabase::thread_atexit(check_global_variable);
        }
    };
...
void* yell(void*) {
    YellObj* p = eabase::get_thread_local<YellObj>();
    EXPECT_TRUE(p);
    EXPECT_EQ(2, YellObj::nc);
    EXPECT_EQ(0, YellObj::nd);
    EXPECT_EQ(p, eabase::get_thread_local<YellObj>());
    EXPECT_EQ(2, YellObj::nc);
    EXPECT_EQ(0, YellObj::nd);
    return NULL;
}
...
TEST_F(BaiduThreadLocalTest, get_thread_local) {
    ...
    ASSERT_EQ(p, eabase::get_thread_local<YellObj>());
    ASSERT_EQ(1, YellObj::nc);
    ASSERT_EQ(0, YellObj::nd);
    pthread_t th;
...
void* proc_dummy(void* arg) {
    ...
    *p = true;
    ...
    eabase::thread_atexit(delete_dummy, deleted + (p - processed));
    return NULL;
}

TEST_F(BaiduThreadLocalTest, sanity) {
    errno = 0;
    ASSERT_EQ(-1, eabase::thread_atexit(NULL));
...
TEST_F(BaiduThreadLocalTest, call_order_and_cancel) {
    eabase::thread_atexit_cancel(NULL);
    eabase::thread_atexit_cancel(NULL, NULL);

    ASSERT_EQ(0, eabase::thread_atexit(check_result));

    ASSERT_EQ(0, eabase::thread_atexit(fun1));
    ASSERT_EQ(0, eabase::thread_atexit(fun1));
    ASSERT_EQ(0, eabase::thread_atexit(fun2));
    ASSERT_EQ(0, eabase::thread_atexit(fun3, (void*)1));
    ASSERT_EQ(0, eabase::thread_atexit(fun3, (void*)1));
    ASSERT_EQ(0, eabase::thread_atexit(fun3, (void*)2));
    ASSERT_EQ(0, eabase::thread_atexit(fun4, NULL));

    eabase::thread_atexit_cancel(NULL);
    eabase::thread_atexit_cancel(NULL, NULL);
    eabase::thread_atexit_cancel(fun1);
    eabase::thread_atexit_cancel(fun3, NULL);
    eabase::thread_atexit_cancel(fun3, (void*)1);
}
