...
TEST(BaiduTimeTest, diff_between_gettimeofday_and_REALTIME) {
    long t1 = eabase::gettimeofday_us();
    timespec time;
    clock_gettime(CLOCK_REALTIME, &time);
    long t2 = eabase::timespec_to_microseconds(time);
...
TEST(BaiduTimeTest, timer_auto_start) {
    eabase::Timer t(eabase::Timer::STARTED);
    usleep(100);
    t.stop();
