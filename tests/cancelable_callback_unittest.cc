...
#include "eabase/utility/cancelable_callback.h"

#include "eabase/utility/bind.h"
#include "eabase/utility/bind_helpers.h"
#include "eabase/utility/memory/ref_counted.h"
...
TEST(CancelableCallbackTest, Cancel) {
  ...
  CancelableClosure cancelable(
      eabase::Bind(&Increment, eabase::Unretained(&count)));

  eabase::Closure callback = cancelable.callback();
  callback.Run();
  EXPECT_EQ(1, count);
...
TEST(CancelableCallbackTest, MultipleCancel) {
  ...
  eabase::Closure callback1 = cancelable.callback();
  eabase::Closure callback2 = cancelable.callback();
  cancelable.Cancel();
  ...
  eabase::Closure callback3 = cancelable.callback();
...
TEST(CancelableCallbackTest, CancelDropsCallback) {
  ...
  CancelableClosure cancelable(eabase::Bind(RefCountedParam, ref_counted));
...
TEST(CancelableCallbackTest, Reset) {
  ...
  cancelable.Reset(
      eabase::Bind(&IncrementBy, eabase::Unretained(&count), 3));
  EXPECT_FALSE(cancelable.IsCancelled());
...
TEST(CancelableCallbackTest, IsNull) {
  ...
  int count = 0;
  cancelable.Reset(eabase::Bind(&Increment,
                              eabase::Unretained(&count)));
  EXPECT_FALSE(cancelable.IsCancelled());
