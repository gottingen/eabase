...
#include "eabase/utility/debug/crash_logging.h"
...
class CrashLoggingTest : public testing::Test {
 public:
  virtual void SetUp() {
    ...
    eabase::debug::SetCrashKeyReportingFunctions(
        &CrashLoggingTest::SetKeyValue,
        &CrashLoggingTest::ClearKeyValue);
  }

  virtual void TearDown() {
    eabase::debug::ResetCrashLoggingForTesting();
 ...
 private:
  static void SetKeyValue(const eabase::StringPiece& key,
                          const eabase::StringPiece& value) {
    (*key_values_)[key.as_string()] = value.as_string();
  }

  static void ClearKeyValue(const eabase::StringPiece& key) {
...
TEST_F(CrashLoggingTest, SetClearSingle) {
  ...
  eabase::debug::CrashKey keys[] = { { kTestKey, 255 } };
  eabase::debug::InitCrashKeys(keys, arraysize(keys), 255);

  eabase::debug::SetCrashKeyValue(kTestKey, "value");
  EXPECT_EQ("value", (*key_values_)[kTestKey]);

  eabase::debug::ClearCrashKey(kTestKey);
...
TEST_F(CrashLoggingTest, SetChunked) {
  ...
  eabase::debug::CrashKey keys[] = { { kTestKey, 15 } };
  eabase::debug::InitCrashKeys(keys, arraysize(keys), 5);

  std::map<std::string, std::string>& values = *key_values_;

  // Fill only the first chunk.
  eabase::debug::SetCrashKeyValue(kTestKey, "foo");
  ...
  eabase::debug::SetCrashKeyValue(kTestKey, "five four three two");
  ...
  eabase::debug::SetCrashKeyValue(kTestKey, "five four three two");
  eabase::debug::SetCrashKeyValue(kTestKey, "allays");
...
TEST_F(CrashLoggingTest, ScopedCrashKey) {
  ...
  EXPECT_TRUE(key_values_->end() == key_values_->find(kTestKey));
  {
    eabase::debug::ScopedCrashKey scoped_crash_key(kTestKey, "value");
    EXPECT_EQ("value", (*key_values_)[kTestKey]);
...
TEST_F(CrashLoggingTest, InitSize) {
  eabase::debug::CrashKey keys[] = {
    { "chunked-3", 15 },
    { "single", 5 },
    { "chunked-6", 30 },
  };

  size_t num_keys = eabase::debug::InitCrashKeys(keys, arraysize(keys), 5);

  EXPECT_EQ(10u, num_keys);

  EXPECT_TRUE(eabase::debug::LookupCrashKey("chunked-3"));
  EXPECT_TRUE(eabase::debug::LookupCrashKey("single"));
  EXPECT_TRUE(eabase::debug::LookupCrashKey("chunked-6"));
  EXPECT_FALSE(eabase::debug::LookupCrashKey("chunked-6-4"));
}

TEST_F(CrashLoggingTest, ChunkValue) {
  using eabase::debug::ChunkCrashKeyValue;

  // Test truncation.
  eabase::debug::CrashKey key = { "chunky", 10 };
...
TEST_F(CrashLoggingTest, ChunkRounding) {
  ...
  // not 2.
  eabase::debug::CrashKey key = { "round", 12 };
  EXPECT_EQ(3u, eabase::debug::InitCrashKeys(&key, 1, 5));
}
