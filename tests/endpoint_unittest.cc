...
#include "eabase/utility/endpoint.h"
#include "eabase/utility/logging.h"
#include "eabase/utility/containers/flat_map.h"
#include "eabase/utility/details/extended_endpoint.hpp"

namespace {

using eabase::details::ExtendedEndPoint;

TEST(EndPointTest, comparisons) {
    eabase::EndPoint p1(eabase::int2ip(1234), 5678);
    eabase::EndPoint p2 = p1;
    ...
    --p2.port;
    p2.ip = eabase::int2ip(eabase::ip2int(p2.ip)-1);
    ASSERT_TRUE(p1 != p2 && !(p1 == p2));
...
TEST(EndPointTest, ip_t) {
    ...
    eabase::ip_t ip0;
    ASSERT_EQ(0, eabase::str2ip("1.1.1.1", &ip0));
    ASSERT_STREQ("1.1.1.1", eabase::ip2str(ip0).c_str());
    ASSERT_EQ(-1, eabase::str2ip("301.1.1.1", &ip0));
    ASSERT_EQ(-1, eabase::str2ip("1.-1.1.1", &ip0));
    ASSERT_EQ(-1, eabase::str2ip("1.1.-101.1", &ip0));
    ASSERT_STREQ("1.0.0.0", eabase::ip2str(eabase::int2ip(1)).c_str());

    eabase::ip_t ip1, ip2, ip3;
    ASSERT_EQ(0, eabase::str2ip("192.168.0.1", &ip1));
    ASSERT_EQ(0, eabase::str2ip("192.168.0.2", &ip2));
    ip3 = ip1;
    ASSERT_LT(ip1, ip2);
    ASSERT_LE(ip1, ip2);
    ASSERT_GT(ip2, ip1);
...
TEST(EndPointTest, show_local_info) {
    LOG(INFO) << "my_ip is " << eabase::my_ip() << std::endl
              << "my_ip_cstr is " << eabase::my_ip_cstr() << std::endl
              << "my_hostname is " << eabase::my_hostname();
}

TEST(EndPointTest, endpoint) {
    eabase::EndPoint p1;
    ASSERT_EQ(eabase::IP_ANY, p1.ip);
    ASSERT_EQ(0, p1.port);
    
    eabase::EndPoint p2(eabase::IP_NONE, -1);
    ASSERT_EQ(eabase::IP_NONE, p2.ip);
    ASSERT_EQ(-1, p2.port);

    eabase::EndPoint p3;
    ASSERT_EQ(-1, eabase::str2endpoint(" 127.0.0.1:-1", &p3));
    ASSERT_EQ(-1, eabase::str2endpoint(" 127.0.0.1:65536", &p3));
    ASSERT_EQ(0, eabase::str2endpoint(" 127.0.0.1:65535", &p3));
    ASSERT_EQ(0, eabase::str2endpoint(" 127.0.0.1:0", &p3));

    eabase::EndPoint p4;
    ASSERT_EQ(0, eabase::str2endpoint(" 127.0.0.1: 289 ", &p4));
    ASSERT_STREQ("127.0.0.1", eabase::ip2str(p4.ip).c_str());
    ...
    ASSERT_STREQ("10.23.249.73", eabase::ip2str(p6.ip).c_str());
    ASSERT_EQ(289, p6.port);
#endif
}

TEST(EndPointTest, hash_table) {
    eabase::hash_map<eabase::EndPoint, int> m;
    eabase::EndPoint ep1(eabase::IP_ANY, 123);
    eabase::EndPoint ep2(eabase::IP_ANY, 456);
    ++m[ep1];
...
TEST(EndPointTest, flat_map) {
    eabase::FlatMap<eabase::EndPoint, int> m;
    ASSERT_EQ(0, m.init(1024));
    uint32_t port = 8088;

    eabase::EndPoint ep1(eabase::IP_ANY, port);
    eabase::EndPoint ep2(eabase::IP_ANY, port);
    ++m[ep1];
    ++m[ep2];
    ASSERT_EQ(1u, m.size());

    eabase::ip_t ip_addr;
    eabase::str2ip("10.10.10.10", &ip_addr);
    int ip = eabase::ip2int(ip_addr);

    for (int i = 0; i < 1023; ++i) {
        eabase::EndPoint ep(eabase::int2ip(++ip), port);
        ++m[ep];
    }

    eabase::BucketInfo info = m.bucket_info();
...
static void test_listen_connect(const std::string& server_addr, const std::string& exp_client_addr) {
    eabase::EndPoint point;
    ASSERT_EQ(0, eabase::str2endpoint(server_addr.c_str(), &point));
    ASSERT_EQ(server_addr, eabase::endpoint2str(point).c_str());

    int listen_fd = eabase::tcp_listen(point);
    ASSERT_GT(listen_fd, 0);
    pthread_t pid;
    pthread_create(&pid, NULL, server_proc, (void*)(int64_t)listen_fd);

    int fd = eabase::tcp_connect(point, NULL);
    ASSERT_GT(fd, 0);

    eabase::EndPoint point2;
    ASSERT_EQ(0, eabase::get_local_side(fd, &point2));

    std::string s = eabase::endpoint2str(point2).c_str();
    if (eabase::get_endpoint_type(point2) == AF_UNIX) {
        ASSERT_EQ(exp_client_addr, s);
    } else {
        ASSERT_EQ(exp_client_addr, s.substr(0, exp_client_addr.size()));
    }
    ASSERT_EQ(0, eabase::get_remote_side(fd, &point2));
    ASSERT_EQ(server_addr, eabase::endpoint2str(point2).c_str());
    close(fd);
...
static void test_parse_and_serialize(const std::string& instr, const std::string& outstr) {
    eabase::EndPoint ep;
    ASSERT_EQ(0, eabase::str2endpoint(instr.c_str(), &ep));
    eabase::EndPointStr s = eabase::endpoint2str(ep);
...
TEST(EndPointTest, ipv6) {
    ...
    eabase::EndPoint ep;
    ASSERT_EQ(-1, eabase::str2endpoint("[2001:db8:1:2:3:ab9:c0a8:102]", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[2001:db8:1:2:3:ab9:c0a8:102]#654321", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("ipv6:2001:db8:1:2:3:ab9:c0a8:102", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[::1", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[]:80", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[]", &ep));
    ASSERT_EQ(-1, eabase::str2endpoint("[]:", &ep));
}
...
TEST(EndPointTest, unix_socket) {
    ...
    eabase::EndPoint point;
    ASSERT_EQ(-1, eabase::str2endpoint("", &point));
    ASSERT_EQ(-1, eabase::str2endpoint("a.sock", &point));
    ASSERT_EQ(-1, eabase::str2endpoint("unix:", &point));
    ASSERT_EQ(-1, eabase::str2endpoint(" unix: ", &point));
    ASSERT_EQ(0, eabase::str2endpoint("unix://a.sock", 123, &point));
    ASSERT_EQ(std::string("unix://a.sock"), eabase::endpoint2str(point).c_str());

    std::string long_path = "unix:";
    long_path.append(sizeof(sockaddr_un::sun_path) - 1, 'a');
    ASSERT_EQ(0, eabase::str2endpoint(long_path.c_str(), &point));
    ASSERT_EQ(long_path, eabase::endpoint2str(point).c_str());
    long_path.push_back('a');
    ASSERT_EQ(-1, eabase::str2endpoint(long_path.c_str(), &point));
    char buf[128] = {0}; // braft use this size of buffer
    size_t ret = snprintf(buf, sizeof(buf), "%s:%d", eabase::endpoint2str(point).c_str(), INT_MAX);
...
TEST(EndPointTest, original_endpoint) {
    ...
    ASSERT_EQ(0, eabase::str2endpoint("1.2.3.4:5678", &ep));
    ASSERT_FALSE(ExtendedEndPoint::is_extended(ep));
    ASSERT_EQ(NULL, ExtendedEndPoint::address(ep));

    // ctor & dtor
    {
        eabase::EndPoint ep2(ep);
        ASSERT_FALSE(ExtendedEndPoint::is_extended(ep));
...
TEST(EndPointTest, extended_endpoint) {
    eabase::EndPoint ep;
    ASSERT_EQ(0, eabase::str2endpoint("unix:sock.file", &ep));
    ...
    ASSERT_EQ(0, eabase::str2endpoint("[::1]:2233", &ep2));
...
TEST(EndPointTest, endpoint_compare) {
    eabase::EndPoint ep1, ep2, ep3;

    ASSERT_EQ(0, eabase::str2endpoint("127.0.0.1:8080", &ep1));
    ASSERT_EQ(0, eabase::str2endpoint("127.0.0.1:8080", &ep2));
    ASSERT_EQ(0, eabase::str2endpoint("127.0.0.3:8080", &ep3));
    ASSERT_EQ(ep1, ep2);
    ASSERT_NE(ep1, ep3);

    ASSERT_EQ(0, eabase::str2endpoint("unix:sock1.file", &ep1));
    ASSERT_EQ(0, eabase::str2endpoint("unix:sock1.file", &ep2));
    ASSERT_EQ(0, eabase::str2endpoint("unix:sock3.file", &ep3));
    ASSERT_EQ(ep1, ep2);
    ASSERT_NE(ep1, ep3);

    ASSERT_EQ(0, eabase::str2endpoint("[::1]:2233", &ep1));
    ASSERT_EQ(0, eabase::str2endpoint("[::1]:2233", &ep2));
    ASSERT_EQ(0, eabase::str2endpoint("[::3]:2233", &ep3));
    ASSERT_EQ(ep1, ep2);
    ASSERT_NE(ep1, ep3);
}
...
TEST(EndPointTest, endpoint_sockaddr_conv_ipv4) {
    eabase::EndPoint ep;
    ASSERT_EQ(0, eabase::str2endpoint("1.2.3.4:8086", &ep));
    ...
    ASSERT_EQ(0, eabase::endpoint2sockaddr(ep, &ss));
    ...
    ASSERT_EQ(0, eabase::endpoint2sockaddr(ep, &ss2, &ss2_size));
    ASSERT_EQ(ss2_size, sizeof(*in4));
    ASSERT_EQ(0, memcmp(&ss2, &ss, sizeof(ss)));

    eabase::EndPoint ep2;
    ASSERT_EQ(0, eabase::sockaddr2endpoint(&ss, sizeof(*in4), &ep2));
    ASSERT_EQ(ep2, ep);

    ASSERT_EQ(AF_INET, eabase::get_endpoint_type(ep));
}

TEST(EndPointTest, endpoint_sockaddr_conv_ipv6) {
    eabase::EndPoint ep;
    ASSERT_EQ(0, eabase::str2endpoint("[::1]:8086", &ep));
    ...
    eabase::EndPoint ep2;
    ASSERT_EQ(0, eabase::sockaddr2endpoint(&ss, sizeof(*sa6), &ep2));
    ASSERT_STREQ("[::1]:8086", eabase::endpoint2str(ep2).c_str());

    ASSERT_EQ(AF_INET6, eabase::get_endpoint_type(ep));
}
...
TEST(EndPointTest, endpoint_sockaddr_conv_unix) {
    ...
    eabase::EndPoint ep2;
    ASSERT_EQ(0, eabase::sockaddr2endpoint(&ss, sizeof(sa_family_t) + strlen(un->sun_path) + 1, &ep2));
    ASSERT_STREQ("unix:sock.file", eabase::endpoint2str(ep2).c_str());

    ASSERT_EQ(AF_UNIX, eabase::get_endpoint_type(ep));
}
...
void concurrent_proc(void* p) {
    for (int i = 0; i < 10000; ++i) {
        ...
        ASSERT_EQ(0, eabase::str2endpoint(str.c_str(), &ep));
        ASSERT_EQ(str, eabase::endpoint2str(ep).c_str());
