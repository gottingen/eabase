...
typedef std::unique_lock<eabase::FiberMutex> Lock;
typedef eabase::ConditionVariable Condition;
eabase::FiberMutex      g_mutex;
Condition           g_cond;
...
struct ProducerStat {
    ...
    eabase::Adder<int> wait_count;
    eabase::Adder<int> wait_timeout_count;
    eabase::Adder<int> wait_success_count;
};
...
TEST(FiberCondBugTest, test_bug) {
    ...
    for (int i = 0; i < PRODUCER_NUM; i++) {
        fiber_start_lazy(&tids[i], NULL, produce_func, (void*)(int64_t)i);
    }
    fiber_t tid;
    fiber_start_lazy(&tid, NULL, consume_func, NULL);
