...
#include "eabase/fiber/condition_variable.h"
#include "eabase/fiber/stack.h"
...
void* signaler(void* void_arg) {
    Arg* a = (Arg*)void_arg;
    signal_start_time = eabase::gettimeofday_us();
    while (!stop) {
...
void* waiter(void* void_arg) {
    ...
    while (!stop) {
        ...
        wake_time.push_back(eabase::gettimeofday_us());
    }
...
TEST(CondTest, sanity) {
    ...
    for (size_t i = 0; i < NW; ++i) {
        ASSERT_EQ(0, fiber_start(&wth[i], NULL, waiter, &a));
    }
    
    fiber_t sth;
    ASSERT_EQ(0, fiber_start(&sth, NULL, signaler, &a));
...
struct WrapperArg {
    eabase::FiberMutex mutex;
    eabase::ConditionVariable cond;
};
...
void* cv_mutex_waiter(void* void_arg) {
    WrapperArg* a = (WrapperArg*)void_arg;
    std::unique_lock<eabase::FiberMutex> lck(a->mutex);
    while (!stop) {
        a->cond.wait(lck);
    }
    return NULL;
...
#define pthread_create fiber_start
#endif
...
protected:
    ...
    int wait(int old_signal) {
        std::unique_lock<eabase::FiberMutex> lck(_m);
...
private:
    eabase::FiberMutex _m;
    eabase::ConditionVariable _c;
    int _signal;
};
...
struct PingPongArg {
    ...
    Signal sig1;
    Signal sig2;
    eabase::atomic<int> nthread;
    eabase::atomic<long> total_count;
};
...
TEST(CondTest, ping_pong) {
    ...
    for (int i = 0; i < 2; ++i) {
        ASSERT_EQ(0, fiber_start(&threads[i], NULL, ping_pong_thread, &arg));
    }
    usleep(1000 * 1000);
    arg.stopped = true;
    arg.sig1.notify();
...
struct BroadcastArg {
    eabase::ConditionVariable wait_cond;
    eabase::ConditionVariable broadcast_cond;
    eabase::FiberMutex mutex;
    int nwaiter;
    int cur_waiter;
    int rounds;
...
void* wait_thread(void* arg) {
    ...
    std::unique_lock<eabase::FiberMutex> lck(ba->mutex);
    while (ba->rounds > 0) {
...
TEST(CondTest, mixed_usage) {
    ...
    for (int i = 0; i < NTHREADS; ++i) {
        ASSERT_EQ(0, fiber_start(&normal_threads[i], NULL, wait_thread, &ba));
    }
...
static void launch_many_fibers() {
    ...
    c.Init();
    eabase::Timer tm;
    fiber_start(&tid, &FIBER_ATTR_PTHREAD, wait_cond_thread, &c);
    std::vector<fiber_t> tids;
    tids.reserve(32768);
    tm.start();
    for (size_t i = 0; i < 32768; ++i) {
        fiber_t t0;
        ASSERT_EQ(0, fiber_start_lazy(&t0, NULL, usleep_thread, NULL));
        tids.push_back(t0);
    }
    tm.stop();
...
TEST(CondTest, too_many_fibers_from_fiber) {
    fiber_t th;
    ASSERT_EQ(0, fiber_start(&th, NULL, run_launch_many_fibers, NULL));
    fiber_join(th, NULL);
}
} // namespace
