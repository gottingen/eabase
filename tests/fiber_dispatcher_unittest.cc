...
struct EA_CACHELINE_ALIGNMENT SocketMeta {
    int fd;
    int epfd;
    eabase::atomic<int> req;
    char* buf;
    size_t buf_cap;
    size_t bytes;
    size_t times;
...
void* process_thread(void* arg) {
    ...
    do {
        ...
        if (m->req.exchange(0, eabase::memory_order_release) == 1) {
            // no events during reading.
            break;
        }
        if (m->req.fetch_add(1, eabase::memory_order_relaxed) != 0) {
...
#endif
            if (m->req.fetch_add(1, eabase::memory_order_acquire) == 0) {
                fiber_t th;
                fiber_start(
...
TEST(DispatcherTest, dispatch_tasks) {
    ...
    for (size_t i = 0; i < NCLIENT; ++i) {
        ...
        m->bytes = 0;
        m->times = 0;
        ASSERT_EQ(0, eabase::make_non_blocking(m->fd));
        sm[i] = m;
    ...
    for (size_t i = 0; i < NEPOLL; ++i) {
        ...
        ASSERT_EQ(0, fiber_start_lazy(&eth[i], NULL, epoll_thread, m));
#else
