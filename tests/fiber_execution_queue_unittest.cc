...
#include <eabase/fiber/execution_queue.h>
...
#include "eabase/utility/gperftools_profiler.h"
...
struct LongIntTask {
    ...
    LongIntTask(long v)
        : value(v), event(NULL)
    {}
    LongIntTask(long v, eabase::CountdownEvent* e)
        : value(v), event(e)
    {}
    LongIntTask() : value(0), event(NULL) {}
};

int add(void* meta, eabase::TaskIterator<LongIntTask> &iter) {
...
void test_single_thread(bool use_pthread) {
    ...
    stopped = false;
    eabase::ExecutionQueueId<LongIntTask> queue_id;
    eabase::ExecutionQueueOptions options;
    ...
    ASSERT_EQ(0, eabase::execution_queue_start(&queue_id, &options,
                                                add, &result));
    for (int i = 0; i < 100; ++i) {
        expected_result += i;
        ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, i));
    }
    LOG(INFO) << "stop";
    ASSERT_EQ(0, eabase::execution_queue_stop(queue_id));
    ASSERT_NE(0, eabase::execution_queue_execute(queue_id, 0));
    ASSERT_EQ(0, eabase::execution_queue_join(queue_id));
...
public:
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(RValue);
...
int add(void* meta, eabase::TaskIterator<RValue> &iter) {
    stopped = iter.is_queue_stopped();
    int* result = (int*)meta;
    for (; iter; ++iter) {
        *result += iter->value();
...
void test_rvalue(bool use_pthread) {
    ...
    int64_t expected_result = 0;
    stopped = false;
    eabase::ExecutionQueueId<RValue> queue_id;
    eabase::ExecutionQueueOptions options;
    options.use_pthread = use_pthread;
    if (options.use_pthread) {
        LOG(INFO) << "================ pthread ================";
    ...
    for (int i = 0; i < 100; ++i) {
        expected_result += i;
        RValue v(i);
        ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, std::move(v)));
    }
    LOG(INFO) << "stop";
    ASSERT_EQ(0, eabase::execution_queue_stop(queue_id));
    ASSERT_NE(0, eabase::execution_queue_execute(queue_id, RValue(0)));
    ASSERT_EQ(0, eabase::execution_queue_join(queue_id));
    ASSERT_EQ(expected_result, result);
    ASSERT_TRUE(stopped);
}
...
struct PushArg {
    eabase::ExecutionQueueId<LongIntTask> id {0};
    eabase::atomic<int64_t> total_num {0};
    eabase::atomic<int64_t> total_time {0};
    eabase::atomic<int64_t> expected_value {0};
...
void* push_thread(void *arg) {
    ...
    timer.stop();
    pa->expected_value.fetch_add(sum, eabase::memory_order_relaxed);
...
void* push_thread_which_addresses_execq(void *arg) {
    ...
    timer.start();
    int num = 0;
    eabase::ExecutionQueue<LongIntTask>::scoped_ptr_t ptr
            = eabase::execution_queue_address(pa->id);
    EXPECT_TRUE(ptr);
...
void test_performance(bool use_pthread) {
    pthread_t threads[8];
    eabase::ExecutionQueueId<LongIntTask> queue_id = { 0 }; // to suppress warnings
    eabase::ExecutionQueueOptions options;
    options.use_pthread = use_pthread;
    if (options.use_pthread) {
        LOG(INFO) << "================ pthread ================";
...
int add_with_suspend(void* meta, eabase::TaskIterator<LongIntTask>& iter) {
    int64_t* result = (int64_t*)meta;
...
void test_execute_urgent(bool use_pthread) {
    ...
    for (int i = 0; i < 100; ++i) {
        ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, -100));
        while (!g_suspending) {
            usleep(100);
        }
        ASSERT_EQ(0, eabase::execution_queue_execute(
            queue_id, -1, &eabase::TASK_OPTIONS_URGENT));
        g_suspending = false;
        usleep(100);
    }
    usleep(500* 1000);
...
void test_urgent_task_is_the_last_task(bool use_pthread) {
    ...
    for (int j = 1; j < 100; ++j) {
        expected += j;
        ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, j));
    }
    ASSERT_EQ(0, eabase::execution_queue_execute(
        queue_id, -1, &eabase::TASK_OPTIONS_URGENT));
    usleep(100);
    g_suspending = false;
    eabase::atomic_thread_fence(eabase::memory_order_acq_rel);
    usleep(10 * 1000);
...
long next_task[1024];
eabase::atomic<int> num_threads(0);

void* push_thread_with_id(void* arg) {
    eabase::ExecutionQueueId<LongIntTask> id = { (uint64_t)arg };
    int thread_id = num_threads.fetch_add(1, eabase::memory_order_relaxed);
    LOG(INFO) << "Start thread" << thread_id;
    for (int i = 0; i < 100000; ++i) {
        eabase::execution_queue_execute(id, ((long)thread_id << 32) | i);
    }
    return NULL;
}

int check_order(void* meta, eabase::TaskIterator<LongIntTask>& iter) {
    for (; iter; ++iter) {
...
int check_running_thread(void* arg, eabase::TaskIterator<LongIntTask>& iter) {
    if (iter.is_queue_stopped()) {
        return 0;
    }
    for (; iter; ++iter) {}
...
void test_in_place_task(bool use_pthread) {
    ...
    ASSERT_EQ(0, eabase::execution_queue_start(&queue_id, &options,
                                                ...
                                                (void*)thread_id));
    ASSERT_EQ(0, eabase::execution_queue_execute(
        queue_id, 0, &eabase::TASK_OPTIONS_INPLACE));
    ASSERT_EQ(0, eabase::execution_queue_stop(queue_id));
    ASSERT_EQ(0, eabase::execution_queue_join(queue_id));
}
...
void *run_first_tasks(void* arg) {
    eabase::ExecutionQueueId<InPlaceTask> queue_id = { (uint64_t)arg };
    InPlaceTask task;
    task.first_task = true;
    task.thread_id = pthread_self();
    EXPECT_EQ(0, eabase::execution_queue_execute(
        queue_id, task, &eabase::TASK_OPTIONS_INPLACE));
    return NULL;
}

int stuck_and_check_running_thread(void* arg, eabase::TaskIterator<InPlaceTask>& iter) {
    if (iter.is_queue_stopped()) {
        return 0;
    }
    eabase::atomic<int>* futex = (eabase::atomic<int>*)arg;
    if (iter->first_task) {
        EXPECT_EQ(pthread_self(), iter->thread_id);
        futex->store(1);
        eabase::futex_wake_private(futex, 1);
        while (futex->load() != 2) {
            eabase::futex_wait_private(futex, 1, NULL);
        }
        ++iter;
        EXPECT_FALSE(iter);
    } else {
...
void test_should_start_new_thread_on_more_tasks(bool use_pthread) {
    eabase::ExecutionQueueId<InPlaceTask> queue_id = { 0 };
    eabase::ExecutionQueueOptions options;
    options.use_pthread = use_pthread;
    if (options.use_pthread) {
        LOG(INFO) << "================ pthread ================";
    ...
    while (futex.load() != 1) {
        eabase::futex_wait_private(&futex, 0, NULL);
    }
...
TEST_F(ExecutionQueueTest, size_of_task_node) {
    LOG(INFO) << "sizeof(TaskNode)=" << sizeof(eabase::TaskNode);
}

int add_with_suspend2(void* meta, eabase::TaskIterator<LongIntTask>& iter) {
    int64_t* result = (int64_t*)meta;
    if (iter.is_queue_stopped()) {
        stopped = true;
        return 0;
...
void test_cancel(bool use_pthread) {
    ...
    g_suspending = false;
    eabase::TaskHandle handle0;
    ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, -100, NULL, &handle0));
    while (!g_suspending) {
        usleep(10);
    }
    ASSERT_EQ(1, eabase::execution_queue_cancel(handle0));
    ASSERT_EQ(1, eabase::execution_queue_cancel(handle0));
    eabase::TaskHandle handle1;
    ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, 100, NULL, &handle1));
    ASSERT_EQ(0, eabase::execution_queue_cancel(handle1));
    g_suspending = false;
    ASSERT_EQ(-1, eabase::execution_queue_cancel(handle1));
    ASSERT_EQ(0, eabase::execution_queue_stop(queue_id));
    ASSERT_EQ(0, eabase::execution_queue_join(queue_id));
    ASSERT_EQ(0, result);
}
...
struct CancelSelf {
    eabase::atomic<eabase::TaskHandle*> handle;
};

int cancel_self(void* /*meta*/, eabase::TaskIterator<CancelSelf*>& iter) {

    for (; iter; ++iter) {
        while ((*iter)->handle == NULL) {
            usleep(10);
        }
        EXPECT_EQ(1, eabase::execution_queue_cancel(*(*iter)->handle.load()));
        EXPECT_EQ(1, eabase::execution_queue_cancel(*(*iter)->handle.load()));
        EXPECT_EQ(1, eabase::execution_queue_cancel(*(*iter)->handle.load()));
    }
    return 0;
...
void test_cancel_self(bool use_pthread) {
    eabase::ExecutionQueueId<CancelSelf*> queue_id = { 0 }; // to suppress warnings
    eabase::ExecutionQueueOptions options;
    options.use_pthread = use_pthread;
    if (options.use_pthread) {
        LOG(INFO) << "================ pthread ================";
    ...
    CancelSelf task;
    task.handle = NULL;
    eabase::TaskHandle handle;
    ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, &task, NULL, &handle));
...
struct AddMeta {
    int64_t sum;
    eabase::atomic<int64_t> expected;
    eabase::atomic<int64_t> succ_times;
    eabase::atomic<int64_t> race_times;
    eabase::atomic<int64_t> fail_times;
};

int add_with_cancel(void* meta, eabase::TaskIterator<AddTask>& iter) {
    if (iter.is_queue_stopped()) {
        return 0;
    }
    ...
    for (; iter; ++iter) {
        if (iter->cancel_task) {
            const int rc = eabase::execution_queue_cancel(iter->handle);
            if (rc == 0) {
...
void test_random_cancel(bool use_pthread) {
    eabase::ExecutionQueueId<AddTask> queue_id = { 0 };
    AddMeta m;
    m.sum = 0;
    m.expected.store(0);
    ...
    ASSERT_EQ(0, eabase::execution_queue_start(&queue_id, NULL,
                                                add_with_cancel, &m));
    int64_t expected = 0;
    for (int i = 0; i < 100000; ++i) {
        eabase::TaskHandle h;
        ...
        t.value = i;
        t.cancel_task = false;
        ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, t, NULL, &h));
        const int r = eabase::fast_rand_less_than(4);
        expected += i;
        if (r == 0) {
            if (eabase::execution_queue_cancel(h) == 0) {
                expected -= i;
            }
        } else if (r == 1) {
            t.cancel_task = true;
            t.cancel_value = i;
            t.handle = h;
            ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, t, NULL));
        } else if (r == 2) {
            t.cancel_task = true;
            t.cancel_value = i;
            t.handle = h;
            ASSERT_EQ(0, eabase::execution_queue_execute(
                queue_id, t, &eabase::TASK_OPTIONS_URGENT));
        } else {
            // do nothing;
        }
    }
...
int add2(void* meta, eabase::TaskIterator<LongIntTask> &iter) {
    if (iter) {
        int64_t* result = (int64_t*)meta;
        *result += iter->value;
        if (iter->event) { iter->event->signal(); }
...
int add_with_suspend3(void* meta, eabase::TaskIterator<LongIntTask>& iter) {
    int64_t* result = (int64_t*)meta;
    if (iter.is_queue_stopped()) {
        stopped = true;
        return 0;
...
void test_cancel_unexecuted_high_priority_task(bool use_pthread) {
    ...
    eabase::TaskHandle h;
    ASSERT_EQ(0, eabase::execution_queue_execute(
        queue_id, -100, &eabase::TASK_OPTIONS_URGENT, &h));
    ASSERT_EQ(0, eabase::execution_queue_cancel(h));

    // Resume executor
    g_suspending = false;

    // Push a normal task
    ASSERT_EQ(0, eabase::execution_queue_execute(queue_id, 12345));
