...
#include "eabase/utility/fd_utility.h"
...
#ifndef NDEBUG
namespace eabase {
extern eabase::atomic<int> break_nums;
...
struct EA_CACHELINE_ALIGNMENT ClientMeta {
    int fd;
    size_t count;
    size_t times;
};
...
# endif
        ...
        cm[i]->times = REP;
#ifdef RUN_CLIENT_IN_FIBER
        eabase::make_non_blocking(cm[i]->fd);
        ASSERT_EQ(0, fiber_start(&cth[i], NULL, client_thread, cm[i]));
#else
    ...
    for (size_t i = 0; i < NEPOLL; ++i) {
        ...
        em->epfd = epfd[i];
#ifdef RUN_EPOLL_IN_FIBER
        ASSERT_EQ(0, fiber_start(&eth[i], epoll_thread, em, NULL);
#else
    ...
    //eabase::stop_and_join_epoll_threads();
    fiber_usleep(100000);

#ifndef NDEBUG
    std::cout << "break_nums=" << eabase::break_nums << std::endl;
#endif
}
...
TEST(FDTest, interrupt_pthread) {
    ...
    eabase::interrupt_pthread(th);
    fiber_usleep(100000L);
    std::cout << "wake up " << th2 << std::endl;
    eabase::interrupt_pthread(th2);
...
void* close_the_fd(void* arg) {
    ...
    EXPECT_EQ(0, fiber_close(*(int*)arg));
    return NULL;
}
...
#endif
    fiber_t th;
    ASSERT_EQ(0, fiber_start(&th, NULL, close_the_fd, &fds[1]));
    eabase::Timer tm;
    tm.start();
#if defined(OS_LINUX)
...
void* wait_for_the_fd(void* arg) {
    timespec ts = eabase::milliseconds_from_now(50);
#if defined(OS_LINUX)
...
TEST(FDTest, timeout) {
    ...
    fiber_t bth;
    ASSERT_EQ(0, fiber_start(&bth, NULL, wait_for_the_fd, &fds[0]));
    eabase::Timer tm;
    tm.start();
