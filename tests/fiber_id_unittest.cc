...
TEST(FiberSessionTest, join_after_destroy) {
    fiber_session_t id1;
    int x = 0xdead;
    ASSERT_EQ(0, fiber_session_create_ranged(&id1, &x, NULL, 2));
    fiber_session_t id2 = { id1.value + 1 };
    ASSERT_EQ(get_version(id1), eabase::id_value(id1));
    ASSERT_EQ(get_version(id1), eabase::id_value(id2));
    pthread_t th[8];
    ...
    ASSERT_EQ(get_version(id1) + 5, eabase::id_value(id1));
    ASSERT_EQ(get_version(id1) + 5, eabase::id_value(id2));
}

TEST(FiberSessionTest, join_before_destroy) {
    fiber_session_t id1;
    int x = 0xdead;
    ...
    ASSERT_EQ(0, fiber_session_join(id1));
    ASSERT_EQ(0xdead + 1, x);
    ASSERT_EQ(get_version(id1) + 4, eabase::id_value(id1));
...
TEST(FiberSessionTest, error_is_destroy) {
    fiber_session_t id1;
    OnResetArg arg = { { 0 }, 0 };
...
TEST(FiberSessionTest, error_is_destroy_ranged) {
    fiber_session_t id1;
    OnResetArg arg = { { 0 }, 0 };
...
TEST(FiberSessionTest, default_error_is_destroy) {
    fiber_session_t id1;
...
TEST(FiberSessionTest, doubly_destroy) {
    fiber_session_t id1;
...
TEST(FiberSessionTest, many_error) {
    fiber_session_t id1;
    ...
    ASSERT_EQ(get_version(id1) + 1, eabase::id_value(id1));
    for (int i = 0; i < N; ++i) {
        ASSERT_EQ(0, fiber_session_error(id1, err++));
    }
    ASSERT_EQ((size_t)N, result.size());
...
TEST(FiberSessionTest, id_lock_and_destroy) {
    fiber_session_t id1;
    ASSERT_EQ(0, fiber_session_create(&id1, NULL, NULL));
    ASSERT_EQ(get_version(id1), eabase::id_value(id1));
    pthread_t th[8];
...
TEST(FiberSessionTest, join_after_destroy_before_unlock) {
    fiber_session_t id1;
    int x = 0xdead;
    ASSERT_EQ(0, fiber_session_create(&id1, &x, NULL));
    ASSERT_EQ(get_version(id1), eabase::id_value(id1));
...
void* stopped_waiter(void* void_arg) {
    ...
    EXPECT_EQ(get_version(args->id) + 4, eabase::id_value(args->id));
    return NULL;
}

TEST(FiberSessionTest, stop_a_wait_after_fight_before_signal) {
    fiber_session_t id1;
    int x = 0xdead;
    ASSERT_EQ(0, fiber_session_create(&id1, &x, NULL));
    ASSERT_EQ(get_version(id1), eabase::id_value(id1));
    ...
    for (size_t i = 0; i < ARRAY_SIZE(th); ++i) {
        ...
        ASSERT_EQ(0, fiber_start(&th[i], NULL, stopped_waiter, &args[i]));
    }
    ...
    fiber_usleep(10000);
    for (size_t i = 0; i < ARRAY_SIZE(th); ++i) {
        ASSERT_TRUE(eabase::TaskGroup::exists(th[i]));
    }
...
void* waiter(void* arg) {
    ...
    EXPECT_EQ(get_version(id) + 4, eabase::id_value(id));
    return NULL;
}
...
TEST(FiberSessionTest, list_signal) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(id); ++i) {
        ...
        ASSERT_EQ(get_version(id[i]), eabase::id_value(id[i]));
...
TEST(FiberSessionTest, status) {
    fiber_session_t id;
    fiber_session_create(&id, NULL, NULL);
    eabase::id_status(id, std::cout);
...
TEST(FiberSessionTest, reset_range) {
    fiber_session_t id;
...
TEST(FiberSessionTest, about_to_destroy_before_locking) {
    fiber_session_t id;
    ASSERT_EQ(0, fiber_session_create(&id, NULL, NULL));
    ...
    ASSERT_EQ(0, fiber_start_lazy(&bth, NULL, fail_to_lock_id, &args));
...
TEST(FiberSessionTest, about_to_destroy_cancelled) {
    fiber_session_t id;
    ASSERT_EQ(0, fiber_session_create(&id, NULL, NULL));
    ASSERT_EQ(0, fiber_session_lock(id, NULL));
    ASSERT_EQ(0, fiber_session_about_to_destroy(id));
    ...
    ASSERT_EQ(0, fiber_start_lazy(&bth, NULL, succeed_to_lock_id, &id));
    // The threads should quit soon.
    pthread_join(pth, NULL);
    fiber_join(bth, NULL);
    eabase::id_status(id, std::cout);
...
TEST(FiberSessionTest, about_to_destroy_during_locking) {
    fiber_session_t id;
    ASSERT_EQ(0, fiber_session_create(&id, NULL, NULL));
    ASSERT_EQ(0, fiber_session_lock(id, NULL));
...
TEST(FiberSessionTest, error_with_descriptions) {
    fiber_session_t id1;
