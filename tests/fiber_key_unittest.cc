...
// Count tls usages.
struct Counters {
    eabase::atomic<size_t> ncreate {0};
    eabase::atomic<size_t> ndestroy {0};
    eabase::atomic<size_t> nenterthread {0};
    eabase::atomic<size_t> nleavethread {0};
};
...
struct CountersWrapper {
    ...
    ~CountersWrapper() {
        if (_c) {
            _c->ndestroy.fetch_add(1, eabase::memory_order_relaxed);
        }
...
static void worker1_impl(Counters* cs) {
    cs->nenterthread.fetch_add(1, eabase::memory_order_relaxed);
    ...
    for (size_t i = 0; i < arraysize(k); ++i) {
        cs->ncreate.fetch_add(1, eabase::memory_order_relaxed);
    ...
    cs->nleavethread.fetch_add(1, eabase::memory_order_relaxed);
}
...
TEST(KeyTest, creating_key_in_parallel) {
    ...
    for (size_t i = 0; i < arraysize(bth); ++i) {
        ASSERT_EQ(0, fiber_start_lazy(&bth[i], NULL, worker1, &args));
    }
    for (size_t i = 0; i < arraysize(th); ++i) {
    ...
    ASSERT_EQ(arraysize(th) + arraysize(bth),
              args.nenterthread.load(eabase::memory_order_relaxed));
    ASSERT_EQ(arraysize(th) + arraysize(bth),
              args.nleavethread.load(eabase::memory_order_relaxed));
    ASSERT_EQ(NKEY_PER_WORKER * (arraysize(th) + arraysize(bth)),
              args.ncreate.load(eabase::memory_order_relaxed));
    ASSERT_EQ(NKEY_PER_WORKER * (arraysize(th) + arraysize(bth)),
              args.ndestroy.load(eabase::memory_order_relaxed));
}

eabase::atomic<size_t> seq(1);
...
TEST(KeyTest, use_one_key_in_different_threads) {
    ...
    fiber_t bth[1];
    for (size_t i = 0; i < arraysize(bth); ++i) {
        ASSERT_EQ(0, fiber_start(&bth[i], NULL, worker2, &k));
    }
    for (size_t i = 0; i < arraysize(th); ++i) {
        ASSERT_EQ(0, pthread_join(th[i], NULL));
    }
...
TEST(KeyTest, use_invalid_keys) {
    ...
    ASSERT_EQ(0, fiber_start(&bth, NULL, use_invalid_keys, &keys));
...
TEST(KeyTest, set_in_dtor) {
    ...
    ASSERT_EQ(0, fiber_start(&bth, NULL, sid_thread, &bth_data));
    ASSERT_EQ(0, fiber_start(&bth2, &FIBER_ATTR_PTHREAD,
...
void set_before_anybth_impl(SBAData* data) {
    ...
    if (data->level++ == 0) {
        fiber_t bth;
        ASSERT_EQ(0, fiber_start(&bth, NULL, set_before_anybth, data));
        ASSERT_EQ(0, fiber_join(bth, NULL));
...
TEST(KeyTest, using_pool) {
    ...
    fiber_t bth;
    ASSERT_EQ(0, fiber_start(&bth, &attr, pool_thread, &bth_data));
    ASSERT_EQ(0, fiber_join(bth, NULL));
    ...
    fiber_t bth2;
    ASSERT_EQ(0, fiber_start(&bth2, &attr2, pool_thread, &bth2_data));
    ASSERT_EQ(0, fiber_join(bth2, NULL));
