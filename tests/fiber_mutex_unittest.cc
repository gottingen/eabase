...
long start_time = eabase::cpuwide_time_ms();
int c = 0;
void* locker(void* arg) {
    fiber_mutex_t* m = (fiber_mutex_t*)arg;
    fiber_mutex_lock(m);
    printf("[%" PRIu64 "] I'm here, %d, %" PRId64 "ms\n", 
           pthread_numeric_id(), ++c, eabase::cpuwide_time_ms() - start_time);
    fiber_usleep(10000);
...
TEST(MutexTest, sanity) {
    ...
    fiber_t th1;
    ASSERT_EQ(0, fiber_start(&th1, NULL, locker, &m));
...
TEST(MutexTest, timedlock) {
    ...
    fiber_mutex_lock (&m2);
    fiber_t pth;
    ASSERT_EQ(0, fiber_start(&pth, NULL, do_locks, &m1));
...
TEST(MutexTest, cpp_wrapper) {
    ...
    {
        std::unique_lock<eabase::FiberMutex> lck1;
        std::unique_lock<eabase::FiberMutex> lck2(mutex);
        lck1.swap(lck2);
        lck1.unlock();
        lck1.lock();
    }
...
struct EA_CACHELINE_ALIGNMENT PerfArgs {
    Mutex* mutex;
    int64_t counter;
    int64_t elapse_ns;
    bool ready;
...
void PerfTest(Mutex* mutex,
    ...
    LOG(INFO) << eabase::class_name<Mutex>() << " in "
...
TEST(MutexTest, performance) {
    ...
    eabase::Mutex base_mutex;
    PerfTest(&base_mutex, (pthread_t*)NULL, thread_num, pthread_create, pthread_join);
    PerfTest(&base_mutex, (fiber_t*)NULL, thread_num, fiber_start_lazy, fiber_join);
    eabase::Mutex bth_mutex;
    PerfTest(&bth_mutex, (pthread_t*)NULL, thread_num, pthread_create, pthread_join);
    PerfTest(&bth_mutex, (fiber_t*)NULL, thread_num, fiber_start_lazy, fiber_join);
}

void* loop_until_stopped(void* arg) {
    eabase::Mutex *m = (eabase::Mutex*)arg;
    while (!g_stopped) {
        BAIDU_SCOPED_LOCK(*m);
        fiber_usleep(20);
    }
...
TEST(MutexTest, mix_thread_types) {
    ...
    for (int i = 0; i < M; ++i) {
        ...
        ASSERT_EQ(0, fiber_start(&fibers[i], attr, loop_until_stopped, &m));
    }
