...
public:
    ...
    void schedule(eabase::TimerThread* timer_thread) {
    ...
    void run()
    {
        ...
        if (saved_sleep_ms > 0) {
            timespec timeout = eabase::milliseconds_to_timespec(saved_sleep_ms);
            eabase::futex_wait_private(&_sleep_ms, saved_sleep_ms, &timeout);
        }
    }
    ...
    void wakeup() {
        if (_sleep_ms != 0) {
            _sleep_ms = 0;
            eabase::futex_wake_private(&_sleep_ms, 1);
        } else {
    ...
    eabase::TimerThread::TaskId _task_id;
...
TEST(TimerThreadTest, RunTasks) {
    eabase::TimerThread timer_thread;
    ASSERT_EQ(0, timer_thread.start(NULL));

    timespec _2s_later = eabase::seconds_from_now(2);
    ...
    timespec _1s_later = eabase::seconds_from_now(1);
    TimeKeeper keeper3(_1s_later, "keeper3");
    keeper3.schedule(&timer_thread);

    timespec _10s_later = eabase::seconds_from_now(10);
    ...
    const timespec keeper6_addtime = eabase::seconds_from_now(0);
...
TEST(TimerThreadTest, start_after_schedule) {
    ...
    ASSERT_EQ(eabase::TimerThread::INVALID_TASK_ID, keeper._task_id);
    ASSERT_EQ(0, timer_thread.start(NULL));
    keeper.schedule(&timer_thread);
    ASSERT_NE(eabase::TimerThread::INVALID_TASK_ID, keeper._task_id);
    timespec current_time = eabase::seconds_from_now(0);
...
class TestTask {
public:
    TestTask(eabase::TimerThread* timer_thread, TimeKeeper* keeper1,
...
private:
    eabase::TimerThread* _timer_thread;  // not owned.
...
TEST(TimerThreadTest, schedule_and_unschedule_in_task) {
    ...
    const timespec _500ms_after = eabase::milliseconds_from_now(500);
    ...
    timespec keeper3_addtime = eabase::seconds_from_now(0);
