...
#include "eabase/fiber/task_meta.h"

namespace eabase {
    extern __thread eabase::LocalStorage tls_bls;
}
...
TEST_F(FiberTest, sizeof_task_meta) {
    LOG(INFO) << "sizeof(TaskMeta)=" << sizeof(eabase::TaskMeta);
}
...
void* spin_and_log(void* arg) {
    ...
    eabase::EveryManyUS every_1s(1000000L);
    size_t i = 0;
    while (!stop) {
        if (every_1s) {
...
void* launcher(void* arg) {
    ...
    for (size_t i = 0; !stop; ++i) {
        fiber_t th;
        fiber_start(&th, NULL, do_nothing, (void*)i);
        fiber_usleep(1000000L);
    }
    return NULL;
}
...
void* misc(void* arg) {
    LOG(INFO) << "misc(" << arg << ")";
    fiber_t th[8];
    EXPECT_EQ(0, fiber_start(&th[0], NULL, sleep_for_awhile, (void*)2));
    EXPECT_EQ(0, fiber_start(&th[1], NULL, just_exit, (void*)3));
    EXPECT_EQ(0, fiber_start(&th[2], NULL, repeated_sleep, (void*)4));
    EXPECT_EQ(0, fiber_start(&th[3], NULL, repeated_sleep, (void*)68));
    EXPECT_EQ(0, fiber_start(&th[4], NULL, spin_and_log, (void*)5));
    EXPECT_EQ(0, fiber_start(&th[5], NULL, spin_and_log, (void*)85));
    EXPECT_EQ(0, fiber_start(&th[6], NULL, launcher, (void*)6));
    EXPECT_EQ(0, fiber_start(&th[7], NULL, stopper, NULL));
...
TEST_F(FiberTest, sanity) {
    ...
    fiber_t th1;
    ASSERT_EQ(0, fiber_start(&th1, NULL, misc, (void*)1));
...
TEST_F(FiberTest, backtrace) {
    fiber_t th;
    ASSERT_EQ(0, fiber_start(&th, NULL, tf, NULL));
...
TEST_F(FiberTest, fiber_self) {
    ASSERT_EQ(0ul, fiber_self());
    fiber_t bth;
    ASSERT_EQ(0, fiber_start(&bth, NULL, show_self, NULL));
...
TEST_F(FiberTest, fiber_join) {
    ...
    // Joining self
    fiber_t th;
    ASSERT_EQ(0, fiber_start(&th, NULL, join_self, NULL));
}
...
TEST_F(FiberTest, errno_not_changed) {
    fiber_t th;
    errno = 1;
    fiber_start(&th, NULL, change_errno, (void*)(intptr_t)2);
    ASSERT_EQ(1, errno);
}
...
void* adding_func(void* arg) {
    eabase::atomic<size_t>* s = (eabase::atomic<size_t>*)arg;
    if (sleep_in_adding_func > 0) {
        long t1 = 0;
        if (10000 == s->fetch_add(1)) {
            t1 = eabase::cpuwide_time_us();
        }
        fiber_usleep(sleep_in_adding_func);
        if (t1) {
            LOG(INFO) << "elapse is " << eabase::cpuwide_time_us() - t1 << "ns";
        }
    } else {
        s->fetch_add(1);
    }
...
TEST_F(FiberTest, small_threads) {
    for (size_t z = 0; z < 2; ++z) {
        ...
        for (size_t j = 0; j < 3; ++j) {
            ...
            LOG(INFO) << "[Round " << j + 1 << "] fiber_start takes "
...
void* fiber_starter(void* void_counter) {
    while (!stop.load(eabase::memory_order_relaxed)) {
        fiber_t th;
        EXPECT_EQ(0, fiber_start(&th, NULL, adding_func, void_counter));
    }
    return NULL;
}

struct EA_CACHELINE_ALIGNMENT AlignedCounter {
    AlignedCounter() : value(0) {}
    eabase::atomic<size_t> value;
};
...
void* log_start_latency(void* void_arg) {
    eabase::Timer* tm = static_cast<eabase::Timer*>(void_arg);
    tm->stop();
    return NULL;
}
...
TEST_F(FiberTest, start_latency_when_high_idle) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        tm.start();
        fiber_t th;
        fiber_start(&th, NULL, log_start_latency, &tm);
        fiber_join(th, NULL);
        fiber_t th2;
        eabase::Timer tm2;
        tm2.start();
        fiber_start_lazy(&th2, NULL, log_start_latency, &tm2);
        fiber_join(th2, NULL);
        if (!warmup) {
            ++REP;
            elp1 += tm.n_elapsed();
...
TEST_F(FiberTest, fiber_exit) {
    ...
    ASSERT_EQ(0, fiber_start(&th1, NULL, just_exit, NULL));
    ASSERT_EQ(0, fiber_start_lazy(&th2, NULL, just_exit, NULL));
    ASSERT_EQ(0, pthread_create(&th3, NULL, just_exit, NULL));
    EXPECT_EQ(0, fiber_start(&th4, &attr, just_exit, NULL));
    EXPECT_EQ(0, fiber_start_lazy(&th5, &attr, just_exit, NULL));
...
TEST_F(FiberTest, fiber_equal) {
    fiber_t th1;
    ASSERT_EQ(0, fiber_start(&th1, NULL, do_nothing, NULL));
    fiber_t th2;
    ASSERT_EQ(0, fiber_start(&th2, NULL, do_nothing, NULL));
...
void* check_sleep(void* pthread_task) {
    ...
    EXPECT_EQ(0, fiber_start(&th1, &attr, mark_run, &run));
    if (pthread_task) {
        fiber_usleep(100000L);
...
TEST_F(FiberTest, fiber_usleep) {
    ...
    fiber_t th1;
    ASSERT_EQ(0, fiber_start(&th1, &FIBER_ATTR_PTHREAD,
                                      check_sleep, (void*)1));
    ASSERT_EQ(0, fiber_join(th1, NULL));
    
    fiber_t th2;
    ASSERT_EQ(0, fiber_start(&th2, NULL,
                                      check_sleep, (void*)0));
    ASSERT_EQ(0, fiber_join(th2, NULL));
}
...
void* test_parent_span(void* p) {
    ...
    *q = (uint64_t)(eabase::tls_bls.rpcz_parent_span);
...
TEST_F(FiberTest, test_span) {
    ...
    eabase::tls_bls.rpcz_parent_span = (void*)target;
    fiber_t th1;
    ASSERT_EQ(0, fiber_start(&th1, &FIBER_ATTR_NORMAL_WITH_SPAN,
                                      test_parent_span, &p1));
    ASSERT_EQ(0, fiber_join(th1, NULL));

    fiber_t th2;
    ASSERT_EQ(0, fiber_start_lazy(&th2, NULL,
                                      test_parent_span, &p2));
    ASSERT_EQ(0, fiber_join(th2, NULL));
...
TEST_F(FiberTest, too_many_nosignal_threads) {
    for (size_t i = 0; i < 100000; ++i) {
        fiber_attr_t attr = FIBER_ATTR_NORMAL | FIBER_NOSIGNAL;
        fiber_t tid;
        ASSERT_EQ(0, fiber_start(&tid, &attr, dummy_thread, NULL));
    }
}
...
TEST_F(FiberTest, yield_single_thread) {
    fiber_t tid;
    ASSERT_EQ(0, fiber_start_lazy(&tid, NULL, yield_thread, NULL));
