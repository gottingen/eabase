...
#include "eabase/utility/files/scoped_temp_dir.h"
...
TEST(FileTest, Create) {
...

  {
    ...
    EXPECT_EQ(eabase::File::FILE_ERROR_FAILED, file.error_details());

    File file2(eabase::File::FILE_ERROR_TOO_MANY_OPENED);
    EXPECT_FALSE(file2.IsValid());
    EXPECT_EQ(eabase::File::FILE_ERROR_TOO_MANY_OPENED, file2.error_details());
  }

  {
    // Open a file that doesn't exist.
    File file(file_path, eabase::File::FLAG_OPEN | eabase::File::FLAG_READ);
    EXPECT_FALSE(file.IsValid());
    EXPECT_EQ(eabase::File::FILE_ERROR_NOT_FOUND, file.error_details());
  }

  {
    // Open or create a file.
    File file(file_path, eabase::File::FLAG_OPEN_ALWAYS | eabase::File::FLAG_READ);
    EXPECT_TRUE(file.IsValid());
    EXPECT_TRUE(file.created());
    EXPECT_EQ(eabase::File::FILE_OK, file.error_details());
  }
...

  {
    ...
    File file;
    file.Initialize(file_path, eabase::File::FLAG_OPEN | eabase::File::FLAG_READ);
    EXPECT_TRUE(file.IsValid());
    EXPECT_FALSE(file.created());
    EXPECT_EQ(eabase::File::FILE_OK, file.error_details());
...

  {
    ...
    File file(file_path, eabase::File::FLAG_CREATE | eabase::File::FLAG_READ);
    EXPECT_FALSE(file.IsValid());
    EXPECT_FALSE(file.created());
    EXPECT_EQ(eabase::File::FILE_ERROR_EXISTS, file.error_details());
  }

  {
    // Create or overwrite a file.
    File file(file_path,
              eabase::File::FLAG_CREATE_ALWAYS | eabase::File::FLAG_WRITE);
    EXPECT_TRUE(file.IsValid());
    EXPECT_TRUE(file.created());
    EXPECT_EQ(eabase::File::FILE_OK, file.error_details());
  }

  {
    ...
    File file(file_path,
              eabase::File::FLAG_OPEN_ALWAYS | eabase::File::FLAG_READ |
                  eabase::File::FLAG_DELETE_ON_CLOSE);
    EXPECT_TRUE(file.IsValid());
    EXPECT_TRUE(file.created());
    EXPECT_EQ(eabase::File::FILE_OK, file.error_details());
  }

  EXPECT_FALSE(eabase::PathExists(file_path));
}
...
TEST(FileTest, Async) {
  ...
  {
    File file(file_path, eabase::File::FLAG_OPEN_ALWAYS | eabase::File::FLAG_ASYNC);
    EXPECT_TRUE(file.IsValid());
    EXPECT_TRUE(file.async());
  }

  {
    File file(file_path, eabase::File::FLAG_OPEN_ALWAYS);
    EXPECT_TRUE(file.IsValid());
...
TEST(FileTest, DeleteOpenFile) {
  ...
  File same_file(file_path,
                 eabase::File::FLAG_OPEN | eabase::File::FLAG_DELETE_ON_CLOSE |
                     eabase::File::FLAG_READ);
  EXPECT_TRUE(file.IsValid());
  EXPECT_FALSE(same_file.created());
  EXPECT_EQ(eabase::File::FILE_OK, same_file.error_details());
...
TEST(FileTest, ReadWrite) {
  ...
  File file(file_path,
            eabase::File::FLAG_CREATE | eabase::File::FLAG_READ |
                eabase::File::FLAG_WRITE);
...
TEST(FileTest, Append) {
  ...
  File file(file_path, eabase::File::FLAG_CREATE | eabase::File::FLAG_APPEND);
  ASSERT_TRUE(file.IsValid());
  ...
  File file2(file_path,
             eabase::File::FLAG_OPEN | eabase::File::FLAG_READ |
                 eabase::File::FLAG_APPEND);
...
TEST(FileTest, DISABLED_TouchGetInfo) {
  ...
  File file(temp_dir.path().AppendASCII("touch_get_info_file"),
            eabase::File::FLAG_CREATE | eabase::File::FLAG_WRITE |
                eabase::File::FLAG_WRITE_ATTRIBUTES);
  ASSERT_TRUE(file.IsValid());

  // Get info for a newly created file.
  eabase::File::Info info;
  ...
  eabase::Time now = eabase::Time::Now() + eabase::TimeDelta::FromSeconds(2);
  ...
  eabase::Time creation_time = info.creation_time;
  ...
  eabase::Time new_last_accessed =
      info.last_accessed + eabase::TimeDelta::FromSeconds(234);
  eabase::Time new_last_modified =
      info.last_modified + eabase::TimeDelta::FromMinutes(567);
...
TEST(FileTest, ReadAtCurrentPosition) {
  ...
  EXPECT_EQ(0, file.Seek(eabase::File::FROM_BEGIN, 0));
...
TEST(FileTest, Seek) {
  ...
  EXPECT_EQ(kOffset, file.Seek(eabase::File::FROM_BEGIN, kOffset));
  EXPECT_EQ(2 * kOffset, file.Seek(eabase::File::FROM_CURRENT, kOffset));
  EXPECT_EQ(kOffset, file.Seek(eabase::File::FROM_CURRENT, -kOffset));
  EXPECT_TRUE(file.SetLength(kOffset * 2));
  EXPECT_EQ(kOffset, file.Seek(eabase::File::FROM_END, -kOffset));
}
