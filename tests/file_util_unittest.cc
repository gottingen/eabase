...
#include "eabase/utility/files/file_enumerator.h"
...
#if defined(OS_WIN)
#include "eabase/utility/win/scoped_handle.h"
#include "eabase/utility/win/windows_version.h"
#endif

#if defined(OS_ANDROID)
#include "eabase/utility/android/content_uri_utils.h"
#endif
...
class ReparsePoint {
 ...
 private:
  ...
  bool created_;
  EA_DISALLOW_COPY_AND_ASSIGN(ReparsePoint);
};
...
#if defined(FixedTextContentsEqual)
...
TEST_F(FileUtilTest, ReadFileToString) {
  ...
  // Delete test file.
  EXPECT_TRUE(eabase::DeleteFile(file_path, false));
...
TEST_F(VerifyPathControlledByUserTest, BadPaths) {
  ...
  FilePath does_not_exist = base_dir_.AppendASCII("does")
                                     ...
                                     .AppendASCII("exist");
  EXPECT_FALSE(
      eabase::VerifyPathControlledByUser(
...
TEST_F(FileUtilTest, CreateDirectoryParentsNotExist) {
  ASSERT_TRUE(eabase::CreateDirectory(temp_dir_.path()));
  eabase::FilePath creating_dir = temp_dir_.path().Append("sub1").Append("sub2");
  std::cout << "path=" << creating_dir.value() << std::endl;
  ASSERT_FALSE(eabase::CreateDirectory(creating_dir, false));
  ASSERT_TRUE(eabase::CreateDirectory(creating_dir, true));
}
...
TEST(ScopedFD, ScopedFDDoesClose) {
  ...
  eabase::ScopedFD read_end_closer(fds[0]);
  {
    eabase::ScopedFD write_end_closer(fds[1]);
  }
