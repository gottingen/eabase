...
#include <stdlib.h>
#include <memory>
#include <eabase/utility/files/temp_file.h>      // TempFile
#include <eabase/utility/containers/flat_map.h>
#include <eabase/utility/macros.h>
#include <eabase/utility/time.h>                 // Timer
#include <eabase/utility/fd_utility.h>           // make_non_blocking
#include <eabase/utility/iobuf.h>
#include <eabase/utility/logging.h>
#include <eabase/utility/fd_guard.h>
#include <eabase/utility/errno.h>
...
extern int32_t block_shared_count(eabase::IOBuf::Block const* b);
extern uint32_t block_cap(eabase::IOBuf::Block const* b);
...
const size_t DEFAULT_PAYLOAD = eabase::IOBuf::DEFAULT_BLOCK_SIZE - BLOCK_OVERHEAD;

void check_tls_block() {
    ASSERT_EQ((eabase::IOBuf::Block*)NULL, eabase::iobuf::get_tls_block_head());
    printf("tls_block of eabase::IOBuf was deleted\n");
}
const int ALLOW_UNUSED check_dummy = eabase::thread_atexit(check_tls_block);

static eabase::FlatSet<void*> s_set;
...
inline bool is_debug_allocator_enabled() {
    return (eabase::iobuf::blockmem_allocate == debug_block_allocate);
}

void install_debug_allocator() {
    if (!is_debug_allocator_enabled()) {
        eabase::iobuf::remove_tls_block_chain();
        s_set.init(1024);
        eabase::iobuf::blockmem_allocate = debug_block_allocate;
        eabase::iobuf::blockmem_deallocate = debug_block_deallocate;
...
static void check_memory_leak() {
    if (is_debug_allocator_enabled()) {
        eabase::IOBuf::Block* p = eabase::iobuf::get_tls_block_head();
        size_t n = 0;
        while (p) {
            ASSERT_TRUE(s_set.seek(p)) << "Memory leak: " << p;
            p = eabase::iobuf::get_portal_next(p);
            ++n;
        }
        ASSERT_EQ(n, s_set.size());
        ASSERT_EQ(n, (size_t)eabase::iobuf::get_tls_block_count());
    }
}
...
std::string to_str(const eabase::IOBuf& p) {
    return p.to_string();
}
...
TEST_F(IOBufTest, reserve) {
    eabase::IOBuf b;
    ASSERT_EQ(eabase::IOBuf::INVALID_AREA, b.reserve(0));
    const size_t NRESERVED1 = 5;
    const eabase::IOBuf::Area a1 = b.reserve(NRESERVED1);
    ASSERT_TRUE(a1 != eabase::IOBuf::INVALID_AREA);
    ...
    const eabase::IOBuf::Area a2 = b.reserve(NRESERVED2);
...
TEST_F(IOBufTest, iobuf_as_queue) {
    ...
    // If INITIAL_CAP gets bigger, creating eabase::IOBuf::Block are very
    // small. Since We don't access eabase::IOBuf::Block::data in this case.
    // We replace eabase::IOBuf::Block with FakeBlock with only nshared (in
    // the same offset)
    FakeBlock* blocks[eabase::IOBuf::INITIAL_CAP+16];
    const size_t NBLOCKS = ARRAY_SIZE(blocks);
    eabase::IOBuf::BlockRef r[NBLOCKS];
    const size_t LENGTH = 7UL;
    for (size_t i = 0; i < NBLOCKS; ++i) {
        ASSERT_TRUE((blocks[i] = new FakeBlock));
        r[i].offset = 1;
        r[i].length = LENGTH;
        r[i].block = (eabase::IOBuf::Block*)blocks[i];
    }
    ...
    ASSERT_EQ(2, eabase::iobuf::block_shared_count(r[0].block));
    ...
    ASSERT_EQ(2, eabase::iobuf::block_shared_count(r[1].block));
    ...
    //ASSERT_EQ(1, eabase::iobuf::block_shared_count(r[0].block));
    ...
    for (size_t i = 0; i < eabase::IOBuf::INITIAL_CAP+2; ++i) {
        ...
        ASSERT_EQ(2, eabase::iobuf::block_shared_count(r[i+2].block));
    }
...
TEST_F(IOBufTest, iobuf_sanity) {
    ...
    LOG(INFO) << "sizeof(eabase::IOBuf)=" << sizeof(eabase::IOBuf)
              << " sizeof(IOPortal)=" << sizeof(eabase::IOPortal);
...
TEST_F(IOBufTest, copy_and_assign) {
    ...
    const size_t TARGET_SIZE = eabase::IOBuf::DEFAULT_BLOCK_SIZE * 2;
    eabase::IOBuf buf0;
    buf0.append("hello");
    ASSERT_EQ(1u, buf0._ref_num());

    // Copy-construct from SmallView
    eabase::IOBuf buf1 = buf0;
...
TEST_F(IOBufTest, cut_into_fd_tiny) {
    ...
    eabase::make_non_blocking(fds[0]);
    eabase::make_non_blocking(fds[1]);
...
TEST_F(IOBufTest, cut_multiple_into_fd_tiny) {
    ...
    for (size_t j = 0; j < ARRAY_SIZE(b1); ++j) {
        ...
        ref.append(s);
        eabase::IOPortal* b = new eabase::IOPortal();
        b->append(s);
        b1[j] = b;
    }
    ...
    ASSERT_EQ((ssize_t)ref.length(),
              eabase::IOBuf::cut_multiple_into_file_descriptor(
                  fds[1], b1, ARRAY_SIZE(b1)));
    for (size_t j = 0; j < ARRAY_SIZE(b1); ++j) {
        ASSERT_TRUE(b1[j]->empty());
        delete (eabase::IOPortal*)b1[j];
        b1[j] = NULL;
    }
    ASSERT_EQ((ssize_t)ref.length(),
...
TEST_F(IOBufTest, cut_by_delim_perf) {
    eabase::iobuf::reset_blockmem_allocate_and_deallocate();
...
TEST_F(IOBufTest, cut_perf) {
    ...
    for (size_t i = 0; i < ARRAY_SIZE(w); ++i) {
        ...
        b.append(p);
        t.stop();
        LOG(INFO) << "IOPortal::append(eabase::IOBuf) takes "
...
TEST_F(IOBufTest, conversion_with_protobuf) {
    ...
    eabase::IOBufAsZeroCopyOutputStream out_wrapper(&buf);
    ...
    eabase::IOBufAsZeroCopyInputStream in_wrapper(buf);
...
TEST_F(IOBufTest, extended_backup) {
    for (int i = 0; i < 2; ++i) {
        ...
        eabase::iobuf::remove_tls_block_chain();
        eabase::IOBuf src;
        const int BLKSIZE = (i == 0 ? 1024 : eabase::IOBuf::DEFAULT_BLOCK_SIZE);
        const int PLDSIZE = BLKSIZE - BLOCK_OVERHEAD;
        eabase::IOBufAsZeroCopyOutputStream out_stream1(&src, BLKSIZE);
        eabase::IOBufAsZeroCopyOutputStream out_stream2(&src);
        eabase::IOBufAsZeroCopyOutputStream & out_stream =
...
TEST_F(IOBufTest, backup_iobuf_never_called_next) {
    {
        ...
        // to check.
        eabase::IOBuf dummy;
        eabase::IOBufAsZeroCopyOutputStream dummy_stream(&dummy);
    ...
    eabase::IOBufAsZeroCopyOutputStream out_stream(&src);
...
void *backup_thread(void *arg) {
    eabase::IOBufAsZeroCopyOutputStream *wrapper =
        (eabase::IOBufAsZeroCopyOutputStream *)arg;
    wrapper->BackUp(1024);
    return NULL;
}
...
TEST_F(IOBufTest, backup_in_another_thread) {
    eabase::IOBuf buf;
    eabase::IOBufAsZeroCopyOutputStream wrapper(&buf);
    size_t alloc_size = 0;
    for (int i = 0; i < 10; ++i) {
        void *data; 
        int len;
...
TEST_F(IOBufTest, own_block) {
    ...
    eabase::IOBuf::Block *saved_tls_block = eabase::iobuf::get_tls_block_head();
    eabase::IOBufAsZeroCopyOutputStream wrapper(&buf, BLOCK_SIZE);
    int alloc_size = 0;
    for (int i = 0; i < 100; ++i) {
        void *data;
        int size;
    ...
    ASSERT_EQ(saved_tls_block, eabase::iobuf::get_tls_block_head());
    ASSERT_EQ(eabase::iobuf::block_cap(buf._front_ref().block), BLOCK_SIZE - BLOCK_OVERHEAD);
}
...
TEST_F(IOBufTest, as_ostream) {
    ...
    eabase::IOBufBuilder builder;
    LOG(INFO) << "sizeof(IOBufBuilder)=" << sizeof(builder) << std::endl
              << "sizeof(IOBuf)=" << sizeof(eabase::IOBuf) << std::endl
              << "sizeof(IOBufAsZeroCopyOutputStream)="
              << sizeof(eabase::IOBufAsZeroCopyOutputStream) << std::endl
              << "sizeof(ZeroCopyStreamAsStreamBuf)="
              << sizeof(eabase::ZeroCopyStreamAsStreamBuf) << std::endl
...
TEST_F(IOBufTest, append_from_fd_with_offset) {
    eabase::TempFile file;
    file.save("dummy");
    eabase::fd_guard fd(open(file.fname(), O_RDWR | O_TRUNC));
...
TEST_F(IOBufTest, slice) {
    ...
    for (size_t i = 0; i < block_count; ++i) {
        eabase::StringPiece p = buf.backing_block(i);
...
TEST_F(IOBufTest, iterate_bytes) {
    ...
    size_t n = 0;
    eabase::IOBufBytesIterator it(a);
    for (; it != NULL; ++it, ++n) {
    ...
    saved_a = a.to_string();
    n = 0;
    for (eabase::IOBufBytesIterator it2(a); it2 != NULL; it2++/*intended post++*/, ++n) {
...
TEST_F(IOBufTest, appender_perf) {
    ...
    eabase::Timer tm2;
    tm2.start();
    eabase::IOBufAppender appender1;
    for (size_t i = 0; i < N1; ++i) {
    ...
    tm2.start();
    eabase::IOBufAppender appender2;
    for (size_t i = 0; i < N2; ++i) {
        appender2.append(s);
    }
    tm2.stop();
...
TEST_F(IOBufTest, printed_as_binary) {
    ...
    os << eabase::ToPrintable(buf, 256);
    ASSERT_STREQ(OUTPUT, os.str().c_str());
    os.str("");
    os << eabase::ToPrintable(str, 256);
    ASSERT_STREQ(OUTPUT, os.str().c_str());
}
...
TEST_F(IOBufTest, copy_to_string_from_iterator) {
    ...
    for (size_t i = 0; i < 1 * 1024 * 1024lu; ++i) {
        b0.push_back(eabase::fast_rand_in('a', 'z'));
    }
    eabase::IOBuf b1(b0);
    eabase::IOBufBytesIterator iter(b0);
    size_t nc = 0;
    while (nc < b0.length()) {
        size_t to_copy = eabase::fast_rand_in(1024lu, 64 * 1024lu);
        eabase::IOBuf b;
        b1.cutn(&b, to_copy);
        std::string s;
...
TEST_F(IOBufTest, append_user_data_and_consume) {
    ...
    eabase::IOBuf::BlockRef r = b0._front_ref();
    ASSERT_EQ(1, eabase::iobuf::block_shared_count(r.block));
...

    {
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            if (i != 255) {
                ASSERT_EQ(1UL, b0._ref_num());
                eabase::IOBuf::BlockRef r = b0._front_ref();
                ASSERT_EQ(i + 2, eabase::iobuf::block_shared_count(r.block));
            } else {
...
TEST_F(IOBufTest, share_tls_block) {
    eabase::iobuf::remove_tls_block_chain();
    eabase::IOBuf::Block* b = eabase::iobuf::acquire_tls_block();
    ASSERT_EQ(0u, eabase::iobuf::block_size(b));

    eabase::IOBuf::Block* b2 = eabase::iobuf::share_tls_block();
    eabase::IOBuf buf;
    for (size_t i = 0; i < eabase::iobuf::block_cap(b2); i++) {
        buf.push_back('x');
    }
    // after pushing to b2, b2 is full but it is still head of tls block.
    ASSERT_NE(b, b2);
    eabase::iobuf::release_tls_block_chain(b);
    ASSERT_EQ(b, eabase::iobuf::share_tls_block());
    // After releasing b, now tls block is b(not full) -> b2(full) -> NULL
    for (size_t i = 0; i < eabase::iobuf::block_cap(b); i++) {
        buf.push_back('x');
    }
    // now tls block is b(full) -> b2(full) -> NULL
    eabase::IOBuf::Block* head_block = eabase::iobuf::share_tls_block();
    ASSERT_EQ(0u, eabase::iobuf::block_size(head_block));
...
TEST_F(IOBufTest, acquire_tls_block) {
    eabase::iobuf::remove_tls_block_chain();
    eabase::IOBuf::Block* b = eabase::iobuf::acquire_tls_block();
    const size_t block_cap = eabase::iobuf::block_cap(b);
    eabase::IOBuf buf;
    for (size_t i = 0; i < block_cap; i++) {
        buf.append("x");
    }
    ASSERT_EQ(1, eabase::iobuf::get_tls_block_count());
    eabase::IOBuf::Block* head = eabase::iobuf::get_tls_block_head();
    ASSERT_EQ(eabase::iobuf::block_cap(head), eabase::iobuf::block_size(head));
    eabase::iobuf::release_tls_block_chain(b);
    ASSERT_EQ(2, eabase::iobuf::get_tls_block_count());
    for (size_t i = 0; i < block_cap; i++) {
        buf.append("x");
    }
    ASSERT_EQ(2, eabase::iobuf::get_tls_block_count());
    head = eabase::iobuf::get_tls_block_head();
    ASSERT_EQ(eabase::iobuf::block_cap(head), eabase::iobuf::block_size(head));
    b = eabase::iobuf::acquire_tls_block();
    ASSERT_EQ(0, eabase::iobuf::get_tls_block_count());
    ASSERT_NE(eabase::iobuf::block_cap(b), eabase::iobuf::block_size(b));
}
