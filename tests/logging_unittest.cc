...
#include "eabase/utility/files/temp_file.h"
...
class LogStateSaver {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(LogStateSaver);
};
...
eabase::atomic<uint64_t> test_logging_count(0);
...
TEST_F(LoggingTest, async_log) {
    ...
    std::string cmd = eabase::string_printf("grep -c %s %s",
        log.c_str(), temp_file.fname());
    ASSERT_LE(0, eabase::read_command_output(oss, cmd.c_str()));
...
void* test_log(void* void_arg) {
    ...
    while (!g_stopped) {
        {
            LOG(INFO) << *args->log;
            test_logging_count.fetch_add(1, eabase::memory_order_relaxed);
        }
        ++args->counter;
    }
    t.stop();
