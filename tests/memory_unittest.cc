...
#include "eabase/utility/process/memory.h"
...
#include "eabase/utility/process/memory_unittest_mac.h"
#endif
#if defined(OS_LINUX)
#include <malloc.h>
#endif
...
TEST(ProcessMemoryTest, GetModuleFromAddress) {
  ...
  HMODULE kernel32_from_address =
      eabase::GetModuleFromAddress(reinterpret_cast<DWORD*>(kernel32) + 1);
  EXPECT_EQ(kernel32, kernel32_from_address);
}

TEST(ProcessMemoryTest, EnableLFH) {
  ASSERT_TRUE(eabase::EnableLowFragmentationHeap());
...
// Note that eabase::EnableTerminationOnHeapCorruption() is called as part of
...
TEST(ProcessMemoryTest, MacMallocFailureDoesNotTerminate) {
  ...
  ASSERT_EXIT(
      {
        eabase::EnableTerminationOnOutOfMemory();
...
TEST_F(OutOfMemoryDeathTest, ViaSharedLibraries) {
  ...
  std::string format = eabase::StringPrintf("%%%zud", test_size_);
  char *value = NULL;
  ASSERT_DEATH({
      SetUpInDeathAssert();
...
TEST_F(OutOfMemoryDeathTest, CFAllocatorSystemDefault) {
  ASSERT_DEATH({
      SetUpInDeathAssert();
      while ((value_ =
              eabase::AllocateViaCFAllocatorSystemDefault(signed_test_size_))) {}
    }, "");
}
...
TEST_F(OutOfMemoryDeathTest, CFAllocatorMalloc) {
  ASSERT_DEATH({
      SetUpInDeathAssert();
      while ((value_ =
              eabase::AllocateViaCFAllocatorMalloc(signed_test_size_))) {}
    }, "");
}
...
TEST_F(OutOfMemoryDeathTest, CFAllocatorMallocZone) {
  ASSERT_DEATH({
      SetUpInDeathAssert();
      while ((value_ =
              eabase::AllocateViaCFAllocatorMallocZone(signed_test_size_))) {}
    }, "");
}
...
TEST_F(OutOfMemoryDeathTest, PsychoticallyBigObjCObject) {
  ASSERT_DEATH({
      SetUpInDeathAssert();
      while ((value_ = eabase::AllocatePsychoticallyBigObjCObject())) {}
    }, "");
}
...
TEST_F(OutOfMemoryHandledTest, UncheckedMalloc) {
  EXPECT_TRUE(eabase::UncheckedMalloc(kSafeMallocSize, &value_));
  EXPECT_TRUE(value_ != NULL);
  free(value_);

  EXPECT_FALSE(eabase::UncheckedMalloc(test_size_, &value_));
  EXPECT_TRUE(value_ == NULL);
}

TEST_F(OutOfMemoryHandledTest, UncheckedCalloc) {
  EXPECT_TRUE(eabase::UncheckedCalloc(1, kSafeMallocSize, &value_));
  EXPECT_TRUE(value_ != NULL);
  ...
  EXPECT_TRUE(
      eabase::UncheckedCalloc(kSafeCallocItems, kSafeCallocSize, &value_));
  EXPECT_TRUE(value_ != NULL);
  ...
  EXPECT_FALSE(eabase::UncheckedCalloc(1, test_size_, &value_));
  EXPECT_TRUE(value_ == NULL);
}
