...
#include "eabase/utility/containers/mru_cache.h"
...
TEST(MRUCacheTest, Basic) {
  typedef eabase::MRUCache<int, CachedItem> Cache;
...
TEST(MRUCacheTest, Owning) {
  typedef eabase::OwningMRUCache<int, CachedItem*> Cache;
  Cache cache(Cache::NO_AUTO_EVICT);
...
TEST(MRUCacheTest, HashingMRUCache) {
  ...
  typedef eabase::HashingMRUCache<std::string, CachedItem> Cache;
  Cache cache(Cache::NO_AUTO_EVICT);
