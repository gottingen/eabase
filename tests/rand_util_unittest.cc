...
#include "eabase/utility/memory/scoped_ptr.h"
...
TEST(RandUtilTest, Sanity) {
    EXPECT_EQ(eabase::RandInt(0, 0), 0);
    EXPECT_EQ(eabase::RandInt(kIntMin, kIntMin), kIntMin);
    EXPECT_EQ(eabase::RandInt(kIntMax, kIntMax), kIntMax);

    for (int i = 0; i < 10; ++i) {
        uint64_t value = eabase::fast_rand_in(
    ...
    for (int i = 0; i < 10; ++i) {
        int64_t value = eabase::fast_rand_in(
    ...
    EXPECT_EQ(eabase::fast_rand_in(-1, -1), -1);
    EXPECT_EQ(eabase::fast_rand_in(1, 1), 1);
    EXPECT_EQ(eabase::fast_rand_in(0, 0), 0);
    EXPECT_EQ(eabase::fast_rand_in(std::numeric_limits<int64_t>::min(),
                                  std::numeric_limits<int64_t>::min()),
              std::numeric_limits<int64_t>::min());
    EXPECT_EQ(eabase::fast_rand_in(std::numeric_limits<int64_t>::max(),
                                  std::numeric_limits<int64_t>::max()),
              std::numeric_limits<int64_t>::max());
    EXPECT_EQ(eabase::fast_rand_in(std::numeric_limits<uint64_t>::min(),
                                  std::numeric_limits<uint64_t>::min()),
              std::numeric_limits<uint64_t>::min());
    EXPECT_EQ(eabase::fast_rand_in(std::numeric_limits<uint64_t>::max(),
...
TEST(RandUtilTest, RandDouble) {
    ...
    volatile double number = eabase::RandDouble();
    EXPECT_GT(1.0, number);
    EXPECT_LE(0.0, number);

    volatile double number2 = eabase::fast_rand_double();
...
TEST(RandUtilTest, RandBytes) {
    ...
    eabase::RandBytes(buffer, buffer_size);
...
TEST(RandUtilTest, RandBytesAsString) {
    std::string random_string = eabase::RandBytesAsString(1);
    EXPECT_EQ(1U, random_string.size());
    random_string = eabase::RandBytesAsString(145);
...
TEST(RandUtilTest, RandGeneratorForRandomShuffle) {
    EXPECT_EQ(eabase::RandGenerator(1), 0U);
...
TEST(RandUtilTest, RandGeneratorIsUniform) {
    ...
    for (int round = 0; round < 2; ++round) {
        ...
        while (count < kMaxAttempts) {
            uint64_t value = (round == 0 ? eabase::RandGenerator(kTopOfRange)
                              : eabase::fast_rand_less_than(kTopOfRange));
...
TEST(RandUtilTest, RandUint64ProducesBothValuesOfAllBits) {
    ...
    for (int round = 0; round < 2; ++round) {
        ...
        for (size_t i = 0; i < 1000; ++i) {
            uint64_t value = (round == 0 ? eabase::RandUint64() : eabase::fast_rand());
            found_ones |= value;
            found_zeros &= value;
...
TEST(RandUtilTest, DISABLED_RandBytesPerf) {
    ...
    const eabase::TimeTicks now = eabase::TimeTicks::HighResNow();
    for (int i = 0; i < kTestIterations; ++i) {
        eabase::RandBytes(buffer.get(), kTestBufferSize);
    }
    const eabase::TimeTicks end = eabase::TimeTicks::HighResNow();
...
TEST(RandUtilTest, fast_rand_perf) {
    ...
    tm.start();
    for (int i = 0; i < kTestIterations; ++i) {
        s += eabase::fast_rand_less_than(kRange);
    }
    tm.stop();
