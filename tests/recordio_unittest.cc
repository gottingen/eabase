...
#include "eabase/utility/recordio.h"
...
class StringReader : public eabase::IReader {
public:
...
class StringWriter : public eabase::IWriter {
public:
...
TEST(RecordIOTest, manipulate_record) {
    ...
    eabase::IOBuf* foo_val = r1.MutableMeta("foo");
    ...
    eabase::IOBuf* bar_val = r1.MutableMeta("bar");
...
TEST(RecordIOTest, write_read_basic) {
    StringWriter sw;
    eabase::RecordWriter rw(&sw);

    eabase::Record src;
    ASSERT_EQ(0, rw.Write(src));

    eabase::IOBuf* foo_val = src.MutableMeta("foo");
    foo_val->append("foo_data");
    ASSERT_EQ(0, rw.Write(src));

    eabase::IOBuf* bar_val = src.MutableMeta("bar");
    bar_val->append("bar_data");
    ASSERT_EQ(0, rw.Write(src));
    ...
    std::cout << "len=" << sw.str().size()
              << " content=" << eabase::PrintedAsBinary(sw.str(), 256) << std::endl;

    StringReader sr(sw.str());
    eabase::RecordReader rr(&sr);
    eabase::Record r1;
    ...
    ASSERT_EQ((int)eabase::RecordReader::END_OF_READER, rr.last_error());
...
TEST(RecordIOTest, incomplete_reader) {
    ...
    std::cout << "len=" << data.size()
              << " content=" << eabase::PrintedAsBinary(data, 256) << std::endl;
...
static std::string rand_string(int min_len, int max_len) {
    const int len = eabase::fast_rand_in(min_len, max_len);
    std::string str;
    str.reserve(len);
    for (int i = 0; i < len; ++i) {
        str.push_back(eabase::fast_rand_in('a', 'Z'));
    }
    return str;
}
...
TEST(RecordIOTest, write_read_random) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        std::string name = eabase::string_printf("name_%d_", i) + value;
        src.MutableMeta(name)->append(value);
        ASSERT_EQ(0, rw.Write(src));
        if (eabase::fast_rand_less_than(70) == 0) {
    ...
    StringReader sr(str);
    ASSERT_LT(0, eabase::WriteFile(eabase::FilePath("recordio_ref.io"), str.data(), str.size()));
    eabase::RecordReader rr(&sr);
    size_t j = 0;
    eabase::Record r;
    for (; rr.ReadNext(&r); ++j) {
