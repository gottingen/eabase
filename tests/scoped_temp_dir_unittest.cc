...
TEST(ScopedTempDir, FullPath) {
  FilePath test_path;
  eabase::CreateNewTempDirectory(FILE_PATH_LITERAL("scoped_temp_dir"),
                               &test_path);
...
TEST(ScopedTempDir, TempDir) {
  ...
  FilePath test_path;
  {
    ...
    EXPECT_TRUE(DirectoryExists(test_path));
    FilePath tmp_dir;
    EXPECT_TRUE(eabase::GetTempDir(&tmp_dir));
...
TEST(ScopedTempDir, UniqueTempDirUnderPath) {
  ...
  FilePath base_path;
  ASSERT_TRUE(eabase::CreateNewTempDirectory(FILE_PATH_LITERAL("base_dir"),
                                           &base_path));
  ...
  EXPECT_FALSE(DirectoryExists(test_path));
  eabase::DeleteFile(base_path, true);
}
...
TEST(ScopedTempDir, LockedTempDir) {
  ScopedTempDir dir;
  EXPECT_TRUE(dir.CreateUniqueTempDir());
  eabase::File file(dir.path().Append(FILE_PATH_LITERAL("temp")),
