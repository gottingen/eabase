...
#include "eabase/utility/memory/scoped_vector.h"
...
class LifeCycleObject {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(LifeCycleObject);
};
...
class LifeCycleWatcher : public LifeCycleObject::Observer {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(LifeCycleWatcher);
};
...
TEST(ScopedVectorTest, PopBack) {
  LifeCycleWatcher watcher;
  EXPECT_EQ(LC_INITIAL, watcher.life_cycle_state());
  eabase::ScopedVector<LifeCycleObject> scoped_vector;
...
TEST(ScopedVectorTest, Scope2) {
  ...
  EXPECT_EQ(LC_INITIAL, watcher.life_cycle_state());
  {
    eabase::ScopedVector<LifeCycleObject> scoped_vector{ watcher.NewLifeCycleObject() };
    EXPECT_EQ(LC_CONSTRUCTED, watcher.life_cycle_state());
    EXPECT_TRUE(watcher.IsWatching(scoped_vector.back()));
  }
  EXPECT_EQ(LC_DESTROYED, watcher.life_cycle_state());
...
TEST(ScopedVectorTest, MoveConstruct) {
  ...
  EXPECT_EQ(LC_INITIAL, watcher.life_cycle_state());
  {
    ...
    eabase::ScopedVector<LifeCycleObject> scoped_vector_copy(scoped_vector.Pass());
    EXPECT_TRUE(scoped_vector.empty());
...
TEST(ScopedVectorTest, MoveAssign) {
  ...
  EXPECT_EQ(LC_INITIAL, watcher.life_cycle_state());
  {
    eabase::ScopedVector<LifeCycleObject> scoped_vector;
    scoped_vector.push_back(watcher.NewLifeCycleObject());
    eabase::ScopedVector<LifeCycleObject> scoped_vector_assign;
    EXPECT_FALSE(scoped_vector.empty());
    EXPECT_TRUE(watcher.IsWatching(scoped_vector.back()));
...
class DeleteCounter {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(DeleteCounter);
};

template <typename T>
eabase::ScopedVector<T> PassThru(eabase::ScopedVector<T> scoper) {
  return scoper.Pass();
}

TEST(ScopedVectorTest, Passed) {
  int deletes = 0;
  eabase::ScopedVector<DeleteCounter> deleter_vector;
  deleter_vector.push_back(new DeleteCounter(&deletes));
  EXPECT_EQ(0, deletes);
  
  EXPECT_EQ(0, deletes);
  eabase::ScopedVector<DeleteCounter> result = deleter_vector.Pass();
  EXPECT_EQ(0, deletes);
  result.clear();
  EXPECT_EQ(1, deletes);
};
