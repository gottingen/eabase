...
#include "eabase/utility/memory/shared_memory.h"
#include "eabase/utility/process/kill.h"
#include "eabase/utility/rand_util.h"
#include "eabase/utility/strings/string_number_conversions.h"
...
#include "eabase/utility/mac/scoped_nsautorelease_pool.h"
#endif
...
class MultipleThreadMain : public PlatformThread::Delegate {
 public:
  ...
  virtual void ThreadMain() OVERRIDE {
    ...
    for (int idx = 0; idx < 100; idx++) {
      *ptr = idx;
      PlatformThread::Sleep(eabase::TimeDelta::FromMilliseconds(1));
      EXPECT_EQ(*ptr, idx);
    }
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(MultipleThreadMain);
};
...
class MultipleLockThread : public PlatformThread::Delegate {
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(MultipleLockThread);
};
#endif
...
TEST(SharedMemoryTest, OpenExclusive) {
  ...
  EXPECT_LT(memory1.mapped_size(),
            kDataSize + eabase::SysInfo::VMAllocationGranularity());
  ...
  EXPECT_LT(memory2.mapped_size(),
            kDataSize2 + eabase::SysInfo::VMAllocationGranularity());
...
TEST(SharedMemoryTest, ShareReadOnly) {
  ...
  if (rv)
    eabase::win::ScopedHandle writable_handle(temp_handle);
#else
