...
#include "eabase/utility/status.h"
...
TEST_F(StatusTest, success_status) {
    ...
    eabase::Status st3 = eabase::Status::OK();
    ASSERT_TRUE(st3.ok());
...
TEST_F(StatusTest, failed_status) {
    ...
    eabase::Status st1(ENOMEM, NO_MEMORY_STR);
    ASSERT_FALSE(st1.ok());
    ...
    eabase::Status st2(EINVAL, "%s%s", NO_MEMORY_STR, NO_CPU_STR);
    ASSERT_FALSE(st2.ok());
    ...
    eabase::Status st3(ENOMEM, NO_MEMORY_STR);
    ASSERT_FALSE(st3.ok());
    ...
    eabase::Status st4(EINVAL, "%s%s", NO_MEMORY_STR, NO_CPU_STR);
    ASSERT_FALSE(st4.ok());
...
TEST_F(StatusTest, message_has_zero) {
    ...
    eabase::StringPiece slice(str);
    ASSERT_EQ(11UL, slice.as_string().size());
    str[5] = '\0';
    ASSERT_EQ(11UL, slice.as_string().size());
    eabase::Status st1(ENOMEM, slice);
    ASSERT_FALSE(st1.ok());
    ASSERT_EQ(ENOMEM, st1.error_code());
