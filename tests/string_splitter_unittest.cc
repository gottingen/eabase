...
TEST_F(StringSplitterTest, sanity) {
    ...
    eabase::StringSplitter ss2(str, ' ', eabase::ALLOW_EMPTY_FIELD);
...
TEST_F(StringSplitterTest, number_list) {
    ...
    eabase::StringMultiSplitter ss(str, ", ");
    ASSERT_TRUE(ss);
    ASSERT_EQ(3ul, ss.length());
    ASSERT_FALSE(strncmp(ss.field(), "123", ss.length()));
    ...
    eabase::StringMultiSplitter ss2(str, str + str_len, ", ");
    ASSERT_TRUE(ss2);
    ASSERT_EQ(3ul, ss2.length());
...
TEST_F(StringSplitterTest, split_limit_len) {
    ...
    eabase::StringSplitter ss(str, str + 5, '\t');
    ...
    eabase::StringSplitter ss2(str, str + 5, '\0');
    ...
    eabase::StringPiece sp(str, 5);
    // Allows using '\0' as separator
    eabase::StringSplitter ss3(sp, '\0');
...
TEST_F(StringSplitterTest, key_value_pairs_splitter_sanity) {
    ...
    for (int i = 0 ; i < 3; ++i) {
        ...
        eabase::KeyValuePairsSplitter* psplitter = NULL;
        if (i == 0) {
            psplitter = new eabase::KeyValuePairsSplitter(kvstr, '&', '=');
        } else if (i == 1) {
            psplitter = new eabase::KeyValuePairsSplitter(
                    kvstr.data(), kvstr.data() + kvstr.size(), '&', '=');
        } else if (i == 2) {
            psplitter = new eabase::KeyValuePairsSplitter(kvstr.c_str(), '&', '=');
        }
        eabase::KeyValuePairsSplitter& splitter = *psplitter;
