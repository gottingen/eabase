...
#include "eabase/utility/threading/thread_checker.h"
#include "eabase/utility/threading/simple_thread.h"
#include <gtest/gtest.h>

// Duplicated from eabase/utility/threading/thread_checker.h so that we can be
...
class ThreadCheckerClass : public ThreadChecker {
 ...
 private:
  EA_DISALLOW_COPY_AND_ASSIGN(ThreadCheckerClass);
};

// Calls ThreadCheckerClass::DoStuff on another thread.
class CallDoStuffOnThread : public eabase::SimpleThread {
 public:
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(CallDoStuffOnThread);
};

// Deletes ThreadCheckerClass on a different thread.
class DeleteThreadCheckerClassOnThread : public eabase::SimpleThread {
 public:
 ...
 private:
  ...
  EA_DISALLOW_COPY_AND_ASSIGN(DeleteThreadCheckerClassOnThread);
};
