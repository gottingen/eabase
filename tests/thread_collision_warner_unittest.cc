...
#include "eabase/utility/threading/thread_collision_warner.h"
...
#undef DFAKE_MUTEX
#define DFAKE_MUTEX(obj) scoped_ptr<eabase::AsserterBase> obj
...
// place.
class AssertReporter : public eabase::AsserterBase {
 public:
  AssertReporter()
      : failed_(false) {}
...
TEST(ThreadCollisionTest, BookCriticalSection) {
  ...
  eabase::ThreadCollisionWarner warner(local_reporter);
...
TEST(ThreadCollisionTest, MTBookCriticalSectionTest) {
  class NonThreadSafeQueue {
   public:
    explicit NonThreadSafeQueue(eabase::AsserterBase* asserter)
        : push_pop_(asserter) {
    }
   ...
   private:
    ...
    EA_DISALLOW_COPY_AND_ASSIGN(NonThreadSafeQueue);
  };

  class QueueUser : public eabase::DelegateSimpleThread::Delegate {
   public:
  ...
  eabase::DelegateSimpleThread thread_a(&queue_user_a, "queue_user_thread_a");
  eabase::DelegateSimpleThread thread_b(&queue_user_b, "queue_user_thread_b");
  ...
  class NonThreadSafeQueue {
   public:
    ...
    void push(int value) {
      ...
      eabase::PlatformThread::Sleep(eabase::TimeDelta::FromSeconds(5));
    }
...
TEST(ThreadCollisionTest, MTSynchedScopedBookCriticalSectionTest) {
  ...
  class NonThreadSafeQueue {
   public:
    ...
    void push(int value) {
      DFAKE_SCOPED_LOCK(push_pop_);
      eabase::PlatformThread::Sleep(eabase::TimeDelta::FromSeconds(2));
    }
  ...
  class QueueUser : public eabase::DelegateSimpleThread::Delegate {
   public:
    QueueUser(NonThreadSafeQueue& queue, eabase::Lock& lock)
        : queue_(queue),
          lock_(lock) {}

    virtual void Run() OVERRIDE {
      {
        eabase::AutoLock auto_lock(lock_);
        queue_.push(0);
      }
      {
        eabase::AutoLock auto_lock(lock_);
