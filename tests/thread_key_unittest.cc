...
#include "eabase/utility/thread_key.h"
...
TEST(ThreadLocalTest, sanity) {
    {
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            ASSERT_EQ(0, eabase::thread_key_create(key, NULL));

            ASSERT_EQ(NULL, eabase::thread_getspecific(key));
            ASSERT_EQ(0, eabase::thread_setspecific(key, (void *)raw_data));
            ASSERT_EQ(raw_data, eabase::thread_getspecific(key));

            ASSERT_EQ(0, eabase::thread_key_delete(key));
            ASSERT_EQ(NULL, eabase::thread_getspecific(key));
            ASSERT_NE(0, eabase::thread_setspecific(key, (void *)raw_data));
        }
    }
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        if (keys.empty() || create) {
            for (uint64_t j = 0; j < num; ++j) {
                keys.emplace_back();
                ASSERT_EQ(0, eabase::thread_key_create(keys.back(), NULL));
            ...
            for (uint64_t j = 0; j < num && !keys.empty(); ++j) {
                ...
                ASSERT_EQ(0, eabase::thread_key_delete(keys[index]));
...
void* THreadKeyCreateAndDeleteFunc(void* arg) {
    while (!g_stopped) {
        ThreadKey key;
        EXPECT_EQ(0, eabase::thread_key_create(key, NULL));
        EXPECT_TRUE(!KEY_UNUSED(key._seq));
        EXPECT_EQ(0, eabase::thread_key_delete(key));
    }
    return NULL;
}
...
struct EA_CACHELINE_ALIGNMENT ThreadKeyArg {
...
void* ThreadKeyFunc(void* arg) {
    ...
    for (auto key : thread_keys) {
        EXPECT_TRUE(eabase::thread_getspecific(*key) == NULL);
        EXPECT_EQ(0, eabase::thread_setspecific(*key, new int(0)));
        EXPECT_EQ(*(static_cast<int*>(eabase::thread_getspecific(*key))), 0);
    }
    while (!g_stopped) {
        uint64_t index =
            fast_rand_less_than(thread_keys.size());
        auto data = static_cast<int*>(eabase::thread_getspecific(*thread_keys[index]));
...
TEST(ThreadLocalTest, thread_key_multi_thread) {
    ...
    for (int i = 0; i < key_num; ++i) {
        ...
        ASSERT_EQ(0, eabase::thread_key_create(*thread_keys.back(), [](void* data) {
            delete static_cast<int*>(data);
        }));
        ASSERT_TRUE(eabase::thread_getspecific(*thread_keys.back()) == NULL);
        ASSERT_EQ(0, eabase::thread_setspecific(*thread_keys.back(), new int(0)));
        ASSERT_EQ(*(static_cast<int*>(eabase::thread_getspecific(*thread_keys.back()))), 0);
    }
    const int thread_num = 8;
    ...
    for (auto key : thread_keys) {
        ASSERT_EQ(0, eabase::thread_key_delete(*key));
        ASSERT_TRUE(eabase::thread_getspecific(*key) == NULL);
    }
    g_deleted = true;
...
struct EA_CACHELINE_ALIGNMENT ThreadKeyPerfArgs {
...
void* ThreadKeyPerfFunc(void* void_arg) {
    ...
    if (args->is_pthread_key) {
        ...
        eabase::thread_setspecific(*args->thread_key, (void*)data.get());
    }
    eabase::Timer t;
    while (!g_stopped) {
        if (g_started) {
    ...
    while (!g_stopped) {
        if (args->is_pthread_key) {
            ...
            eabase::thread_getspecific(*args->thread_key);
        }
        ++args->counter;
    }
    t.stop();
...
void ThreadKeyPerfTest(int thread_num, bool test_pthread_key) {
    ...
    g_stopped = false;
    pthread_key_t pthread_key;
    eabase::ThreadKey thread_key;
    if (test_pthread_key) {
        ASSERT_EQ(0, pthread_key_create(&pthread_key, NULL));
    } else {
        ASSERT_EQ(0, eabase::thread_key_create(thread_key, NULL));
    }
    pthread_t threads[thread_num];
    ...
    if (test_pthread_key) {
        ...
        ASSERT_EQ(0, eabase::thread_key_delete(thread_key));
    }
...
struct EA_CACHELINE_ALIGNMENT ThreadLocalPerfArgs {
    ThreadLocal<int>* tl;
    int64_t counter;
    int64_t elapse_ns;
    bool ready;
