...
#include "eabase/utility/threading/thread_local.h"
#include "eabase/utility/synchronization/waitable_event.h"
...
class ThreadLocalTesterBase : public eabase::DelegateSimpleThreadPool::Delegate {
 public:
  typedef eabase::ThreadLocalPointer<ThreadLocalTesterBase> TLPType;

  ThreadLocalTesterBase(TLPType* tlp, eabase::WaitableEvent* done)
      : tlp_(tlp),
        done_(done) {
  }
 ...
 protected:
  TLPType* tlp_;
  eabase::WaitableEvent* done_;
};

class SetThreadLocal : public ThreadLocalTesterBase {
 public:
  SetThreadLocal(TLPType* tlp, eabase::WaitableEvent* done)
...
class GetThreadLocal : public ThreadLocalTesterBase {
 public:
  GetThreadLocal(TLPType* tlp, eabase::WaitableEvent* done)
      : ThreadLocalTesterBase(tlp, done),
        ptr_(NULL) {
  }
...
TEST(ThreadLocalTest, Pointer) {
  eabase::DelegateSimpleThreadPool tp1("ThreadLocalTest tp1", 1);
  eabase::DelegateSimpleThreadPool tp2("ThreadLocalTest tp1", 1);
  tp1.Start();
  tp2.Start();

  eabase::ThreadLocalPointer<ThreadLocalTesterBase> tlp;
  ...
  eabase::WaitableEvent done(true, false);
