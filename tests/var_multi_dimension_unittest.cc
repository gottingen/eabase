...
#include "eabase/rapidjson/rapidjson.h"
#include "eabase/rapidjson/document.h"
...
static long start_perf_test_with_madder(size_t num_thread, eabase::Adder<uint64_t>* adder) {
...
static void *thread_maxer(void *arg) {
    eabase::Maxer<uint64_t> *reducer = (eabase::Maxer<uint64_t> *)arg;
    eabase::Timer timer;
    timer.start();
...
static long start_perf_test_with_mmaxer(size_t num_thread, eabase::Maxer<uint64_t>* maxer) {
...
static void *thread_miner(void *arg) {
    eabase::Miner<uint64_t> *reducer = (eabase::Miner<uint64_t> *)arg;
    eabase::Timer timer;
    timer.start();
    for (size_t i = 1; i <= OPS_PER_THREAD; ++i) {
        (*reducer) << -2 * i * OPS_PER_THREAD;
...
static long start_perf_test_with_mminer(size_t num_thread, eabase::Miner<uint64_t>* miner) {
...
static void *thread_intrecorder(void *arg) {
    eabase::IntRecorder *reducer = (eabase::IntRecorder *)arg;
    eabase::Timer timer;
    timer.start();
...
static long start_perf_test_with_mintrecorder(size_t num_thread, eabase::IntRecorder* intrecorder) {
...
TEST_F(MultiDimensionTest, madder) {
    ...
    eabase::MultiDimension<eabase::Adder<uint32_t> > my_madder1("request_count_madder_uint32_t", labels);
    eabase::Adder<uint32_t>* my_adder1 = my_madder1.get_stats(labels_value);
    ASSERT_TRUE(my_adder1);
    ASSERT_TRUE(my_adder1->valid());
    *my_adder1 << 2 << 4;
    ASSERT_EQ(6u, my_adder1->get_value());

    eabase::MultiDimension<eabase::Adder<double> > my_madder2("request_count_madder_double", labels);
    eabase::Adder<double>* my_adder2 = my_madder2.get_stats(labels_value);
    ASSERT_TRUE(my_adder2);
    ASSERT_TRUE(my_adder2->valid());
    *my_adder2 <<  2.0 << 4.0;
    ASSERT_EQ(6.0, my_adder2->get_value());

    eabase::MultiDimension<eabase::Adder<int> > my_madder3("request_count_madder_int", labels);
    eabase::Adder<int>* my_adder3 = my_madder3.get_stats(labels_value);
    ASSERT_TRUE(my_adder3);
    ASSERT_TRUE(my_adder3->valid());
    *my_adder3 << -9 << 1 << 0 << 3;
    ASSERT_EQ(-5, my_adder3->get_value());

    eabase::MultiDimension<eabase::Adder<std::string> > my_madder_str("my_string", labels);
    eabase::Adder<std::string> *my_str1 = my_madder_str.get_stats(labels_value);
    ASSERT_TRUE(my_str1);
...
TEST_F(MultiDimensionTest, mmadder_perf) {
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::MultiDimension<eabase::Adder<uint64_t> > my_madder1("request_count_madder_uint64_t", labels);
    eabase::Adder<uint64_t>* my_adder = my_madder1.get_stats(labels_value);
...
TEST_F(MultiDimensionTest, mmaxer) {
    eabase::MultiDimension<eabase::Maxer<int> > my_mmaxer("request_count_mmaxer", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::Maxer<int>* my_maxer = my_mmaxer.get_stats(labels_value);
...
TEST_F(MultiDimensionTest, mmaxer_perf) {
    eabase::MultiDimension<eabase::Maxer<uint64_t> > my_mmaxer("request_count_mmaxer", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::Maxer<uint64_t>* my_maxer = my_mmaxer.get_stats(labels_value);
    ASSERT_TRUE(my_maxer);
...
TEST_F(MultiDimensionTest, mminer) {
    eabase::MultiDimension<eabase::Miner<int> > my_mminer("client_request_count_mminer", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::Miner<int>* my_miner = my_mminer.get_stats(labels_value);
...
TEST_F(MultiDimensionTest, mminer_perf) {
    eabase::MultiDimension<eabase::Miner<uint64_t> > my_mminer("request_count_mminer", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::Miner<uint64_t>* my_miner = my_mminer.get_stats(labels_value);
    ASSERT_TRUE(my_miner);
...
TEST_F(MultiDimensionTest, mintrecoder) {
    eabase::MultiDimension<eabase::IntRecorder> my_mintrecorder("client_request_count_mintrecorder", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::IntRecorder* my_intrecorder = my_mintrecorder.get_stats(labels_value);
...
TEST_F(MultiDimensionTest, mintrecorder_perf) {
    eabase::MultiDimension<eabase::IntRecorder> my_mintrecorder("request_count_mintrecorder", labels);
    std::list<std::string> labels_value = {"bj", "get", "200"};
    eabase::IntRecorder* my_intrecorder = my_mintrecorder.get_stats(labels_value);
    ASSERT_TRUE(my_intrecorder);
...
TEST_F(MultiDimensionTest, stats) {
    ...
    eabase::MultiDimension<eabase::Adder<int> > my_madder("test_stats", labels);
    ...
    eabase::Adder<int>* adder4 = my_madder.get_stats(labels_value4);
    ASSERT_TRUE(adder4);
    ...
    ASSERT_FALSE(my_madder.has_stats(labels_value3));
    ASSERT_FALSE(my_madder.has_stats(labels_value4));
    eabase::Adder<int> *adder5 = my_madder.get_stats(labels_value1);
    ASSERT_TRUE(adder5);
}

TEST_F(MultiDimensionTest, get_description) {
    eabase::MultiDimension<eabase::Adder<int> > my_madder("test_get_description", labels);
    ...
    *adder2 << 2;
    ...
    rapidjson::Value& labels = doc["labels"];
...
TEST_F(MultiDimensionTest, mlatencyrecorder) {
    ...
    GFLAGS_NS::GetCommandLineOption("var_latency_p1", &old_var_latency_p1);
    GFLAGS_NS::GetCommandLineOption("var_latency_p2", &old_var_latency_p2);
    GFLAGS_NS::GetCommandLineOption("var_latency_p3", &old_var_latency_p3);

    GFLAGS_NS::SetCommandLineOption("var_dump_interval", "1");
    GFLAGS_NS::SetCommandLineOption("mvar_dump", "true");
    GFLAGS_NS::SetCommandLineOption("var_latency_p1", "60");
    GFLAGS_NS::SetCommandLineOption("var_latency_p2", "70");
    GFLAGS_NS::SetCommandLineOption("var_latency_p3", "80");

    eabase::MultiDimension<eabase::LatencyRecorder> my_mlatencyrecorder("client_request_count_mlatencyrecorder", labels);
    std::list<std::string> labels_value = {"tc", "get", "200"};
    eabase::LatencyRecorder* my_latencyrecorder = my_mlatencyrecorder.get_stats(labels_value);
    ...
    *my_latencyrecorder << 1 << 2 << 3 << 4 << 5 << 6 << 7;
    ...
    GFLAGS_NS::SetCommandLineOption("var_latency_p1", old_var_latency_p1.c_str());
    GFLAGS_NS::SetCommandLineOption("var_latency_p2", old_var_latency_p2.c_str());
    GFLAGS_NS::SetCommandLineOption("var_latency_p3", old_var_latency_p3.c_str());
}

TEST_F(MultiDimensionTest, mstatus) {
    eabase::MultiDimension<eabase::Status<int> > my_mstatus("my_mstatus", labels);
    std::list<std::string> labels_value {"tc", "get", "200"};
    eabase::Status<int>* my_status = my_mstatus.get_stats(labels_value);
    ...
    for (int i = 0; i < idc_count; i++) {
        ...
        for (int j = 0; j < method_count; j++) {
            ...
            for (int k = 0; k < status_count; k++) {
                ...
                eabase::Timer timer(eabase::Timer::STARTED);
