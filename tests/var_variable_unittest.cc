...
TEST_F(VariableTest, status) {
    ...
    ASSERT_EQ("9", eabase::Variable::describe_exposed("var1"));
    ...
    ASSERT_EQ("9", eabase::Variable::describe_exposed("var1"));
    ASSERT_EQ("10", eabase::Variable::describe_exposed("var2"));
    eabase::Variable::list_exposed(&vars);
    ...
    ASSERT_EQ("", eabase::Variable::describe_exposed("Var2 Again"));
    ASSERT_EQ("10", eabase::Variable::describe_exposed("var2_again"));
    eabase::Variable::list_exposed(&vars);
    ASSERT_EQ(2UL, vars.size());
    ASSERT_EQ("var1", vars[0]);
    ...
    eabase::Status<int> st3("var3", 11);
    ASSERT_EQ("var3", st3.name());
    ASSERT_EQ(3UL, eabase::Variable::count_exposed());
    ASSERT_EQ("11", eabase::Variable::describe_exposed("var3"));
    eabase::Variable::list_exposed(&vars);
...
#endif
    ASSERT_EQ("12", eabase::Variable::describe_exposed("var4"));
    eabase::Variable::list_exposed(&vars);
...
class MyDumper : public eabase::Dumper {
public:
    bool dump(const std::string& name,
              const eabase::StringPiece& description) {
...
TEST_F(VariableTest, dump) {
    ...
    // Nothing to dump yet.
    eabase::FLAGS_var_log_dumpped = true;
    ASSERT_EQ(0, eabase::Variable::dump_exposed(&d, NULL));
    ASSERT_TRUE(d._list.empty());

    eabase::Adder<int> v2("var2");
    v2 << 2;
    eabase::Status<int> v1("var1", 1);
    eabase::Status<int> v1_2("var1", 12);
    eabase::Status<int> v3("foo.bar.Apple", "var3", 3);
    eabase::Adder<int> v4("foo.bar.BaNaNa", "var4");
    v4 << 4;
    eabase::BasicPassiveStatus<int> v5(
        "foo::bar::Car_Rot", "var5", print_int, NULL);
    
    ASSERT_EQ(5, eabase::Variable::dump_exposed(&d, NULL));
    ...
    d._list.clear();
    eabase::DumpOptions opts;
    opts.white_wildcards = "foo_bar_*";
    opts.black_wildcards = "*var5";
    ASSERT_EQ(2, eabase::Variable::dump_exposed(&d, &opts));
    ...
    d._list.clear();
    opts = eabase::DumpOptions();
    opts.white_wildcards = "*?rot*";
    ASSERT_EQ(1, eabase::Variable::dump_exposed(&d, &opts));
    ...
    ASSERT_EQ(3, eabase::Variable::dump_exposed(&d, &opts));
    ...
    ASSERT_EQ(0, eabase::Variable::dump_exposed(&d, &opts));
...
TEST_F(VariableTest, latency_recorder) {
    ...
    eabase::Variable::list_exposed(&names);
